...
//
//    (1)  precond = new NALU_HYPRE_LSI_BlockP(NALU_HYPRE_IJMatrix Amat)
...
typedef struct NALU_HYPRE_LSI_BlockPrecond_Struct
{
   void *precon;
} 
NALU_HYPRE_LSI_BlockPrecond;
...
extern "C" 
int NALU_HYPRE_LSI_BlockPrecondCreate(MPI_Comm mpi_comm, NALU_HYPRE_Solver *solver)
{
   (void) mpi_comm;
...
extern "C" int NALU_HYPRE_LSI_BlockPrecondDestroy(NALU_HYPRE_Solver solver)
{
   int err=0;

   NALU_HYPRE_LSI_BlockPrecond *cprecon = (NALU_HYPRE_LSI_BlockPrecond *) solver;
   if ( cprecon == NULL ) err = 1;
   else
   {
      NALU_HYPRE_LSI_BlockP *precon = (NALU_HYPRE_LSI_BlockP *) cprecon->precon;
...
extern "C"
int NALU_HYPRE_LSI_BlockPrecondSetLumpedMasses(NALU_HYPRE_Solver solver, int length, 
                                          double *mass_v)
{
   int err=0;
...
extern "C" int NALU_HYPRE_LSI_BlockPrecondSetParams(NALU_HYPRE_Solver solver,
                                               char *params)
{
   int err=0;
...
extern "C" int NALU_HYPRE_LSI_BlockPrecondSetLookup(NALU_HYPRE_Solver solver, 
                                               NALU_HYPRE_Lookup *lookup)
{
   int err=0;
...
extern "C" 
int NALU_HYPRE_LSI_BlockPrecondSetup(NALU_HYPRE_Solver solver, NALU_HYPRE_ParCSRMatrix Amat,
...
extern "C" 
int NALU_HYPRE_LSI_BlockPrecondSolve(NALU_HYPRE_Solver solver, NALU_HYPRE_ParCSRMatrix Amat,
                                NALU_HYPRE_ParVector b, NALU_HYPRE_ParVector x)
{
   int err=0;
...
extern "C" int NALU_HYPRE_LSI_BlockPrecondSetA11Tolerance(NALU_HYPRE_Solver solver,
                                                     double tol)
{
   int    err=0;
   double tol2;
...
NALU_HYPRE_LSI_BlockP::NALU_HYPRE_LSI_BlockP()
{
   Amat_                     = NULL;
   A11mat_                   = NULL;
   A12mat_                   = NULL;
   ...
   scheme_                   = NALU_HYPRE_INCFLOW_BDIAG;
   printFlag_                = 0;
   A11Solver_                = NULL;
   A11Precond_               = NULL;
   A22Solver_                = NULL;
...
NALU_HYPRE_LSI_BlockP::~NALU_HYPRE_LSI_BlockP()
{
   if ( A11mat_         != NULL ) NALU_HYPRE_IJMatrixDestroy(A11mat_);
   if ( A12mat_         != NULL ) NALU_HYPRE_IJMatrixDestroy(A12mat_);
   if ( A22mat_         != NULL ) NALU_HYPRE_IJMatrixDestroy(A22mat_);
   ...
   if ( F1vec_          != NULL ) NALU_HYPRE_IJVectorDestroy( F1vec_ );
   if ( F2vec_          != NULL ) NALU_HYPRE_IJVectorDestroy( F2vec_ );
   if ( X1vec_          != NULL ) NALU_HYPRE_IJVectorDestroy( X1vec_ );
   if ( X2vec_          != NULL ) NALU_HYPRE_IJVectorDestroy( X2vec_ );
   if ( X1aux_          != NULL ) NALU_HYPRE_IJVectorDestroy( X1aux_ );
...
int NALU_HYPRE_LSI_BlockP::setLumpedMasses(int length, double *Mdata)
{
   if ( length <= 0 )
   {
      printf("NALU_HYPRE_LSI_BlockP setLumpedMasses ERROR : M has length <= 0\n");
      exit(1);
   }
...
int NALU_HYPRE_LSI_BlockP::setParams(char *params)
{
   ...
   {
      printf("NALU_HYPRE_LSI_BlockP::parameters not for me.\n");
      return 1;
   }
   ...
   else if ( !strcmp(param2, "blockD") )
   {
      scheme_ = NALU_HYPRE_INCFLOW_BDIAG;
      if ( outputLevel_ > 0 ) 
         printf("NALU_HYPRE_LSI_BlockP::select block diagonal.\n");
   }
   else if ( !strcmp(param2, "blockS") )
   {
      scheme_ = NALU_HYPRE_INCFLOW_SDIAG;
      if ( outputLevel_ > 0 ) 
         printf("NALU_HYPRE_LSI_BlockP::select block diagonal with Schur.\n");
   }
   else if ( !strcmp(param2, "blockT") )
   {
      scheme_ = NALU_HYPRE_INCFLOW_BTRI;
      if ( outputLevel_ > 0 ) 
         printf("NALU_HYPRE_LSI_BlockP::select block triangular.\n");
   }
   else if ( !strcmp(param2, "blockLU") )
   {
      scheme_ = NALU_HYPRE_INCFLOW_BLU;
      if ( outputLevel_ > 0 ) 
         printf("NALU_HYPRE_LSI_BlockP::select block LU.\n");
   }
   else if ( !strcmp(param2, "outputLevel") )
   {
      sscanf(params,"%s %s %d", param1, param2, &outputLevel_);
      if ( outputLevel_ > 0 ) 
         printf("NALU_HYPRE_LSI_BlockP::outputLevel = %d.\n", outputLevel_);
   }
   else if ( !strcmp(param2, "block1FieldID") )
   {
      sscanf(params,"%s %s %d", param1, param2, &block1FieldID_);
      if ( outputLevel_ > 0 ) 
         printf("NALU_HYPRE_LSI_BlockP::block1FieldID = %d.\n", block1FieldID_);
   }
   else if ( !strcmp(param2, "block2FieldID") )
   {
      sscanf(params,"%s %s %d", param1, param2, &block2FieldID_);
      if ( outputLevel_ > 0 ) 
         printf("NALU_HYPRE_LSI_BlockP::block2FieldID = %d.\n", block2FieldID_);
   }
   else if ( !strcmp(param2, "printInfo") )
   {
      printFlag_ = 1;
      if ( outputLevel_ > 0 ) 
         printf("NALU_HYPRE_LSI_BlockP::set print flag.\n");
   }
   else if ( !strcmp(param2, "invA11PSNlevels") )
   {
      sscanf(params,"%s %s %d", param1, param2, &lumpedMassNlevels_);
      if ( outputLevel_ > 0 ) 
         printf("NALU_HYPRE_LSI_BlockP::invA11PSNlevels = %d.\n",lumpedMassNlevels_);
   }
   else if ( !strcmp(param2, "invA11PSThresh") )
   {
      sscanf(params,"%s %s %lg", param1, param2, &lumpedMassThresh_);
      if ( outputLevel_ > 0 ) 
         printf("NALU_HYPRE_LSI_BlockP::invA11PSThresh = %e.\n", lumpedMassThresh_);
   }
   else if ( !strcmp(param2, "invA11Scheme") )
   {
      ...
      {
         lumpedMassScheme_ = 0;
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::invA11Scheme=diag\n");
      }
      else if ( !strcmp(param3, "ainv") ) 
      {
         lumpedMassScheme_ = 1;
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::invA11Scheme = ainv\n");
      }
   }
   else if ( !strcmp(param2, "A11Solver") )
   {
      ...
      if ( !strcmp(param3, "cg") ) 
      {
         ...
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11 solver = cg\n");
      }
      else if ( !strcmp(param3, "gmres") ) 
      {
         A11Params_.SolverID_ = 1;
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11 solver = gmres\n");
      }
      else if ( !strcmp(param3, "boomeramg") ) 
      {
         A11Params_.SolverID_ = 2;
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11 solver = boomeramg\n");
      }
      else if ( !strcmp(param3, "diagonal") ) 
      {
         A11Params_.SolverID_ = 3;
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11 solver = diag\n");
      }
      else if ( !strcmp(param3, "identity") ) 
      {
         A11Params_.SolverID_ = 9;
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11 solver = identity\n");
      }
      else
      {
         printf("NALU_HYPRE_LSI_BlockP::invalid A11 solver %s.\n",param3);
      }
   }
   else if ( !strcmp(param2, "A22Solver") )
   {
      ...
      if ( !strcmp(param3, "cg") ) 
      {
         ...
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22 solver = cg\n");
      }
      else if ( !strcmp(param3, "gmres") ) 
      {
         A22Params_.SolverID_ = 1;
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22 solver = gmres\n");
      }
      else if ( !strcmp(param3, "boomeramg") ) 
      {
         A22Params_.SolverID_ = 2;
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22 solver = boomeramg\n");
      }
      else if ( !strcmp(param3, "diagonal") ) 
      {
         A22Params_.SolverID_ = 3;
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22 solver = diag\n");
      }
      else if ( !strcmp(param3, "identity") ) 
      {
         A22Params_.SolverID_ = 9;
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22 solver = identity\n");
      }
      else
      {
         printf("NALU_HYPRE_LSI_BlockP::invalid A22 solver %s.\n",param3);
      }
   }
   else if ( !strcmp(param2, "A11Tolerance") )
   {
      ...
      if (outputLevel_ > 0) 
         printf("NALU_HYPRE_LSI_BlockP::A11 tol = %e\n", A11Params_.Tol_);
   }
   else if ( !strcmp(param2, "A22Tolerance") )
   {
      ...
      if (outputLevel_ > 0) 
         printf("NALU_HYPRE_LSI_BlockP::A22 tol = %e\n", A22Params_.Tol_);
   }
   else if ( !strcmp(param2, "A11MaxIterations") )
   {
      ...
      if (outputLevel_ > 0) 
         printf("NALU_HYPRE_LSI_BlockP::A11 maxiter = %d\n", A11Params_.MaxIter_);
   }
   else if ( !strcmp(param2, "A22MaxIterations") )
   {
      ...
      if (outputLevel_ > 0) 
         printf("NALU_HYPRE_LSI_BlockP::A22 maxiter = %d\n", A22Params_.MaxIter_);
   }
   else if ( !strcmp(param2, "A11Precon") )
   {
      ...
      if ( !strcmp(param3, "diagonal") ) 
      {
         ...
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A11 precon = diagonal\n");
      }
      else if ( !strcmp(param3, "parasails") ) 
      {
         A11Params_.PrecondID_ = 2;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A11 precon = parasails\n");
      }
      else if ( !strcmp(param3, "boomeramg") ) 
      {
         A11Params_.PrecondID_ = 3;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A11 precon = boomeramg\n");
      }
      else if ( !strcmp(param3, "pilut") ) 
      {
         A11Params_.PrecondID_ = 4;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A11 precon = pilut\n");
      }
      else if ( !strcmp(param3, "euclid") ) 
      {
         A11Params_.PrecondID_ = 5;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A11 precon = euclid\n");
      }
      else if ( !strcmp(param3, "ddilut") ) 
      {
         A11Params_.PrecondID_ = 6;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A11 precon = ddilut\n");
      }
      else if ( !strcmp(param3, "ml") ) 
      {
         A11Params_.PrecondID_ = 7;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A11 precon = ml\n");
      }
      else if ( !strcmp(param3, "mli") ) 
      {
         A11Params_.PrecondID_ = 8;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A11 precon = mli\n");
      }
   }
   else if ( !strcmp(param2, "A22Precon") )
   {
      ...
      if ( !strcmp(param3, "diagonal") ) 
      {
         ...
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A22 precon = diagonal\n");
      }
      else if ( !strcmp(param3, "parasails") ) 
      {
         A22Params_.PrecondID_ = 2;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A22 precon = parasails\n");
      }
      else if ( !strcmp(param3, "boomeramg") ) 
      {
         A22Params_.PrecondID_ = 3;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A22 precon = boomeramg\n");
      }
      else if ( !strcmp(param3, "pilut") ) 
      {
         A22Params_.PrecondID_ = 4;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A22 precon = pilut\n");
      }
      else if ( !strcmp(param3, "euclid") ) 
      {
         A22Params_.PrecondID_ = 5;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A22 precon = euclid\n");
      }
      else if ( !strcmp(param3, "ddilut") ) 
      {
         A22Params_.PrecondID_ = 6;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A22 precon = ddilut\n");
      }
      else if ( !strcmp(param3, "ml") ) 
      {
         A22Params_.PrecondID_ = 7;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A22 precon = ml\n");
      }
      else if ( !strcmp(param3, "mli") ) 
      {
         A22Params_.PrecondID_ = 8;
         if (outputLevel_ > 0) 
            printf("NALU_HYPRE_LSI_BlockP::A22 precon = mli\n");
      }
   }
   else if ( !strcmp(param2, "A11PreconPSNlevels") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconPSNLevels\n");
   }
   else if ( !strcmp(param2, "A22PreconPSNlevels") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconPSNLevels\n");
   }
   else if ( !strcmp(param2, "A11PreconPSThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconPSThresh\n");
   }
   else if ( !strcmp(param2, "A22PreconPSThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconPSThresh\n");
   }
   else if ( !strcmp(param2, "A11PreconPSFilter") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconPSFilter\n");
   }
   else if ( !strcmp(param2, "A22PreconPSFilter") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconPSFilter\n");
   }
   else if ( !strcmp(param2, "A11PreconAMGThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconAMGThresh\n");
   }
   else if ( !strcmp(param2, "A22PreconAMGThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconAMGThresh\n");
   }
   else if ( !strcmp(param2, "A11PreconAMGRelaxType") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconAMGRelaxType\n");
   }
   else if ( !strcmp(param2, "A22PreconAMGRelaxType") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconAMGRelaxType\n");
   }
   else if ( !strcmp(param2, "A11PreconAMGNumSweeps") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconAMGNSweeps\n");
   }
   else if ( !strcmp(param2, "A22PreconAMGNumSweeps") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconAMGNSweeps\n");
   }
   else if ( !strcmp(param2, "A11PreconAMGSystemSize") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconAMGSystemSize\n");
   }
   else if ( !strcmp(param2, "A22PreconAMGSystemSize") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconAMGSystemSize\n");
   }
   else if ( !strcmp(param2, "A11PreconEuclidNLevels") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconEuclidNLevels\n");
   }
   else if ( !strcmp(param2, "A22PreconEuclidNLevels") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconEuclidNLevels\n");
   }
   else if ( !strcmp(param2, "A11PreconEuclidThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconEuclidThresh\n");
   }
   else if ( !strcmp(param2, "A22PreconEuclidThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconEuclidThresh\n");
   }
   else if ( !strcmp(param2, "A11PreconPilutFillin") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconPilutFillin\n");
   }
   else if ( !strcmp(param2, "A22PreconPilutFillin") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconPilutFillin\n");
   }
   else if ( !strcmp(param2, "A11PreconPilutDropTol") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconPilutDropTol\n");
   }
   else if ( !strcmp(param2, "A22PreconPilutDropTol") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconPilutDropTol\n");
   }
   else if ( !strcmp(param2, "A11PreconDDIlutFillin") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconDDIlutFillin\n");
   }
   else if ( !strcmp(param2, "A22PreconDDIlutFillin") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconDDIlutFillin\n");
   }
   else if ( !strcmp(param2, "A11PreconDDIlutDropTol") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconDDIlutDropTol\n");
   }
   else if ( !strcmp(param2, "A22PreconDDIlutDropTol") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconDDIlutDropTol\n");
   }
   else if ( !strcmp(param2, "A11PreconMLThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconMLThresh\n");
   }
   else if ( !strcmp(param2, "A22PreconMLThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconMLThresh\n");
   }
   else if ( !strcmp(param2, "A11PreconMLNumSweeps") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconMLNumSweeps\n");
   }
   else if ( !strcmp(param2, "A22PreconMLNumSweeps") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconMLNumSweeps\n");
   }
   else if ( !strcmp(param2, "A11PreconMLIThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconMLIThresh\n");
   }
   else if ( !strcmp(param2, "A22PreconMLIThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconMLIThresh\n");
   }
   else if ( !strcmp(param2, "A11PreconMLIRelaxType") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconMLIRelaxType\n");
   }
   else if ( !strcmp(param2, "A22PreconMLIRelaxType") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconMLIRelaxType\n");
   }
   else if ( !strcmp(param2, "A11PreconMLINumSweeps") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconMLINSweeps\n");
   }
   else if ( !strcmp(param2, "A22PreconMLINumSweeps") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconMLINSweeps\n");
   }
   else if ( !strcmp(param2, "A11PreconMLIPweight") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconMLIPweight\n");
   }
   else if ( !strcmp(param2, "A22PreconMLIPweight") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconMLIPweight\n");
   }
   else if ( !strcmp(param2, "A11PreconMLINodeDOF") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconMLINodeDOF\n");
   }
   else if ( !strcmp(param2, "A22PreconMLINodeDOF") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconMLINodeDOF\n");
   }
   else if ( !strcmp(param2, "A11PreconMLINullDim") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A11PreconMLINullDim\n");
   }
   else if ( !strcmp(param2, "A22PreconMLINullDim") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_BlockP::A22PreconMLINullDim\n");
   }
   else 
   {
      printf("NALU_HYPRE_LSI_BlockP:: string not recognized %s\n", params);
   }
   return 0;
}
...
int NALU_HYPRE_LSI_BlockP::setLookup(Lookup *object)
{
   lookup_ = object;
   return 0;
}
...
int NALU_HYPRE_LSI_BlockP::setup(NALU_HYPRE_ParCSRMatrix Amat)
{
   ...
   FILE     *fp;
   MPI_Comm mpi_comm;
   NALU_HYPRE_IJMatrix     Mmat, B22mat;
   NALU_HYPRE_ParCSRMatrix Cmat_csr, Mmat_csr, Smat_csr, A22mat_csr, B22mat_csr;
   NALU_HYPRE_ParCSRMatrix A11mat_csr;
   NALU_HYPRE_Solver       parasails;
   ...
   // Extract the velocity mass matrix in NALU_HYPRE_ParCSRMatrix format :
   // the mass matrix comes either from user (lumpedMassDiag_) or 
   // extracted from the diagonal of the A(1,1) matrix => mass_v
   //------------------------------------------------------------------

   NALU_HYPRE_ParCSRMatrixGetComm( Amat_, &mpi_comm );
   ...
   {
      NALU_HYPRE_ParaSailsCreate(mpi_comm, &parasails);
      NALU_HYPRE_ParaSailsSetParams(parasails,lumpedMassThresh_,
                               lumpedMassNlevels_);
      NALU_HYPRE_ParaSailsSetFilter(parasails, 0.1);
      NALU_HYPRE_ParaSailsSetLogging(parasails, 1);
      NALU_HYPRE_IJMatrixGetObject(A11mat_, (void **) &A11mat_csr);
      NALU_HYPRE_ParaSailsSetup(parasails, A11mat_csr, NULL, NULL);
      NALU_HYPRE_ParaSailsBuildIJMatrix(parasails, &Mmat);
   }
   else
   {
      ...
      NALU_HYPRE_IJMatrixCreate(mpi_comm, MStartRow, MStartRow+MNRows-1,
                           MStartRow, MStartRow+MNRows-1, &Mmat);
      NALU_HYPRE_IJMatrixSetObjectType(Mmat, NALU_HYPRE_PARCSR);
      NALU_HYPRE_IJMatrixSetRowSizes(Mmat, MRowLengs);
      NALU_HYPRE_IJMatrixInitialize(Mmat);
      delete [] MRowLengs;
      newRow = MStartRow;
      for ( irow = AStart; irow <= AEnd; irow++ ) 
      {
         ...
         if ( searchInd < 0 )
         {
            ...
            {
               NALU_HYPRE_ParCSRMatrixGetRow(Amat_,irow,&rowSize,&colInd,&colVal);
               for ( j = 0; j < rowSize; j++ ) 
                  if ( colInd[j] == irow ) { dtemp = colVal[j]; break;}
               NALU_HYPRE_ParCSRMatrixRestoreRow(Amat_,irow,&rowSize,&colInd,
                                            &colVal);
            }
            dtemp = 1.0 / dtemp;
            if (scheme_ == NALU_HYPRE_INCFLOW_BDIAG) dtemp = 0.0;
            NALU_HYPRE_IJMatrixSetValues(Mmat, 1, &one, (const int *) &newRow, 
   ...
   {
      B22mat = A22mat_;
      NALU_HYPRE_IJMatrixGetObject(B22mat, (void **) &B22mat_csr);
   } 
   else B22mat = NULL;
      
   SNRows    = P22Size_;
   SStartRow = P22Offsets_[mypid];
   NALU_HYPRE_IJMatrixCreate(mpi_comm, SStartRow, SStartRow+SNRows-1,
                        SStartRow, SStartRow+SNRows-1, &A22mat_);
   NALU_HYPRE_IJMatrixSetObjectType(A22mat_, NALU_HYPRE_PARCSR);

   SRowLengs = new int[SNRows];
   maxRowSize = 0;
   for ( irow = SStartRow; irow < SStartRow+SNRows; irow++ ) 
   {
      NALU_HYPRE_ParCSRMatrixGetRow(Smat_csr,irow,&rowSize,&colInd,NULL);
      newRowSize = rowSize;
      if ( B22mat != NULL )
      {
         NALU_HYPRE_ParCSRMatrixGetRow(B22mat_csr,irow,&rowSize2,&colInd2,NULL);
#if 1
         //==========================================
         // extract the whole S
         //------------------------------------------
         ...
         newRowSize = count;
         NALU_HYPRE_ParCSRMatrixRestoreRow(B22mat_csr,irow,&rowSize2,&colInd2,NULL);
         delete [] newColInd;
      }
      SRowLengs[irow-SStartRow] = newRowSize;
      maxRowSize = ( newRowSize > maxRowSize ) ? newRowSize : maxRowSize;
      NALU_HYPRE_ParCSRMatrixRestoreRow(Smat_csr,irow,&rowSize,&colInd,NULL);
   }
   NALU_HYPRE_IJMatrixSetRowSizes(A22mat_, SRowLengs);
   NALU_HYPRE_IJMatrixInitialize(A22mat_);
   delete [] SRowLengs;

   for ( irow = SStartRow; irow < SStartRow+SNRows; irow++ ) 
   {
      NALU_HYPRE_ParCSRMatrixGetRow(Smat_csr,irow,&rowSize,&colInd,&colVal);
      if ( B22mat == NULL )
      {
#if 1
         //==========================================
      ...
      {
         NALU_HYPRE_ParCSRMatrixGetRow(B22mat_csr,irow,&rowSize2,&colInd2,&colVal2);
#if 1
         //==========================================
         // extract the whole S
         //------------------------------------------
         ...
         if ( newRowSize > 0 ) count++;
         newRowSize = count;
         NALU_HYPRE_ParCSRMatrixRestoreRow(B22mat_csr,irow,&rowSize2,
                                      &colInd2,&colVal2);
      }
      NALU_HYPRE_IJMatrixSetValues(A22mat_, 1, &newRowSize, (const int *) &irow,
	                  (const int *) newColInd, (const double *) newColVal);
      NALU_HYPRE_ParCSRMatrixRestoreRow(Smat_csr,irow,&rowSize,&colInd,&colVal);
      delete [] newColInd;
      delete [] newColVal;
   }
   NALU_HYPRE_IJMatrixAssemble(A22mat_);
   NALU_HYPRE_IJMatrixGetObject(A22mat_, (void **) &A22mat_csr);
   if ( B22mat != NULL ) NALU_HYPRE_IJMatrixDestroy(B22mat);
   if ( Mmat != NULL ) NALU_HYPRE_IJMatrixDestroy(Mmat);
   if ( Smat_csr != NULL ) NALU_HYPRE_ParCSRMatrixDestroy(Smat_csr);
   ...
   if ( outputLevel_ > 2 && A22mat_csr != NULL )
   {
      ...
      for ( irow = SStartRow; irow < SStartRow+SNRows; irow++ ) 
      {
         NALU_HYPRE_ParCSRMatrixGetRow(A22mat_csr,irow,&rowSize,&colInd,&colVal);
         for ( j = 0; j < rowSize; j++ )
            fprintf(fp," %9d %9d %25.16e\n", irow+1, colInd[j]+1, colVal[j]);
         NALU_HYPRE_ParCSRMatrixRestoreRow(A22mat_csr,irow,&rowSize,&colInd,&colVal);
      }
      fclose(fp);
   }
   ...
   NALU_HYPRE_IJVectorCreate(mpi_comm, V1Start, V1Start+V1Leng-1, &F1vec_);
   NALU_HYPRE_IJVectorSetObjectType(F1vec_, NALU_HYPRE_PARCSR);
   NALU_HYPRE_IJVectorInitialize(F1vec_);
   NALU_HYPRE_IJVectorAssemble(F1vec_);

   NALU_HYPRE_IJVectorCreate(mpi_comm, V1Start, V1Start+V1Leng-1, &X1vec_);
   NALU_HYPRE_IJVectorSetObjectType(X1vec_, NALU_HYPRE_PARCSR);
   NALU_HYPRE_IJVectorInitialize(X1vec_);
   NALU_HYPRE_IJVectorAssemble(X1vec_);

   if ( scheme_ == NALU_HYPRE_INCFLOW_BLU )
   {
      NALU_HYPRE_IJVectorCreate(mpi_comm, V1Start, V1Start+V1Leng-1, &X1aux_);
      NALU_HYPRE_IJVectorSetObjectType(X1aux_, NALU_HYPRE_PARCSR);
      NALU_HYPRE_IJVectorInitialize(X1aux_);
      NALU_HYPRE_IJVectorAssemble(X1aux_);
   }

   V2Leng  = P22Size_;
   V2Start = P22Offsets_[mypid];
   NALU_HYPRE_IJVectorCreate(mpi_comm, V2Start, V2Start+V2Leng-1, &F2vec_);
   NALU_HYPRE_IJVectorSetObjectType(F2vec_, NALU_HYPRE_PARCSR);
   NALU_HYPRE_IJVectorInitialize(F2vec_);
   NALU_HYPRE_IJVectorAssemble(F2vec_);

   NALU_HYPRE_IJVectorCreate(mpi_comm, V2Start, V2Start+V2Leng-1, &X2vec_);
   NALU_HYPRE_IJVectorSetObjectType(X2vec_, NALU_HYPRE_PARCSR);
   NALU_HYPRE_IJVectorInitialize(X2vec_);
   NALU_HYPRE_IJVectorAssemble(X2vec_);
...
int NALU_HYPRE_LSI_BlockP::solve(NALU_HYPRE_ParVector fvec, NALU_HYPRE_ParVector xvec)
{
   ...
   for ( irow = AStart; irow < AEnd; irow++ ) 
   {
      ...
      if ( searchInd >= 0 )
      {
         ...
         NALU_HYPRE_IJVectorSetValues(F2vec_, 1, (const int *) &V2Cnt,
                                 (const double *) &ddata);
         V2Cnt++;
      }
      else
      {
         NALU_HYPRE_IJVectorSetValues(F1vec_, 1, (const int *) &V1Cnt,
   ...
   if ( A11Params_.SolverID_ == 0 )
      NALU_HYPRE_ParCSRPCGSetTol( A11Solver_, A11Params_.Tol_ );
   else if ( A11Params_.SolverID_ == 1 )
      NALU_HYPRE_ParCSRGMRESSetTol( A11Solver_, A11Params_.Tol_);
   else if ( A11Params_.SolverID_ == 2 )
      NALU_HYPRE_BoomerAMGSetTol( A11Solver_, A11Params_.Tol_);

   switch (scheme_)
   {
      case NALU_HYPRE_INCFLOW_BDIAG : solveBDSolve(X1vec_, X2vec_, F1vec_, F2vec_);
                                 break;

      case NALU_HYPRE_INCFLOW_SDIAG : solveBDSolve(X1vec_, X2vec_, F1vec_, F2vec_);
                                 break;

      case NALU_HYPRE_INCFLOW_BTRI :  solveBTSolve(X1vec_, X2vec_, F1vec_, F2vec_);
                                 break;

      case NALU_HYPRE_INCFLOW_BLU  :  solveBLUSolve(X1vec_, X2vec_, F1vec_, F2vec_);
                                 break;

      default :
           printf("NALU_HYPRE_LSI_BlockP ERROR : scheme not recognized.\n");
           exit(1);
   }
   ...
   for ( irow = AStart; irow < AEnd; irow++ ) 
   {
      ...
      {
         NALU_HYPRE_IJVectorGetValues(X1vec_, 1, &V1Cnt, &xvals[irow-AStart]);
         V1Cnt++;
      }
   } 
   return 0;
...
int NALU_HYPRE_LSI_BlockP::print()
{
   int      mypid;
   MPI_Comm mpi_comm;
   ...
   {
      printf("*****************************************************\n");
      printf("***********NALU_HYPRE_LSI_BlockP Information**************\n");
...
int NALU_HYPRE_LSI_BlockP::destroySolverPrecond()
{
   if ( A11Solver_ != NULL ) 
   {
      if      (A11Params_.SolverID_ == 0) NALU_HYPRE_ParCSRPCGDestroy(A11Solver_);
      else if (A11Params_.SolverID_ == 1) NALU_HYPRE_ParCSRGMRESDestroy(A11Solver_);
      else if (A11Params_.SolverID_ == 2) NALU_HYPRE_BoomerAMGDestroy(A11Solver_);
   }
   if ( A22Solver_ != NULL ) 
   {
      if      (A22Params_.SolverID_ == 0) NALU_HYPRE_ParCSRPCGDestroy(A22Solver_);
      else if (A22Params_.SolverID_ == 1) NALU_HYPRE_ParCSRGMRESDestroy(A22Solver_);
      else if (A22Params_.SolverID_ == 2) NALU_HYPRE_BoomerAMGDestroy(A22Solver_);
   }
   if ( A11Precond_ != NULL ) 
   {
      if (A11Params_.PrecondID_ == 2) NALU_HYPRE_ParCSRParaSailsDestroy(A11Precond_);
      else if (A11Params_.PrecondID_ == 3) NALU_HYPRE_BoomerAMGDestroy(A11Precond_);
      else if (A11Params_.PrecondID_ == 4) NALU_HYPRE_ParCSRPilutDestroy(A11Precond_);
      else if (A11Params_.PrecondID_ == 5) NALU_HYPRE_EuclidDestroy(A11Precond_);
      else if (A11Params_.PrecondID_ == 6) NALU_HYPRE_LSI_DDIlutDestroy(A11Precond_);
#ifdef HAVE_ML
      else if (A11Params_.PrecondID_ == 7) NALU_HYPRE_LSI_MLDestroy(A11Precond_);
#endif
#ifdef HAVE_MLI
      else if (A11Params_.PrecondID_ == 8) NALU_HYPRE_LSI_MLIDestroy(A11Precond_);
#endif
   }
   if ( A22Precond_ != NULL ) 
   {
      if (A22Params_.PrecondID_ == 2) NALU_HYPRE_ParCSRParaSailsDestroy(A22Precond_);
      else if (A22Params_.PrecondID_ == 3) NALU_HYPRE_BoomerAMGDestroy(A22Precond_);
      else if (A22Params_.PrecondID_ == 4) NALU_HYPRE_ParCSRPilutDestroy(A22Precond_);
      else if (A22Params_.PrecondID_ == 5) NALU_HYPRE_EuclidDestroy(A22Precond_);
      else if (A22Params_.PrecondID_ == 6) NALU_HYPRE_LSI_DDIlutDestroy(A22Precond_);
#ifdef HAVE_ML
      else if (A22Params_.PrecondID_ == 7) NALU_HYPRE_LSI_MLDestroy(A22Precond_);
#endif
#ifdef HAVE_MLI
      else if (A22Params_.PrecondID_ == 8) NALU_HYPRE_LSI_MLIDestroy(A22Precond_);
#endif
   }
   A11Solver_  = NULL;
   A22Solver_  = NULL;
...
int NALU_HYPRE_LSI_BlockP::computeBlockInfo()
{
   ...
   P22Offsets_    = NULL;
   assembled_     = 0;
   NALU_HYPRE_ParCSRMatrixGetRowPartitioning( Amat_, &APartition_ );
   NALU_HYPRE_ParCSRMatrixGetComm( Amat_, &mpi_comm );
   MPI_Comm_rank( mpi_comm, &mypid );
   MPI_Comm_size( mpi_comm, &nprocs );
   ...
   if ( block1FieldID_ == -3 && block2FieldID_ == -3 )
   {
      ...
      {
         NALU_HYPRE_ParCSRMatrixGetRow(Amat_,irow,&row_size,&col_ind,&col_val);
         zero_diag = 1;
         ...
         NALU_HYPRE_ParCSRMatrixRestoreRow(Amat_,irow,&row_size,&col_ind,&col_val);
...
int NALU_HYPRE_LSI_BlockP::buildBlocks()
{
   ...
   FILE   *fp;
   MPI_Comm mpi_comm;
   NALU_HYPRE_ParCSRMatrix A11mat_csr, A22mat_csr, A12mat_csr;

   //------------------------------------------------------------------
   // extract information about the system matrix
   //------------------------------------------------------------------

   NALU_HYPRE_ParCSRMatrixGetRowPartitioning( Amat_, &partition );
   NALU_HYPRE_ParCSRMatrixGetComm( Amat_, &mpi_comm );
   MPI_Comm_rank( mpi_comm, &mypid );
   MPI_Comm_size( mpi_comm, &nprocs );
   ...
   for ( irow = AStartRow; irow < AStartRow+ANRows; irow++ ) 
   {
      ...
      NALU_HYPRE_ParCSRMatrixRestoreRow(Amat_, irow, &rowSize, &inds, &vals);
   }
   ...
   NALU_HYPRE_IJMatrixCreate(mpi_comm, A11StartRow, A11StartRow+A11NRows-1,
                                A11StartCol, A11StartCol+A11NCols-1, &A11mat_);
   NALU_HYPRE_IJMatrixSetObjectType(A11mat_, NALU_HYPRE_PARCSR);
   NALU_HYPRE_IJMatrixSetRowSizes(A11mat_, A11RowLengs);
   NALU_HYPRE_IJMatrixInitialize(A11mat_);
   delete [] A11RowLengs;
   NALU_HYPRE_IJMatrixCreate(mpi_comm, A12StartRow, A12StartRow+A12NRows-1,
                        A12StartCol, A12StartCol+A12NCols-1, &A12mat_);
   NALU_HYPRE_IJMatrixSetObjectType(A12mat_, NALU_HYPRE_PARCSR);
   NALU_HYPRE_IJMatrixSetRowSizes(A12mat_, A12RowLengs);
   NALU_HYPRE_IJMatrixInitialize(A12mat_);
   delete [] A12RowLengs;
   if ( A22MaxRowLeng > 0 )
   {
      NALU_HYPRE_IJMatrixCreate(mpi_comm,A22StartRow,A22StartRow+A22NRows-1,
                           A22StartCol, A22StartCol+A22NCols-1, &A22mat_);
      NALU_HYPRE_IJMatrixSetObjectType(A22mat_, NALU_HYPRE_PARCSR);
      NALU_HYPRE_IJMatrixSetRowSizes(A22mat_, A22RowLengs);
      NALU_HYPRE_IJMatrixInitialize(A22mat_);
   }
   else A22mat_ = NULL;
   delete [] A22RowLengs;
   ...
   for ( irow = AStartRow; irow < AStartRow+ANRows; irow++ ) 
   {
      ...
      if ( searchInd < 0 )   // A(1,1) or A(1,2) block
      {
         ...
         {
            index = inds[j];
            searchInd = NALU_HYPRE_LSI_Search(P22GlobalInds_,index,P22GSize_);
         ...
         NALU_HYPRE_IJMatrixSetValues(A11mat_, 1, &A11NewSize, 
	                    (const int *) &A11RowCnt, (const int *) A11_inds,
                            (const double *) A11_vals);
         NALU_HYPRE_IJMatrixSetValues(A12mat_, 1, &A12NewSize, 
      ...
      else if ( A22MaxRowLeng > 0 ) // A(2,2) block
      {
         ...
         NALU_HYPRE_IJMatrixSetValues(A22mat_, 1, &A22NewSize, 
   ...
   if ( outputLevel_ >= 3 )
   {
      ...
      {
         NALU_HYPRE_ParCSRMatrixGetRow(A11mat_csr,irow,&rowSize,&inds,&vals);
         for ( j = 0; j < rowSize; j++ )
            fprintf(fp," %9d %9d %25.16e\n", irow+1, inds[j]+1, vals[j]);
         NALU_HYPRE_ParCSRMatrixRestoreRow(A11mat_csr,irow,&rowSize,&inds,&vals);
      }
      fclose(fp);
      ...
      {
         NALU_HYPRE_ParCSRMatrixGetRow(A12mat_csr,irow,&rowSize,&inds,&vals);
         for ( j = 0; j < rowSize; j++ )
            fprintf(fp, " %9d %9d %25.16e\n", irow+1, inds[j]+1, vals[j]);
         NALU_HYPRE_ParCSRMatrixRestoreRow(A12mat_csr,irow,&rowSize,&inds,&vals);
      }
      fclose(fp);
      if ( A22mat_csr != NULL )
      {
         ...
         {
            NALU_HYPRE_ParCSRMatrixGetRow(A22mat_csr,irow,&rowSize,&inds,&vals);
            for ( j = 0; j < rowSize; j++ )
               fprintf(fp," %9d %9d %25.16e\n", irow+1, inds[j]+1, vals[j]);
            NALU_HYPRE_ParCSRMatrixRestoreRow(A22mat_csr,irow,&rowSize,&inds,&vals);
         }
         fclose(fp);
      }
   }
...
int NALU_HYPRE_LSI_BlockP::setupPrecon(NALU_HYPRE_Solver *precon, NALU_HYPRE_IJMatrix Amat,
                                  NALU_HYPRE_LSI_BLOCKP_PARAMS param_ptr)
{
...
#endif
   MPI_Comm           mpi_comm;
   NALU_HYPRE_ParCSRMatrix Amat_csr;

   //------------------------------------------------------------------
   // fetch machine parameters 
   //------------------------------------------------------------------

   NALU_HYPRE_IJMatrixGetObject( Amat, (void **) &Amat_csr );
   NALU_HYPRE_ParCSRMatrixGetComm( Amat_csr, &mpi_comm );
   MPI_Comm_size(mpi_comm, &nprocs);
   precon = NULL;
   ...
   {
      case 2 : 
          NALU_HYPRE_ParCSRParaSailsCreate( mpi_comm, precon ); 
          if (param_ptr.SolverID_ == 0) NALU_HYPRE_ParCSRParaSailsSetSym(*precon,1);
          else                          NALU_HYPRE_ParCSRParaSailsSetSym(*precon,0);
          NALU_HYPRE_ParCSRParaSailsSetParams(*precon, param_ptr.PSThresh_,
                                         param_ptr.PSNLevels_);
          NALU_HYPRE_ParCSRParaSailsSetFilter(*precon, param_ptr.PSFilter_);
          break;
      case 3 :
          NALU_HYPRE_BoomerAMGCreate(precon);
          NALU_HYPRE_BoomerAMGSetMaxIter(*precon, 1);
          NALU_HYPRE_BoomerAMGSetCycleType(*precon, 1);
          NALU_HYPRE_BoomerAMGSetPrintLevel(*precon, outputLevel_);
          NALU_HYPRE_BoomerAMGSetMaxLevels(*precon, 25);
          NALU_HYPRE_BoomerAMGSetMeasureType(*precon, 0);
          NALU_HYPRE_BoomerAMGSetCoarsenType(*precon, 0);
          ...
          //   NALU_HYPRE_BoomerAMGSetRelaxOmega(*precon, relax_wt);
          //}
          break;
      case 4 :
          NALU_HYPRE_ParCSRPilutCreate( mpi_comm, precon );
          NALU_HYPRE_ParCSRPilutSetMaxIter( *precon, 1 );
          NALU_HYPRE_ParCSRPilutSetFactorRowSize(*precon,param_ptr.PilutFillin_);
          NALU_HYPRE_ParCSRPilutSetDropTolerance(*precon,param_ptr.PilutDropTol_);
          break;
      case 5 :
          ...
          NALU_HYPRE_EuclidSetParams(*precon, 4, targv);
          for ( i = 0; i < 4; i++ ) free(targv[i]);
          free(targv);
          break;
      case 6 :
          NALU_HYPRE_LSI_DDIlutCreate( mpi_comm, precon );
          NALU_HYPRE_LSI_DDIlutSetFillin(*precon, param_ptr.DDIlutFillin_);
          NALU_HYPRE_LSI_DDIlutSetDropTolerance(*precon, param_ptr.DDIlutDropTol_);
          break;
      case 7 :
#ifdef HAVE_ML
          NALU_HYPRE_LSI_MLCreate( mpi_comm, precon );
          NALU_HYPRE_LSI_MLSetCoarseSolver(*precon, 0);
          NALU_HYPRE_LSI_MLSetStrongThreshold(*precon, param_ptr.MLThresh_);
          NALU_HYPRE_LSI_MLSetNumPreSmoothings(*precon, param_ptr.MLNSweeps_);
          NALU_HYPRE_LSI_MLSetNumPostSmoothings(*precon,param_ptr.MLNSweeps_);
          NALU_HYPRE_LSI_MLSetPreSmoother(*precon, 1);
          NALU_HYPRE_LSI_MLSetPostSmoother(*precon, 1);
          break;
#else
      ...
      case 8 :
#ifdef HAVE_MLI
          NALU_HYPRE_LSI_MLICreate( mpi_comm, precon );
          sprintf(paramString, "MLI outputLevel %d", outputLevel_); 
          NALU_HYPRE_LSI_MLISetParams(*precon, paramString); 
...
int NALU_HYPRE_LSI_BlockP::setupSolver(NALU_HYPRE_Solver *solver, NALU_HYPRE_IJMatrix Amat,
                                  NALU_HYPRE_IJVector fvec, NALU_HYPRE_IJVector xvec,
                                  NALU_HYPRE_Solver precon, 
                                  NALU_HYPRE_LSI_BLOCKP_PARAMS param_ptr)
{
   int                i, nprocs, *nsweeps, *relaxType;
   MPI_Comm           mpi_comm;
   NALU_HYPRE_ParCSRMatrix Amat_csr;
   NALU_HYPRE_ParVector    f_csr, x_csr;
   ...
   {
      case 0 :
          NALU_HYPRE_ParCSRPCGCreate(mpi_comm, solver);
          NALU_HYPRE_ParCSRPCGSetMaxIter(*solver, param_ptr.MaxIter_ );
          NALU_HYPRE_ParCSRPCGSetTol(*solver, param_ptr.Tol_);
          NALU_HYPRE_ParCSRPCGSetLogging(*solver, outputLevel_);
          NALU_HYPRE_ParCSRPCGSetRelChange(*solver, 0);
          NALU_HYPRE_ParCSRPCGSetTwoNorm(*solver, 1);
          switch ( param_ptr.PrecondID_ )
          {
             case 1 : 
                  NALU_HYPRE_ParCSRPCGSetPrecond(*solver, NALU_HYPRE_ParCSRDiagScale,
                                         NALU_HYPRE_ParCSRDiagScaleSetup,precon);
                  break;
             case 2 : 
                  NALU_HYPRE_ParCSRPCGSetPrecond(*solver,NALU_HYPRE_ParCSRParaSailsSolve,
                                         NALU_HYPRE_ParCSRParaSailsSetup,precon);
                  break;
             case 3 : 
                  NALU_HYPRE_ParCSRPCGSetPrecond(*solver, NALU_HYPRE_BoomerAMGSolve,
                                         NALU_HYPRE_BoomerAMGSetup, precon);
                  break;
             case 4 : 
                  NALU_HYPRE_ParCSRPCGSetPrecond(*solver, NALU_HYPRE_ParCSRPilutSolve,
                                            NALU_HYPRE_ParCSRPilutSetup, precon); 
                  break;
             case 5 : 
                  NALU_HYPRE_ParCSRPCGSetPrecond(*solver, NALU_HYPRE_EuclidSolve,
                                            NALU_HYPRE_EuclidSetup, precon);
                  break;
             case 6 : 
                  NALU_HYPRE_ParCSRPCGSetPrecond(*solver, NALU_HYPRE_LSI_DDIlutSolve,
                                            NALU_HYPRE_LSI_DDIlutSetup, precon);
                  break;
             case 7 : 
#ifdef HAVE_ML
                  NALU_HYPRE_ParCSRPCGSetPrecond(*solver,NALU_HYPRE_LSI_MLSolve,
                                            NALU_HYPRE_LSI_MLSetup, precon);
#else
             ...
             case 8 : 
#ifdef HAVE_MLI
                  NALU_HYPRE_ParCSRPCGSetPrecond(*solver,NALU_HYPRE_LSI_MLISolve,
                                            NALU_HYPRE_LSI_MLISetup, precon);
#else
          ...
          NALU_HYPRE_ParCSRPCGSetup(*solver, Amat_csr, f_csr, x_csr);
          break;

      case 1 :
          NALU_HYPRE_ParCSRGMRESCreate(mpi_comm, solver);
          NALU_HYPRE_ParCSRGMRESSetMaxIter(*solver, param_ptr.MaxIter_);
          NALU_HYPRE_ParCSRGMRESSetTol(*solver, param_ptr.Tol_);
          NALU_HYPRE_ParCSRGMRESSetLogging(*solver, outputLevel_);
          NALU_HYPRE_ParCSRGMRESSetKDim(*solver, 50);
          switch ( param_ptr.PrecondID_ )
          {
             case 1 : 
                  ...
                  break;
             case 2 : 
                  NALU_HYPRE_ParCSRGMRESSetPrecond(*solver,NALU_HYPRE_ParCSRParaSailsSolve,
                                         NALU_HYPRE_ParCSRParaSailsSetup,precon);
                  break;
             case 3 : 
                  NALU_HYPRE_ParCSRGMRESSetPrecond(*solver, NALU_HYPRE_BoomerAMGSolve,
                                         NALU_HYPRE_BoomerAMGSetup, precon);
                  break;
             case 4 : 
                  NALU_HYPRE_ParCSRGMRESSetPrecond(*solver, NALU_HYPRE_ParCSRPilutSolve,
                                            NALU_HYPRE_ParCSRPilutSetup, precon); 
                  break;
             case 5 : 
                  NALU_HYPRE_ParCSRGMRESSetPrecond(*solver, NALU_HYPRE_EuclidSolve,
                                            NALU_HYPRE_EuclidSetup, precon);
                  break;
             case 6 : 
                  NALU_HYPRE_ParCSRGMRESSetPrecond(*solver, NALU_HYPRE_LSI_DDIlutSolve,
                                            NALU_HYPRE_LSI_DDIlutSetup, precon);
                  break;
             case 7 : 
#ifdef HAVE_ML
                  NALU_HYPRE_ParCSRGMRESSetPrecond(*solver,NALU_HYPRE_LSI_MLSolve,
                                              NALU_HYPRE_LSI_MLSetup, precon);
#else
                  printf("blockP setupSolver ERROR : ml not available.\n");
                  exit(1);
             ...
             case 8 : 
#ifdef HAVE_MLI
                  NALU_HYPRE_ParCSRGMRESSetPrecond(*solver,NALU_HYPRE_LSI_MLISolve,
                                              NALU_HYPRE_LSI_MLISetup, precon);
#else
                  printf("blockP setupSolver ERROR : mli not available.\n");
                  exit(1);
          ...
          NALU_HYPRE_ParCSRGMRESSetup(*solver, Amat_csr, f_csr, x_csr);
          break;

      case 2 :
          NALU_HYPRE_BoomerAMGCreate(solver);
          NALU_HYPRE_BoomerAMGSetMaxIter(*solver, param_ptr.MaxIter_);
          NALU_HYPRE_BoomerAMGSetCycleType(*solver, 1);
          //NALU_HYPRE_BoomerAMGSetTol(*solver, 1.0e-20);
          NALU_HYPRE_BoomerAMGSetPrintLevel(*solver, outputLevel_);
          NALU_HYPRE_BoomerAMGSetMaxLevels(*solver, 25);
          NALU_HYPRE_BoomerAMGSetMeasureType(*solver, 0);
          NALU_HYPRE_BoomerAMGSetCoarsenType(*solver, 0);
          ...
          //{
          //   for ( i = 0; i < 25; i++ ) relax_wt[i] = -10.0;
          //   NALU_HYPRE_BoomerAMGSetRelaxOmega(*solver, relax_wt);
          //}
          precon = NULL;
          NALU_HYPRE_BoomerAMGSetup(*solver, Amat_csr, f_csr, x_csr);
          break;

      case 3 :
          NALU_HYPRE_ParCSRDiagScaleSetup(*solver, Amat_csr, f_csr, x_csr);
          precon = NULL;
          break;
   }
   return 0;
...
int NALU_HYPRE_LSI_BlockP::solveBDSolve(NALU_HYPRE_IJVector x1,NALU_HYPRE_IJVector x2,
                                   NALU_HYPRE_IJVector f1,NALU_HYPRE_IJVector f2)
{
   NALU_HYPRE_ParCSRMatrix A11mat_csr, A22mat_csr;
   NALU_HYPRE_ParVector    x1_csr, x2_csr, f1_csr, f2_csr;
   ...
   NALU_HYPRE_IJMatrixGetObject( A11mat_, (void **) &A11mat_csr );
   NALU_HYPRE_IJMatrixGetObject( A22mat_, (void **) &A22mat_csr );
   NALU_HYPRE_IJVectorGetObject( F1vec_, (void **) &f1_csr );
   NALU_HYPRE_IJVectorGetObject( F2vec_, (void **) &f2_csr );
   NALU_HYPRE_IJVectorGetObject( X1vec_, (void **) &x1_csr );
   NALU_HYPRE_IJVectorGetObject( X2vec_, (void **) &x2_csr );
   ...
   if ( A22Params_.SolverID_ == 0 )
      NALU_HYPRE_ParCSRPCGSolve(A22Solver_, A22mat_csr, f2_csr, x2_csr);
   else if ( A22Params_.SolverID_ == 1 )
      NALU_HYPRE_ParCSRGMRESSolve(A22Solver_, A22mat_csr, f2_csr, x2_csr);
   else if ( A22Params_.SolverID_ == 2 )
      NALU_HYPRE_BoomerAMGSolve( A22Solver_, A22mat_csr, f2_csr, x2_csr );
   ...
   if ( A11Params_.SolverID_ == 0 )
      NALU_HYPRE_ParCSRPCGSolve(A11Solver_, A11mat_csr, f1_csr, x1_csr);
   else if ( A11Params_.SolverID_ == 1 )
      NALU_HYPRE_ParCSRGMRESSolve(A11Solver_, A11mat_csr, f1_csr, x1_csr);
   else if ( A11Params_.SolverID_ == 2 )
      NALU_HYPRE_BoomerAMGSolve( A11Solver_, A11mat_csr, f1_csr, x1_csr );
...
int NALU_HYPRE_LSI_BlockP::solveBTSolve(NALU_HYPRE_IJVector x1,NALU_HYPRE_IJVector x2,
                                   NALU_HYPRE_IJVector f1,NALU_HYPRE_IJVector f2)
{
   NALU_HYPRE_ParCSRMatrix A11mat_csr, A22mat_csr, A12mat_csr;
   NALU_HYPRE_ParVector    x1_csr, x2_csr, f1_csr, f2_csr;
   ...
   NALU_HYPRE_ParCSRMatrixMatvec(-1.0, A12mat_csr, x2_csr, 1.0, f1_csr);
   if ( A11Params_.SolverID_ == 0 )
      NALU_HYPRE_ParCSRPCGSolve(A11Solver_, A11mat_csr, f1_csr, x1_csr);
   else if ( A11Params_.SolverID_ == 1 )
      NALU_HYPRE_ParCSRGMRESSolve(A11Solver_, A11mat_csr, f1_csr, x1_csr);
...
int NALU_HYPRE_LSI_BlockP::solveBLUSolve(NALU_HYPRE_IJVector x1,NALU_HYPRE_IJVector x2,
                                    NALU_HYPRE_IJVector f1,NALU_HYPRE_IJVector f2)
{
   NALU_HYPRE_ParCSRMatrix A11mat_csr, A22mat_csr, A12mat_csr;
   NALU_HYPRE_ParVector    x1_csr, x2_csr, f1_csr, f2_csr, y1_csr;
   ...
   NALU_HYPRE_IJMatrixGetObject( A22mat_, (void **) &A22mat_csr );
   NALU_HYPRE_IJMatrixGetObject( A12mat_, (void **) &A12mat_csr );
   NALU_HYPRE_IJVectorGetObject( f1, (void **) &f1_csr );
   NALU_HYPRE_IJVectorGetObject( f2, (void **) &f2_csr );
   NALU_HYPRE_IJVectorGetObject( x1, (void **) &x1_csr );
   NALU_HYPRE_IJVectorGetObject( x2, (void **) &x2_csr );
   NALU_HYPRE_IJVectorGetObject( X1aux_, (void **) &y1_csr );
   ...
   if ( A11Params_.SolverID_ == 0 )
      NALU_HYPRE_ParCSRPCGSolve(A11Solver_, A11mat_csr, f1_csr, y1_csr);
   else if ( A11Params_.SolverID_ == 1 )
      NALU_HYPRE_ParCSRGMRESSolve(A11Solver_, A11mat_csr, f1_csr, y1_csr);
   else if ( A11Params_.SolverID_ == 2 )
      NALU_HYPRE_BoomerAMGSolve( A11Solver_, A11mat_csr, f1_csr, x1_csr );
   else if ( A11Params_.SolverID_ == 3 )
      NALU_HYPRE_ParCSRDiagScale( A11Solver_, A11mat_csr, f1_csr, x1_csr );
   ...
   NALU_HYPRE_ParCSRMatrixMatvecT(1.0, A12mat_csr, y1_csr, -1.0, f2_csr);
   if ( A22Params_.SolverID_ == 0 )
      NALU_HYPRE_ParCSRPCGSolve(A22Solver_, A22mat_csr, f2_csr, x2_csr);
   else if ( A22Params_.SolverID_ == 1 )
      NALU_HYPRE_ParCSRGMRESSolve(A22Solver_, A22mat_csr, f2_csr, x2_csr);
   ...
   NALU_HYPRE_ParCSRMatrixMatvec(-1.0, A12mat_csr, x2_csr, 0.0, f1_csr);
   if ( A11Params_.SolverID_ == 0 )
      NALU_HYPRE_ParCSRPCGSolve(A11Solver_, A11mat_csr, f1_csr, x1_csr);
   else if ( A11Params_.SolverID_ == 1 )
      NALU_HYPRE_ParCSRGMRESSolve(A11Solver_, A11mat_csr, f1_csr, x1_csr);
