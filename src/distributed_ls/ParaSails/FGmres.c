...
GeneratePlaneRotation(NALU_HYPRE_Real dx, NALU_HYPRE_Real dy, NALU_HYPRE_Real *cs, NALU_HYPRE_Real *sn)
{
  if (dy == 0.0) {
    ...
    *sn = 1.0 / nalu_hypre_sqrt( 1.0 + temp*temp );
    *cs = temp * *sn;
  } else {
    NALU_HYPRE_Real temp = dy / dx;
    *cs = 1.0 / nalu_hypre_sqrt( 1.0 + temp*temp );
    *sn = temp * *cs;
  }
}
  ...
  NALU_HYPRE_Int dim, NALU_HYPRE_Real tol, NALU_HYPRE_Int max_iter)
{
    ...
    do
    {
        ...
        beta = nalu_hypre_sqrt(InnerProd(n, V(0), V(0), comm));     /* beta = norm(V(0)) */
        ...
        do
        {
            ...
            H(i+1, i) = nalu_hypre_sqrt(InnerProd(n, V(i+1), V(i+1), comm));
            /* V(i+1) = V(i+1) / H(i+1, i) */
