...

{
   ...
   NALU_HYPRE_Int               graph_type = hypre_SStructGraphObjectType(graph);
   hypre_SStructGrid      *grid       = hypre_SStructGraphGrid(graph);
   NALU_HYPRE_IJMatrix          ij_A       = hypre_SStructMatrixIJMatrix(A);
   NALU_HYPRE_Int               matrix_type = hypre_SStructMatrixObjectType(A);
   ...
   fgrid_crse_extents      = hypre_TAlloc(hypre_BoxArray **,  nvars, NALU_HYPRE_MEMORY_HOST);
   fbox_interior           = hypre_TAlloc(hypre_BoxArray **,  nvars, NALU_HYPRE_MEMORY_HOST);
   fbox_bdy                = hypre_TAlloc(hypre_BoxArrayArray **,  nvars, NALU_HYPRE_MEMORY_HOST);
   interior_fboxi          = hypre_TAlloc(NALU_HYPRE_Int **,  nvars, NALU_HYPRE_MEMORY_HOST);
   bdy_fboxi               = hypre_TAlloc(NALU_HYPRE_Int **,  nvars, NALU_HYPRE_MEMORY_HOST);
   cboxi_fboxes            = hypre_TAlloc(NALU_HYPRE_Int **,  nvars, NALU_HYPRE_MEMORY_HOST);
   cboxi_fcnt              = hypre_TAlloc(NALU_HYPRE_Int *,  nvars, NALU_HYPRE_MEMORY_HOST);
   ...
   for (var1 = 0; var1 < nvars; var1++)
   {
      ...
      fgrid_crse_extents[var1] = hypre_TAlloc(hypre_BoxArray *,
                                              hypre_BoxArraySize(cgrid_boxes), NALU_HYPRE_MEMORY_HOST);
      fbox_interior[var1] = hypre_TAlloc(hypre_BoxArray *,
                                         hypre_BoxArraySize(cgrid_boxes), NALU_HYPRE_MEMORY_HOST);
      fbox_bdy[var1]     = hypre_TAlloc(hypre_BoxArrayArray *,
                                        hypre_BoxArraySize(cgrid_boxes), NALU_HYPRE_MEMORY_HOST);
      interior_fboxi[var1] = hypre_TAlloc(NALU_HYPRE_Int *,  hypre_BoxArraySize(cgrid_boxes),
                                          NALU_HYPRE_MEMORY_HOST);
      bdy_fboxi[var1]     = hypre_TAlloc(NALU_HYPRE_Int *,  hypre_BoxArraySize(cgrid_boxes),
                                         NALU_HYPRE_MEMORY_HOST);

      fgrid = hypre_SStructPGridSGrid(hypre_SStructPMatrixPGrid(A_pmatrix), var1);
      fgrid_boxes = hypre_StructGridBoxes(fgrid);

      cboxi_fboxes[var1] = hypre_CTAlloc(NALU_HYPRE_Int *,  hypre_BoxArraySize(fgrid_boxes),
                                         NALU_HYPRE_MEMORY_HOST);
      cboxi_fcnt[var1]  = hypre_CTAlloc(NALU_HYPRE_Int,  hypre_BoxArraySize(fgrid_boxes), NALU_HYPRE_MEMORY_HOST);
      ...
      hypre_ForBoxI(ci, cgrid_boxes)
      {
         ...
         cnt1 = 0;
         temp1 = hypre_CTAlloc(NALU_HYPRE_Int,  hypre_BoxArraySize(fgrid_boxes), NALU_HYPRE_MEMORY_HOST);
         ...
         interior_fboxi[var1][ci] = hypre_CTAlloc(NALU_HYPRE_Int,  cnt1, NALU_HYPRE_MEMORY_HOST);
         bdy_fboxi[var1][ci]      = hypre_CTAlloc(NALU_HYPRE_Int,  cnt1, NALU_HYPRE_MEMORY_HOST);
         ...
         hypre_TFree(temp1, NALU_HYPRE_MEMORY_HOST);
      ...
      hypre_ForBoxI(fi, fgrid_boxes)
      {
         ...
         temp1 = hypre_CTAlloc(NALU_HYPRE_Int,  hypre_BoxArraySize(cgrid_boxes), NALU_HYPRE_MEMORY_HOST);
         ...
         cboxi_fboxes[var1][fi] = hypre_TAlloc(NALU_HYPRE_Int,  cboxi_fcnt[var1][fi], NALU_HYPRE_MEMORY_HOST);
    ...
    *--------------------------------------------------------------------------*/
   OffsetA =  hypre_CTAlloc(NALU_HYPRE_Int *,  2, NALU_HYPRE_MEMORY_HOST);
   for (i = 0; i < 2; i++)
   {
      OffsetA[i] = hypre_CTAlloc(NALU_HYPRE_Int,  refine_factors[i + 1], NALU_HYPRE_MEMORY_HOST);
   }
      ...
      for (var2 = 0; var2 < nvars; var2++)
      {
         ...
         if (stencils != NULL)
         {
               ...
               else
               {
                  ...
                  for (i = 0; i < stencil_size; i++)
                  {
                     ...
                     if (abs_stencil_shape)
                     {
                        found = trueV;
                        stencil_marker = trueV;
                        hypre_TFree(stencil_contrib_cnt, NALU_HYPRE_MEMORY_HOST);
                        hypre_TFree(stencil_ranks, NALU_HYPRE_MEMORY_HOST);
                        for (i = 0; i < stencil_size; i++)
                        {
                           hypre_BoxDestroy(shift_box[i]);
                        }
                        hypre_TFree(shift_box, NALU_HYPRE_MEMORY_HOST);
                        hypre_TFree(volume_shift_box, NALU_HYPRE_MEMORY_HOST);
                        ...
                        for (j = 1; j < max_stencil_size; j++)
                        {
                           ...
                           if (stencil_i != -1)
                           {
                              hypre_TFree(stencil_contrib_i[stencil_i], NALU_HYPRE_MEMORY_HOST);
                              hypre_TFree(weight_contrib_i[stencil_i], NALU_HYPRE_MEMORY_HOST);
                           }
                        }
                        hypre_TFree(stencil_contrib_i, NALU_HYPRE_MEMORY_HOST);
                        hypre_TFree(weight_contrib_i, NALU_HYPRE_MEMORY_HOST);
                        hypre_TFree(rank_stencils, NALU_HYPRE_MEMORY_HOST);
                     }
            ...
            if (stencil_marker == trueV)
            {
               ...
               stencil_contrib_cnt = hypre_CTAlloc(NALU_HYPRE_Int,  stencil_size, NALU_HYPRE_MEMORY_HOST);
               stencil_contrib_i  = hypre_TAlloc(NALU_HYPRE_Int *,  stencil_size, NALU_HYPRE_MEMORY_HOST);
               weight_contrib_i   = hypre_TAlloc(NALU_HYPRE_Real *,  stencil_size, NALU_HYPRE_MEMORY_HOST);
               stencil_ranks      = hypre_TAlloc(NALU_HYPRE_Int,  stencil_size, NALU_HYPRE_MEMORY_HOST);
               rank_stencils      = hypre_TAlloc(NALU_HYPRE_Int,  max_stencil_size, NALU_HYPRE_MEMORY_HOST);
               shift_box          = hypre_TAlloc(hypre_Box *,  stencil_size, NALU_HYPRE_MEMORY_HOST);
               volume_shift_box   = hypre_TAlloc(NALU_HYPRE_Int,  stencil_size, NALU_HYPRE_MEMORY_HOST);
               ...
               if (stencil_i != -1)
               {
                  stencil_contrib_i[stencil_i] =
                     hypre_TAlloc(NALU_HYPRE_Int,  stencil_contrib_cnt[stencil_i], NALU_HYPRE_MEMORY_HOST);
                  weight_contrib_i[stencil_i] =
                     hypre_TAlloc(NALU_HYPRE_Real,  stencil_contrib_cnt[stencil_i], NALU_HYPRE_MEMORY_HOST);
                  sum = 0.0;
                  k = 0;
               ...
               if (ndim == 2)
               {
                  ...
                  for (i = 4; i <= 5; i++)
                  {
                     ...
                     {
                        stencil_contrib_cnt[stencil_i]++;
                        stencil_contrib_i[stencil_i] = hypre_TAlloc(NALU_HYPRE_Int,  1, NALU_HYPRE_MEMORY_HOST);
                        weight_contrib_i[stencil_i] =  hypre_TAlloc(NALU_HYPRE_Real,  1, NALU_HYPRE_MEMORY_HOST);
                ...
                *  averages.
                *-----------------------------------------------------------------*/
               vals = hypre_CTAlloc(NALU_HYPRE_Real,  max_contribut_size, NALU_HYPRE_MEMORY_HOST);
             ...
             *---------------------------------------------------------------------*/
            a_ptrs   = hypre_TAlloc(NALU_HYPRE_Real *,  stencil_size, NALU_HYPRE_MEMORY_HOST);
            crse_ptrs = hypre_TAlloc(NALU_HYPRE_Real *,  stencil_size, NALU_HYPRE_MEMORY_HOST);
               ...
               hypre_ForBoxArrayI(arrayi, fbox_bdy_ci)
               {
                  ...
                  hypre_ForBoxI(fi, fbox_bdy_ci_fi)
                  {
                                               ...
                                               crse_dbox, cstart, stridec, iAc);
                     {
                        ...
                        temp3 = hypre_CTAlloc(NALU_HYPRE_Real,  volume_coarse_cell_box, NALU_HYPRE_MEMORY_HOST);
                        ...
                        if (nUventries > 0)
                        {
                           temp2 = hypre_CTAlloc(NALU_HYPRE_Int,  volume_coarse_cell_box, NALU_HYPRE_MEMORY_HOST);
                           cnt1 = 0;
                           for (l = index1[2]; l <= index2[2]; l++)
                           {
                              ll = l * refine_factors[1] * refine_factors[0];
                           ...
                           ncols = hypre_TAlloc(NALU_HYPRE_Int,  cnt1, NALU_HYPRE_MEMORY_HOST);
                           for (l = 0; l < cnt1; l++)
                           {
                              ncols[l] = 1;
                           }

                           rows = hypre_TAlloc(NALU_HYPRE_BigInt,  cnt1, NALU_HYPRE_MEMORY_HOST);
                           cols = hypre_TAlloc(NALU_HYPRE_BigInt,  cnt1, NALU_HYPRE_MEMORY_HOST);
                           vals2 = hypre_CTAlloc(NALU_HYPRE_Real,  cnt1, NALU_HYPRE_MEMORY_HOST);
                           ...
                           for (l = index1[2]; l <= index2[2]; l++)
                           {
                              ...
                              for (k = index1[1]; k <= index2[1]; k++)
                              {
                                 ...
                                 for (j = index1[0]; j <= index2[0]; j++)
                                 {
                                    ...
                                    if (found)
                                    {
                                       Uventry = hypre_SStructGraphUVEntry(graph, (NALU_HYPRE_Int)(rank - startrank));
                           ...
                           hypre_TFree(vals2, NALU_HYPRE_MEMORY_HOST);
                           hypre_TFree(temp2, NALU_HYPRE_MEMORY_HOST);
                        ...
                        hypre_TFree(temp3, NALU_HYPRE_MEMORY_HOST);
            ...
            hypre_TFree(a_ptrs, NALU_HYPRE_MEMORY_HOST);
            hypre_TFree(crse_ptrs, NALU_HYPRE_MEMORY_HOST);
   ...
   if (OffsetA)
   {
      for (j = 0; j < 2; j++)
      {
         if (OffsetA[j])
         {
            hypre_TFree(OffsetA[j], NALU_HYPRE_MEMORY_HOST);
         }
      }
      hypre_TFree(OffsetA, NALU_HYPRE_MEMORY_HOST);
   }
   ...
   if (nUventries > 0)
   {
      ...
      for (var1 = 0; var1 < nvars; var1++)
      {
         ...
         box_starts = hypre_CTAlloc(NALU_HYPRE_Int,  hypre_BoxArraySize(fgrid_boxes), NALU_HYPRE_MEMORY_HOST);
         box_ends  = hypre_CTAlloc(NALU_HYPRE_Int,  hypre_BoxArraySize(fgrid_boxes), NALU_HYPRE_MEMORY_HOST);
         ...
         box_graph_indices = hypre_CTAlloc(NALU_HYPRE_Int *,  box_array_size, NALU_HYPRE_MEMORY_HOST);
         box_graph_cnts   = hypre_CTAlloc(NALU_HYPRE_Int,  box_array_size, NALU_HYPRE_MEMORY_HOST);

         data_space = hypre_StructMatrixDataSpace(crse_smatrix);
         cdata_space_ranks = hypre_CTAlloc(NALU_HYPRE_Int,  cbox_array_size, NALU_HYPRE_MEMORY_HOST);
         ...
         for (fi = 0; fi < box_array_size; fi++)
         {
            ...
            if (i >= 0)
            {
               ...
               box_graph_indices[fi] = hypre_TAlloc(NALU_HYPRE_Int,  j - i + 1, NALU_HYPRE_MEMORY_HOST);
         ...
         for (fi = 0; fi < box_array_size; fi++)
         {
             ...
             *-------------------------------------------------------------*/
            box_ranks       = hypre_TAlloc(NALU_HYPRE_Int,  box_graph_cnts[fi], NALU_HYPRE_MEMORY_HOST);
            box_connections = hypre_TAlloc(NALU_HYPRE_Int,  box_graph_cnts[fi], NALU_HYPRE_MEMORY_HOST);
            parents         = hypre_TAlloc(NALU_HYPRE_Int,  box_graph_cnts[fi], NALU_HYPRE_MEMORY_HOST);
            temp1           = hypre_CTAlloc(NALU_HYPRE_Int,  box_graph_cnts[fi] + 1, NALU_HYPRE_MEMORY_HOST);
            temp2           = hypre_CTAlloc(NALU_HYPRE_Int,  box_graph_cnts[fi], NALU_HYPRE_MEMORY_HOST);
            Uv_cindex       = hypre_TAlloc(hypre_Index,  box_graph_cnts[fi], NALU_HYPRE_MEMORY_HOST);
             ...
             *  grid node.
             *-----------------------------------------------------------------*/
            parents_cnodes      = hypre_TAlloc(NALU_HYPRE_Int,  cnt1, NALU_HYPRE_MEMORY_HOST);
            fine_interface_ranks = hypre_TAlloc(NALU_HYPRE_Int,  cnt1, NALU_HYPRE_MEMORY_HOST);
            box_ranks_cnt       = hypre_CTAlloc(NALU_HYPRE_Int,  cnt1, NALU_HYPRE_MEMORY_HOST);
            coarse_contrib_Uv   = hypre_TAlloc(NALU_HYPRE_Int *,  cnt1, NALU_HYPRE_MEMORY_HOST);
            cindex              = hypre_TAlloc(hypre_Index,  cnt1, NALU_HYPRE_MEMORY_HOST);
            ...
            for (i = 0; i < box_graph_cnts[fi]; i++)
            {
               if (box_ranks[i] != -1)
               {
                  ...
                  coarse_contrib_Uv[j]   = hypre_TAlloc(NALU_HYPRE_Int,  box_ranks_cnt[j], NALU_HYPRE_MEMORY_HOST);
            ...
            if (box_ranks)
            {
               hypre_TFree(box_ranks, NALU_HYPRE_MEMORY_HOST);
            }
            if (box_connections)
            {
               hypre_TFree(box_connections, NALU_HYPRE_MEMORY_HOST);
            }
            if (parents)
            {
               hypre_TFree(parents, NALU_HYPRE_MEMORY_HOST);
            }
            if (temp1)
            {
               hypre_TFree(temp1, NALU_HYPRE_MEMORY_HOST);
            ...
            if (Uv_cindex)
            {
               hypre_TFree(Uv_cindex, NALU_HYPRE_MEMORY_HOST);
            }
            ...
            nrows = 0;
            box_to_ranks_cnt =  hypre_CTAlloc(NALU_HYPRE_Int,  cnt1, NALU_HYPRE_MEMORY_HOST);
            for (i = 0; i < cnt1; i++)
            {
            ...
            ncols = hypre_TAlloc(NALU_HYPRE_Int,  nrows, NALU_HYPRE_MEMORY_HOST);
            for (i = 0; i < nrows; i++)
            {
               ncols[i] = 1;
            }

            rows =  hypre_TAlloc(NALU_HYPRE_BigInt,  nrows, NALU_HYPRE_MEMORY_HOST);
            cols =  hypre_TAlloc(NALU_HYPRE_BigInt,  nrows, NALU_HYPRE_MEMORY_HOST);
            vals =  hypre_CTAlloc(NALU_HYPRE_Real,  nrows, NALU_HYPRE_MEMORY_HOST);

            interface_max_stencil_ranks =  hypre_TAlloc(NALU_HYPRE_Int *,  cnt1, NALU_HYPRE_MEMORY_HOST);
            interface_max_stencil_cnt  =  hypre_TAlloc(NALU_HYPRE_Int *,  cnt1, NALU_HYPRE_MEMORY_HOST);
            interface_rank_stencils    =  hypre_TAlloc(NALU_HYPRE_Int *,  cnt1, NALU_HYPRE_MEMORY_HOST);
            interface_stencil_ranks    =  hypre_TAlloc(NALU_HYPRE_Int *,  cnt1, NALU_HYPRE_MEMORY_HOST);
            coarse_stencil_cnt         =  hypre_CTAlloc(NALU_HYPRE_Int,  cnt1, NALU_HYPRE_MEMORY_HOST);
            ...
            for (i = 0; i < cnt1; i++)
            {
                ...
                *-----------------------------------------------------------------*/
               interface_max_stencil_ranks[i] = hypre_TAlloc(NALU_HYPRE_Int,  box_to_ranks_cnt[i], NALU_HYPRE_MEMORY_HOST);
               interface_max_stencil_cnt[i]  = hypre_CTAlloc(NALU_HYPRE_Int,  max_stencil_size, NALU_HYPRE_MEMORY_HOST);
               ...
               hypre_TFree(coarse_contrib_Uv[i], NALU_HYPRE_MEMORY_HOST);
               ...
               interface_stencil_ranks[i] = hypre_TAlloc(NALU_HYPRE_Int,  l, NALU_HYPRE_MEMORY_HOST);
               interface_rank_stencils[i] = hypre_TAlloc(NALU_HYPRE_Int,  max_stencil_size, NALU_HYPRE_MEMORY_HOST);
            ...
            hypre_TFree(coarse_contrib_Uv, NALU_HYPRE_MEMORY_HOST);
            hypre_TFree(box_ranks_cnt, NALU_HYPRE_MEMORY_HOST);
            hypre_TFree(cindex, NALU_HYPRE_MEMORY_HOST);
            ...
            for (i = 0; i < cnt1; i++)
            {
               stencil_vals = hypre_CTAlloc(NALU_HYPRE_Real,  coarse_stencil_cnt[i], NALU_HYPRE_MEMORY_HOST);
               ...
               hypre_TFree(interface_max_stencil_ranks[i], NALU_HYPRE_MEMORY_HOST);
               hypre_TFree(interface_max_stencil_cnt[i], NALU_HYPRE_MEMORY_HOST);
               hypre_TFree(interface_rank_stencils[i], NALU_HYPRE_MEMORY_HOST);
               ...
               {
                  temp1 = hypre_TAlloc(NALU_HYPRE_Int,  coarse_stencil_cnt[i], NALU_HYPRE_MEMORY_HOST);
                  for (j = 0; j < coarse_stencil_cnt[i]; j++)
                  {
                     temp1[j] = j;
                  }

                  hypre_qsort1(interface_stencil_ranks[i], (NALU_HYPRE_Real *) temp1, 0,
                               coarse_stencil_cnt[i] - 1);

                  /*---------------------------------------------------------------
                   * swap the stencil_vals to agree with the rank swapping.
                   *---------------------------------------------------------------*/
                  temp3  = hypre_TAlloc(NALU_HYPRE_Real,  coarse_stencil_cnt[i], NALU_HYPRE_MEMORY_HOST);
                  for (j = 0; j < coarse_stencil_cnt[i]; j++)
                  {
                     m         = temp1[j];
                ...
                *-----------------------------------------------------------------*/
               common_rank_stencils = hypre_TAlloc(NALU_HYPRE_Int,  stencil_size, NALU_HYPRE_MEMORY_HOST);
               common_stencil_ranks = hypre_TAlloc(NALU_HYPRE_Int,  stencil_size, NALU_HYPRE_MEMORY_HOST);
               common_stencil_i    = hypre_TAlloc(NALU_HYPRE_Int,  stencil_size, NALU_HYPRE_MEMORY_HOST);
                  ...
                  switch (abs_stencil_shape)
                  {
                     ...
                     case 2:    /* corners in 2-d or edges in 3-d */
                        ...
                        else if (ndim == 3)
                        {
                           ...
                           temp1 =  hypre_TAlloc(NALU_HYPRE_Int,  2, NALU_HYPRE_MEMORY_HOST);
                     ...
                     case 1:     /* e,w in 1-d, or edges in 2-d, or faces in 3-d */
                        ...
                        {
                           l    =  common_stencil_ranks[j];
                           temp1 =  hypre_TAlloc(NALU_HYPRE_Int,  8, NALU_HYPRE_MEMORY_HOST);
               ...
               hypre_TFree(interface_stencil_ranks[i], NALU_HYPRE_MEMORY_HOST);

               hypre_TFree(stencil_vals, NALU_HYPRE_MEMORY_HOST);
               hypre_TFree(temp3, NALU_HYPRE_MEMORY_HOST);
               hypre_TFree(common_rank_stencils, NALU_HYPRE_MEMORY_HOST);
               hypre_TFree(common_stencil_ranks, NALU_HYPRE_MEMORY_HOST);
               hypre_TFree(common_stencil_ranks, NALU_HYPRE_MEMORY_HOST);
               hypre_TFree(common_stencil_i, NALU_HYPRE_MEMORY_HOST);

            }          /* for (i= 0; i< cnt1; i++) */

            hypre_TFree(box_to_ranks_cnt, NALU_HYPRE_MEMORY_HOST);
            hypre_TFree(interface_max_stencil_ranks, NALU_HYPRE_MEMORY_HOST);
            hypre_TFree(interface_max_stencil_cnt, NALU_HYPRE_MEMORY_HOST);
            hypre_TFree(interface_rank_stencils, NALU_HYPRE_MEMORY_HOST);
            hypre_TFree(interface_stencil_ranks, NALU_HYPRE_MEMORY_HOST);
            hypre_TFree(coarse_stencil_cnt, NALU_HYPRE_MEMORY_HOST);
            hypre_TFree(fine_interface_ranks, NALU_HYPRE_MEMORY_HOST);
            hypre_TFree(parents_cnodes, NALU_HYPRE_MEMORY_HOST);
            hypre_TFree(vals, NALU_HYPRE_MEMORY_HOST);
         ...
         hypre_TFree(stencil_ranks, NALU_HYPRE_MEMORY_HOST);
         hypre_TFree(rank_stencils, NALU_HYPRE_MEMORY_HOST);
         hypre_TFree(cdata_space_ranks, NALU_HYPRE_MEMORY_HOST);
         hypre_TFree(box_graph_cnts, NALU_HYPRE_MEMORY_HOST);
         for (i = 0; i < box_array_size; i++)
         {
            if (box_graph_indices[i])
            {
               hypre_TFree(box_graph_indices[i], NALU_HYPRE_MEMORY_HOST);
            }
         }
         hypre_TFree(box_graph_indices, NALU_HYPRE_MEMORY_HOST);

         hypre_TFree(box_starts, NALU_HYPRE_MEMORY_HOST);
         hypre_TFree(box_ends, NALU_HYPRE_MEMORY_HOST);
      }  /* for (var1= 0; var1< nvars; var1++) */
   }    /* if (nUventries > 0) */
...
#undef DEVICE_VAR
   ...
   for (var1 = 0; var1 < nvars; var1++)
   {
      ...
      hypre_ForBoxI(ci, cgrid_boxes)
      {
         ...
         hypre_TFree(interior_fboxi[var1][ci], NALU_HYPRE_MEMORY_HOST);
         hypre_TFree(bdy_fboxi[var1][ci], NALU_HYPRE_MEMORY_HOST);
      }
      hypre_TFree(fgrid_crse_extents[var1], NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(fbox_interior[var1], NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(fbox_bdy[var1], NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(interior_fboxi[var1], NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(bdy_fboxi[var1], NALU_HYPRE_MEMORY_HOST);

      hypre_ForBoxI(fi, fgrid_boxes)
      {
         hypre_TFree(cboxi_fboxes[var1][fi], NALU_HYPRE_MEMORY_HOST);
      }
      hypre_TFree(cboxi_fboxes[var1], NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(cboxi_fcnt[var1], NALU_HYPRE_MEMORY_HOST);
   }
   hypre_TFree(fgrid_crse_extents, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(fbox_interior, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(fbox_bdy, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(interior_fboxi, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(bdy_fboxi, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(cboxi_fboxes, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(cboxi_fcnt, NALU_HYPRE_MEMORY_HOST);
