...
/*
    ...
    note: all parameters are inputs; the only output 
          is the "NALU_HYPRE_Int" returned by Hash_i_dhLookup.
*/
extern void Hash_i_dhCreate(Hash_i_dh *h, NALU_HYPRE_Int size);
...
extern void Hash_i_dhInsert(Hash_i_dh h, NALU_HYPRE_Int key, NALU_HYPRE_Int data);
  /* throws error if <data, data> is already inserted;
     grows hash table if out of space.
   */

extern NALU_HYPRE_Int  Hash_i_dhLookup(Hash_i_dh h, NALU_HYPRE_Int key);
