                                ...
                                NALU_HYPRE_Complex          *g_HashVals )
{
        ...
        i += blockDim.y)
   {
      ...
      mult = __shfl_sync(NALU_HYPRE_WARP_FULL_MASK, mult, 0, blockDim.x);
                                        ...
                                        NALU_HYPRE_Complex          *ac_start )
{
   ...
   if (__any_sync(NALU_HYPRE_WARP_FULL_MASK, s_HashKeys != NULL))
   {
      for (NALU_HYPRE_Int k = lane_id; k < SHMEM_HASH_SIZE; k += STEP_SIZE)
      {
         NALU_HYPRE_Int sum;
         NALU_HYPRE_Int key = s_HashKeys ? s_HashKeys[k] : -1;
         NALU_HYPRE_Int pos = group_prefix_sum<NALU_HYPRE_Int, STEP_SIZE>(lane_id, (NALU_HYPRE_Int) (key != -1), sum);
   ...
   if (HAS_GHASH)
   {
      ...
      for (NALU_HYPRE_Int k = lane_id; __any_sync(NALU_HYPRE_WARP_FULL_MASK, k < ghash_size); k += STEP_SIZE)
      {
         NALU_HYPRE_Int sum;
...
#pragma unroll UNROLL_FACTOR
      ...
      group_read<GROUP_SIZE>(item, ic + ii, GROUP_SIZE >= NALU_HYPRE_WARP_SIZE || i < M, istart_c, iend_c);
#else
      group_read<GROUP_SIZE>(item, ic + ii, GROUP_SIZE >= NALU_HYPRE_WARP_SIZE || i < M, istart_c);
#endif
...
{
   /* number of groups in the grid */
   const NALU_HYPRE_Int grid_num_groups = get_num_groups() * gridDim.x;
   /* group id inside the block */
   const NALU_HYPRE_Int group_id = get_group_id();
   /* group id in the grid */
   const NALU_HYPRE_Int grid_group_id = blockIdx.x * get_num_groups() + group_id;
   /* lane id inside the group */
