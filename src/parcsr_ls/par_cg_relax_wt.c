                          ...
                          NALU_HYPRE_Real *rlx_wt_ptr)
{
   ...
   CF_marker         = (nalu_hypre_ParAMGDataCFMarkerArray(amg_data)[level] != NULL) ?
                       nalu_hypre_IntArrayData(nalu_hypre_ParAMGDataCFMarkerArray(amg_data)[level]) : NULL;
   ...
   for (jj = 0; jj < num_cg_sweeps; jj++)
   {
      ...
      alpha = gamma / (nalu_hypre_ParVectorInnerProd(Ptemp, Vtemp) + NALU_HYPRE_REAL_MIN);
      alphinv = 1.0 / (alpha + NALU_HYPRE_REAL_MIN);
      tridiag[jj + 1] = alphinv;
      tridiag[jj] *= beta;
      tridiag[jj] += alphinv;
      trioffd[jj] *= nalu_hypre_sqrt(beta);
      trioffd[jj + 1] = -alphinv;
      row_sum = nalu_hypre_abs(tridiag[jj]) + nalu_hypre_abs(trioffd[jj]);
      if (row_sum > max_row_sum) { max_row_sum = row_sum; }
      if (jj > 0)
      {
         row_sum = nalu_hypre_abs(tridiag[jj - 1]) + nalu_hypre_abs(trioffd[jj - 1])
                   + nalu_hypre_abs(trioffd[jj]);
         if (row_sum > max_row_sum) { max_row_sum = row_sum; }
         ...
         if (nalu_hypre_abs(rlx_wt - rlx_wt_old) < 1.e-3 )
         {
   ...
   rlx_wt = nalu_hypre_abs(tridiag[0])+nalu_hypre_abs(trioffd[1]);

   for (i=1; i < num_cg_sweeps-1; i++)
   {
      row_sum = nalu_hypre_abs(tridiag[i]) + nalu_hypre_abs(trioffd[i]) + nalu_hypre_abs(trioffd[i+1]);
      if (row_sum > rlx_wt) rlx_wt = row_sum;
   }
   row_sum = nalu_hypre_abs(tridiag[num_cg_sweeps-1]) + nalu_hypre_abs(trioffd[num_cg_sweeps-1]);
   if (row_sum > rlx_wt) rlx_wt = row_sum;
                ...
                NALU_HYPRE_Real tol, NALU_HYPRE_Int k, NALU_HYPRE_Real *ev_ptr)
{
   ...
   while (nalu_hypre_abs(y - z) > tol * (nalu_hypre_abs(y) + nalu_hypre_abs(z)))
   {
      x = (y + z) / 2;
