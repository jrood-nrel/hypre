...
/*--------------------------------------------------------------------------
 * NALU_HYPRE_FSAISetLocalSolveType
 *--------------------------------------------------------------------------*/

NALU_HYPRE_Int
NALU_HYPRE_FSAISetLocalSolveType( NALU_HYPRE_Solver solver,
                             NALU_HYPRE_Int    local_solve_type  )
{
   return ( nalu_hypre_FSAISetLocalSolveType( (void *) solver, local_solve_type ) );
}

/*--------------------------------------------------------------------------
 * NALU_HYPRE_FSAIGetLocalSolveType
 *--------------------------------------------------------------------------*/

NALU_HYPRE_Int
NALU_HYPRE_FSAIGetLocalSolveType( NALU_HYPRE_Solver  solver,
                             NALU_HYPRE_Int    *local_solve_type  )
{
   return ( nalu_hypre_FSAIGetLocalSolveType( (void *) solver, local_solve_type ) );
}
...
/*--------------------------------------------------------------------------
 * NALU_HYPRE_FSAISetMaxNnzRow
 *--------------------------------------------------------------------------*/

NALU_HYPRE_Int
NALU_HYPRE_FSAISetMaxNnzRow( NALU_HYPRE_Solver solver,
                        NALU_HYPRE_Int    max_nnz_row  )
{
   return ( nalu_hypre_FSAISetMaxNnzRow( (void *) solver, max_nnz_row ) );
}

/*--------------------------------------------------------------------------
 * NALU_HYPRE_FSAIGetMaxNnzRow
 *--------------------------------------------------------------------------*/

NALU_HYPRE_Int
NALU_HYPRE_FSAIGetMaxNnzRow( NALU_HYPRE_Solver  solver,
                        NALU_HYPRE_Int    *max_nnz_row  )
{
   return ( nalu_hypre_FSAIGetMaxNnzRow( (void *) solver, max_nnz_row ) );
}
...
/*--------------------------------------------------------------------------
 * NALU_HYPRE_FSAISetNumLevels
 *--------------------------------------------------------------------------*/

NALU_HYPRE_Int
NALU_HYPRE_FSAISetNumLevels( NALU_HYPRE_Solver solver,
                        NALU_HYPRE_Int    num_levels )
{
   return ( nalu_hypre_FSAISetNumLevels( (void *) solver, num_levels ) );
}

/*--------------------------------------------------------------------------
 * NALU_HYPRE_FSAIGetNumLevels
 *--------------------------------------------------------------------------*/

NALU_HYPRE_Int
NALU_HYPRE_FSAIGetNumLevels( NALU_HYPRE_Solver  solver,
                        NALU_HYPRE_Int    *num_levels )
{
   return ( nalu_hypre_FSAIGetNumLevels( (void *) solver, num_levels ) );
}

/*--------------------------------------------------------------------------
 * NALU_HYPRE_FSAISetThreshold
 *--------------------------------------------------------------------------*/

NALU_HYPRE_Int
NALU_HYPRE_FSAISetThreshold( NALU_HYPRE_Solver solver,
                        NALU_HYPRE_Real   threshold )
{
   return ( nalu_hypre_FSAISetThreshold( (void *) solver, threshold ) );
}

/*--------------------------------------------------------------------------
 * NALU_HYPRE_FSAIGetThreshold
 *--------------------------------------------------------------------------*/

NALU_HYPRE_Int
NALU_HYPRE_FSAIGetThreshold( NALU_HYPRE_Solver  solver,
                        NALU_HYPRE_Real   *threshold )
{
   return ( nalu_hypre_FSAIGetThreshold( (void *) solver, threshold ) );
}
