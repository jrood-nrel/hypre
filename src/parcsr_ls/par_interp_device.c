                                     ...
                                     nalu_hypre_ParCSRMatrix  **P_ptr)
{
   ...
   */
   ...
   if (P_offd_size)
   {
      ...
      NALU_HYPRE_Int *P_marker = nalu_hypre_TAlloc(NALU_HYPRE_Int, num_cols_A_offd, NALU_HYPRE_MEMORY_DEVICE);
      NALU_HYPRE_Int *P_colids = nalu_hypre_TAlloc(NALU_HYPRE_Int, nalu_hypre_max(P_offd_size, num_cols_A_offd),
                                         NALU_HYPRE_MEMORY_DEVICE);

      nalu_hypre_TMemcpy(P_colids, P_offd_j, NALU_HYPRE_Int, P_offd_size, NALU_HYPRE_MEMORY_DEVICE, memory_location);
      /* sort and unique */
#if defined(NALU_HYPRE_USING_SYCL)
...
#endif
      nalu_hypre_assert(new_end - P_colids == num_cols_P_offd);

      tmp_map_offd_h = nalu_hypre_TAlloc(NALU_HYPRE_Int, num_cols_P_offd, NALU_HYPRE_MEMORY_HOST);
      nalu_hypre_TMemcpy(tmp_map_offd_h, P_colids, NALU_HYPRE_Int, num_cols_P_offd, NALU_HYPRE_MEMORY_HOST,
                    NALU_HYPRE_MEMORY_DEVICE);

      nalu_hypre_TFree(P_colids, NALU_HYPRE_MEMORY_DEVICE);
      nalu_hypre_TFree(P_marker, NALU_HYPRE_MEMORY_DEVICE);
   }
   ...
   nalu_hypre_ParCSRMatrixColMapOffd(P) = nalu_hypre_CTAlloc(NALU_HYPRE_BigInt, num_cols_P_offd, NALU_HYPRE_MEMORY_HOST);
   nalu_hypre_CSRMatrixNumCols(P_offd)  = num_cols_P_offd;

   nalu_hypre_GetCommPkgRTFromCommPkgA(P, A, fine_to_coarse_h, tmp_map_offd_h);
   ...
   *P_ptr = P;
   ...
   nalu_hypre_TFree(tmp_map_offd_h,   NALU_HYPRE_MEMORY_HOST);
