   ...
   nalu_hypre_StructAssumedPart **p_assumed_partition )
{
   ...
   for (i = 0; i < size; i++)
   {
      ...
      rn_cubes = nalu_hypre_doubleBoxVolume(box) / nalu_hypre_pow(wmin, ndim);
      rn_cube_procs = proc_count / rn_cubes;
      rn_cube_divs = nalu_hypre_pow(rn_cube_procs, (1.0 / (NALU_HYPRE_Real)ndim));
      ...
      for (d = 0; d < ndim; d++)
      {
         ...
         nalu_hypre_IndexD(div_index, d) = (NALU_HYPRE_Int) nalu_hypre_floor(rdiv + 1.0e-6);
   ...
   size = (NALU_HYPRE_Int)(1.2 * nalu_hypre_BoxArraySize(local_boxes));
   ...
   nalu_hypre_ForBoxI(i, local_boxes)
   {
      ...
      {
         size = (NALU_HYPRE_Int)(size + proc_count + 1.2 * (nalu_hypre_BoxArraySize(local_boxes) - i));
