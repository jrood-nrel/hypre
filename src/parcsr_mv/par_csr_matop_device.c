...
{
   const NALU_HYPRE_Int row = hypre_gpu_get_grid_warp_id<1, 1>(item);
...
hypre_ConcatDiagAndOffdDevice(hypre_ParCSRMatrix *A)
{
   ...
   hypre_CSRMatrixInitialize_v2(B, 0, NALU_HYPRE_MEMORY_DEVICE);
   ...
   NALU_HYPRE_Int  diag_ncol = hypre_CSRMatrixNumCols(A_diag);
   NALU_HYPRE_Int *d_diag_i = hypre_CSRMatrixI(A_diag);
   NALU_HYPRE_Int *d_diag_j = hypre_CSRMatrixJ(A_diag);
   NALU_HYPRE_Complex *d_diag_a = hypre_CSRMatrixData(A_diag);
   NALU_HYPRE_Int *d_offd_i = hypre_CSRMatrixI(A_offd);
   NALU_HYPRE_Int *d_offd_j = hypre_CSRMatrixJ(A_offd);
   NALU_HYPRE_Complex *d_offd_a = hypre_CSRMatrixData(A_offd);
   NALU_HYPRE_Int *cols_offd_map = NULL;
   NALU_HYPRE_Int *d_ib = hypre_CSRMatrixI(B);
   NALU_HYPRE_Int *d_jb = hypre_CSRMatrixJ(B);
   NALU_HYPRE_Complex *d_ab = hypre_CSRMatrixData(B);
...
hypre_ConcatDiagAndOffdDevice(hypre_ParCSRMatrix *A)
{
   ...
   hypre_CSRMatrix *B;
   NALU_HYPRE_Int        B_nrows = hypre_CSRMatrixNumRows(A_diag);
   NALU_HYPRE_Int        B_ncols = hypre_CSRMatrixNumCols(A_diag) + hypre_CSRMatrixNumCols(A_offd);
   NALU_HYPRE_Int        B_nnz   = A_diag_nnz + A_offd_nnz;
   NALU_HYPRE_Int       *B_ii = hypre_TAlloc(NALU_HYPRE_Int,     B_nnz, NALU_HYPRE_MEMORY_DEVICE);
   NALU_HYPRE_Int       *B_j  = hypre_TAlloc(NALU_HYPRE_Int,     B_nnz, NALU_HYPRE_MEMORY_DEVICE);
   NALU_HYPRE_Complex   *B_a  = hypre_TAlloc(NALU_HYPRE_Complex, B_nnz, NALU_HYPRE_MEMORY_DEVICE);
                                 ...
                                 NALU_HYPRE_BigInt      **cols_map_offd_ptr)
{
   ...
   hypre_TFree(cols_offd_map, NALU_HYPRE_MEMORY_DEVICE);

   hypre_TMemcpy(hypre_CSRMatrixI(B) + hypre_ParCSRMatrixNumRows(A) + 1, hypre_CSRMatrixI(E) + 1,
                 NALU_HYPRE_Int, hypre_CSRMatrixNumRows(E),
...
{
   hypre_CSRMatrix *A_diag          = hypre_ParCSRMatrixDiag(A);
   NALU_HYPRE_Int        A_nrows         = hypre_CSRMatrixNumRows(A_diag);
   NALU_HYPRE_Int        A_ncols         = hypre_CSRMatrixNumCols(A_diag);
   NALU_HYPRE_Int       *A_diag_i        = hypre_CSRMatrixI(A_diag);
   NALU_HYPRE_Int       *A_diag_j        = hypre_CSRMatrixJ(A_diag);
   NALU_HYPRE_Complex   *A_diag_a        = hypre_CSRMatrixData(A_diag);
   NALU_HYPRE_Int        A_diag_nnz      = hypre_CSRMatrixNumNonzeros(A_diag);
   ...
   NALU_HYPRE_Complex   *A_offd_a        = hypre_CSRMatrixData(A_offd);
   NALU_HYPRE_Int        A_offd_nnz      = hypre_CSRMatrixNumNonzeros(A_offd);
   NALU_HYPRE_BigInt     first_col_A     = hypre_ParCSRMatrixFirstColDiag(A);
   NALU_HYPRE_BigInt     last_col_A      = hypre_ParCSRMatrixLastColDiag(A);
   ...
   NALU_HYPRE_Int       *E_i     = hypre_CSRMatrixI(E);
   NALU_HYPRE_BigInt    *E_bigj  = hypre_CSRMatrixBigJ(E);
   NALU_HYPRE_Complex   *E_a     = hypre_CSRMatrixData(E);
   NALU_HYPRE_Int        E_nrows = hypre_CSRMatrixNumRows(E);
   NALU_HYPRE_Int        E_nnz   = hypre_CSRMatrixNumNonzeros(E);
   ...
   hypre_CSRMatrixSplitDevice_core(1,
                                   ...
                                   NULL);
   hypre_TFree(E_ii, NALU_HYPRE_MEMORY_DEVICE);
                                      ...
                                      void                **request_ptr)
{
   ...
   NALU_HYPRE_Int     *B_ext_i_d      = hypre_CSRMatrixI(B_ext);
   NALU_HYPRE_BigInt  *B_ext_j_d      = hypre_CSRMatrixBigJ(B_ext);
   NALU_HYPRE_Complex *B_ext_a_d      = hypre_CSRMatrixData(B_ext);
   NALU_HYPRE_Int      B_ext_ncols    = hypre_CSRMatrixNumCols(B_ext);
   NALU_HYPRE_Int      B_ext_nrows    = hypre_CSRMatrixNumRows(B_ext);
   NALU_HYPRE_Int      B_ext_nnz      = hypre_CSRMatrixNumNonzeros(B_ext);
   NALU_HYPRE_Int     *B_ext_rownnz_d = hypre_TAlloc(NALU_HYPRE_Int, B_ext_nrows + 1, NALU_HYPRE_MEMORY_DEVICE);
   NALU_HYPRE_Int     *B_ext_rownnz_h = hypre_TAlloc(NALU_HYPRE_Int, B_ext_nrows,     NALU_HYPRE_MEMORY_HOST);
   NALU_HYPRE_Int     *B_ext_i_h      = hypre_TAlloc(NALU_HYPRE_Int, B_ext_nrows + 1, NALU_HYPRE_MEMORY_HOST);
   ...
   NALU_HYPRE_Int       *B_int_i_h   = hypre_TAlloc(NALU_HYPRE_Int, B_int_nrows + 1, NALU_HYPRE_MEMORY_HOST);
   NALU_HYPRE_Int       *B_int_i_d   = hypre_TAlloc(NALU_HYPRE_Int, B_int_nrows + 1, NALU_HYPRE_MEMORY_DEVICE);
...
#endif
   hypre_TMemcpy(B_ext_rownnz_h, B_ext_rownnz_d + 1, NALU_HYPRE_Int, B_ext_nrows,
   ...
   B_ext_i_h[0] = 0;
   hypre_TMemcpy(B_ext_i_h + 1, B_ext_rownnz_h, NALU_HYPRE_Int, B_ext_nrows, NALU_HYPRE_MEMORY_HOST,
   ...
   B_int_j_d = hypre_TAlloc(NALU_HYPRE_BigInt, B_int_nnz, NALU_HYPRE_MEMORY_DEVICE);
   if (want_data)
   {
      B_int_a_d = hypre_TAlloc(NALU_HYPRE_Complex, B_int_nnz, NALU_HYPRE_MEMORY_DEVICE);
   }
   ...
   hypre_TMemcpy(B_int_i_d, B_int_i_h, NALU_HYPRE_Int, B_int_nrows + 1, NALU_HYPRE_MEMORY_DEVICE,
                 NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_CSRMatrixMemoryLocation(B_int_d) = NALU_HYPRE_MEMORY_DEVICE;

   /* output */
   vrequest = hypre_TAlloc(void *, 3, NALU_HYPRE_MEMORY_HOST);
   ...
   *request_ptr = (void *) vrequest;
   ...
   /* free */
   hypre_TFree(B_ext_rownnz_d, NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(B_ext_rownnz_h, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(B_ext_i_h,      NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(B_int_i_h,      NALU_HYPRE_MEMORY_HOST);
                                         ...
                                         void               **request_ptr)
{
   ...
   hypre_assert( hypre_GetActualMemLocation(
            hypre_CSRMatrixMemoryLocation(hypre_ParCSRMatrixDiag(B))) == NALU_HYPRE_MEMORY_DEVICE );
   */
                                       ...
                                       void                **request_ptr)
{
   ...
   hypre_ParCSRCommPkg     *comm_pkg_j = NULL;
   hypre_ParCSRCommHandle  *comm_handle, *comm_handle_j, *comm_handle_a;
   /* NALU_HYPRE_Int global_num_rows = hypre_ParCSRMatrixGlobalNumRows(A); */
   /* diag part of A */
   hypre_CSRMatrix *A_diag   = hypre_ParCSRMatrixDiag(A);
   NALU_HYPRE_Complex   *A_diag_a = hypre_CSRMatrixData(A_diag);
   NALU_HYPRE_Int       *A_diag_i = hypre_CSRMatrixI(A_diag);
   NALU_HYPRE_Int       *A_diag_j = hypre_CSRMatrixJ(A_diag);
   /* NALU_HYPRE_Int local_num_rows  = hypre_CSRMatrixNumRows(A_diag); */
   /* off-diag part of A */
   hypre_CSRMatrix *A_offd   = hypre_ParCSRMatrixOffd(A);
   NALU_HYPRE_Complex   *A_offd_a = hypre_CSRMatrixData(A_offd);
   NALU_HYPRE_Int       *A_offd_i = hypre_CSRMatrixI(A_offd);
   ...
   /* NALU_HYPRE_Int       *row_starts      = hypre_ParCSRMatrixRowStarts(A); */
   /* NALU_HYPRE_Int        first_row       = hypre_ParCSRMatrixFirstRowIndex(A); */
   ...
   NALU_HYPRE_BigInt    *d_col_map_offd_A = hypre_ParCSRMatrixDeviceColMapOffd(A);
   ...
   d_send_i   = hypre_TAlloc(NALU_HYPRE_Int, num_rows_send + 1, NALU_HYPRE_MEMORY_DEVICE);
   d_send_map = hypre_TAlloc(NALU_HYPRE_Int, num_rows_send,     NALU_HYPRE_MEMORY_DEVICE);
   send_i     = hypre_TAlloc(NALU_HYPRE_Int, num_rows_send,     NALU_HYPRE_MEMORY_HOST);
   recv_i     = hypre_TAlloc(NALU_HYPRE_Int, num_rows_recv + 1, NALU_HYPRE_MEMORY_HOST);
   d_recv_i   = hypre_TAlloc(NALU_HYPRE_Int, num_rows_recv + 1, NALU_HYPRE_MEMORY_DEVICE);

   /* fill the send array with row lengths */
   hypre_TMemcpy(d_send_map, hypre_ParCSRCommPkgSendMapElmts(comm_pkg), NALU_HYPRE_Int,
                 num_rows_send, NALU_HYPRE_MEMORY_DEVICE, NALU_HYPRE_MEMORY_HOST);

   hypre_Memset(d_send_i, 0, sizeof(NALU_HYPRE_Int), NALU_HYPRE_MEMORY_DEVICE);
   ...
   hypre_TMemcpy(send_i, d_send_i + 1, NALU_HYPRE_Int, num_rows_send, NALU_HYPRE_MEMORY_HOST,
   ...
   hypre_TMemcpy(&num_nnz_send, d_send_i + num_rows_send, NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST,
                 NALU_HYPRE_MEMORY_DEVICE);

   /* prepare data to send out. overlap with the above commmunication */
   d_send_j = hypre_TAlloc(NALU_HYPRE_BigInt, num_nnz_send, NALU_HYPRE_MEMORY_DEVICE);
   if (want_data)
   {
      d_send_a = hypre_TAlloc(NALU_HYPRE_Complex, num_nnz_send, NALU_HYPRE_MEMORY_DEVICE);
   }

   if (d_col_map_offd_A == NULL)
   {
      d_col_map_offd_A = hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_A_offd, NALU_HYPRE_MEMORY_DEVICE);
      hypre_TMemcpy(d_col_map_offd_A, col_map_offd_A, NALU_HYPRE_BigInt, num_cols_A_offd,
   ...
   d_recv_j = hypre_TAlloc(NALU_HYPRE_BigInt, num_nnz_recv, NALU_HYPRE_MEMORY_DEVICE);
   if (want_data)
   {
      d_recv_a = hypre_TAlloc(NALU_HYPRE_Complex, num_nnz_recv, NALU_HYPRE_MEMORY_DEVICE);
   }

   recv_jstarts = hypre_TAlloc(NALU_HYPRE_Int, num_recvs + 1, NALU_HYPRE_MEMORY_HOST);
   recv_jstarts[0] = 0;
   for (i = 1; i <= num_recvs; i++)
   {
   ...
   hypre_TMemcpy(d_recv_i, recv_i, NALU_HYPRE_Int, num_rows_recv + 1, NALU_HYPRE_MEMORY_DEVICE,
                 NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_CSRMatrixMemoryLocation(A_ext) = NALU_HYPRE_MEMORY_DEVICE;
   ...
   *request_ptr = (void *) vrequest;
   ...
   hypre_TFree(recv_i,     NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(d_send_i,   NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(d_send_map, NALU_HYPRE_MEMORY_DEVICE);
...
hypre_ParcsrGetExternalRowsDeviceWait(void *vrequest)
{
   ...
   hypre_TFree(send_j, NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(send_a, NALU_HYPRE_MEMORY_DEVICE);
                                  ...
                                  NALU_HYPRE_Int             num_cols )
{
   ...
   NALU_HYPRE_Int       *send_map       = hypre_ParCSRCommPkgDeviceSendMapElmts(comm_pkg);
   ...
   hypre_CSRMatrixMemoryLocation(matrix_E) = NALU_HYPRE_MEMORY_DEVICE;

   /* Build default (original) send_map_elements array */
   if (num_components > 1)
   {
      send_map_def = hypre_TAlloc(NALU_HYPRE_Int, num_elements, NALU_HYPRE_MEMORY_DEVICE);
   ...
   /* Allocate arrays */
   e_ii = hypre_TAlloc(NALU_HYPRE_Int, num_elements, NALU_HYPRE_MEMORY_DEVICE);
   e_j  = hypre_TAlloc(NALU_HYPRE_Int, num_elements, NALU_HYPRE_MEMORY_DEVICE);

   /* Build e_ii and e_j */
   hypre_TMemcpy(e_ii, send_map_def, NALU_HYPRE_Int, num_elements,
                 NALU_HYPRE_MEMORY_DEVICE, NALU_HYPRE_MEMORY_DEVICE);
#if defined(NALU_HYPRE_USING_SYCL)
...
#endif
   nid = new_end - e_ii;
   e_ii = hypre_TReAlloc_v2(e_ii, NALU_HYPRE_Int, num_elements,
   ...
   if (num_components > 1)
   {
      hypre_TFree(send_map_def, NALU_HYPRE_MEMORY_DEVICE);
   }
...
hypre_MergeDiagAndOffdDevice(hypre_ParCSRMatrix *A)
{
   ...
   NALU_HYPRE_Int        local_num_rows   = hypre_CSRMatrixNumRows(A_diag);
   NALU_HYPRE_BigInt     glbal_num_cols   = hypre_ParCSRMatrixGlobalNumCols(A);
   NALU_HYPRE_BigInt     first_col        = hypre_ParCSRMatrixFirstColDiag(A);
   NALU_HYPRE_Int        num_cols_A_offd  = hypre_CSRMatrixNumCols(A_offd);
   NALU_HYPRE_BigInt    *col_map_offd_A   = hypre_ParCSRMatrixColMapOffd(A);
   NALU_HYPRE_BigInt    *d_col_map_offd_A = hypre_ParCSRMatrixDeviceColMapOffd(A);
   ...
   NALU_HYPRE_Int       *B_i = hypre_TAlloc(NALU_HYPRE_Int, B_nrows + 1, NALU_HYPRE_MEMORY_DEVICE);
   ...
   hypre_Memset(B_i, 0, sizeof(NALU_HYPRE_Int), NALU_HYPRE_MEMORY_DEVICE);
   ...
   /* total number of nnz */
   hypre_TMemcpy(&B_nnz, B_i + B_nrows, NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);

   B_j = hypre_TAlloc(NALU_HYPRE_BigInt,  B_nnz, NALU_HYPRE_MEMORY_DEVICE);
   B_a = hypre_TAlloc(NALU_HYPRE_Complex, B_nnz, NALU_HYPRE_MEMORY_DEVICE);
                                ...
                                NALU_HYPRE_Complex      **values )
{
   ...
   if ( !hypre_ParCSRMatrixRowvalues(mat) && (col_ind || values) )
   {
      ...
      NALU_HYPRE_Int *row_nnz = hypre_TAlloc(NALU_HYPRE_Int, nrows, NALU_HYPRE_MEMORY_DEVICE);

      hypreDevice_GetRowNnz(nrows, NULL, hypre_CSRMatrixI(Aa), hypre_CSRMatrixI(Ba), row_nnz);

      hypre_TMemcpy(size, row_nnz + local_row, NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
      ...
      */
      ...
      hypre_TFree(row_nnz, NALU_HYPRE_MEMORY_DEVICE);

      hypre_ParCSRMatrixRowvalues(mat)  =
         (NALU_HYPRE_Complex *) hypre_TAlloc(NALU_HYPRE_Complex, max_row_nnz, hypre_ParCSRMatrixMemoryLocation(mat));
      hypre_ParCSRMatrixRowindices(mat) =
         (NALU_HYPRE_BigInt *)  hypre_TAlloc(NALU_HYPRE_BigInt,  max_row_nnz, hypre_ParCSRMatrixMemoryLocation(mat));
   }
   else
   {
      NALU_HYPRE_Int *size_d = hypre_TAlloc(NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_DEVICE);
      hypreDevice_GetRowNnz(1, NULL, hypre_CSRMatrixI(Aa) + local_row, hypre_CSRMatrixI(Ba) + local_row,
                            size_d);
      hypre_TMemcpy(size, size_d, NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
      hypre_TFree(size_d, NALU_HYPRE_MEMORY_DEVICE);
   }
   ...
   if (col_ind || values)
   {
      if (hypre_ParCSRMatrixDeviceColMapOffd(mat) == NULL)
      {
         hypre_ParCSRMatrixDeviceColMapOffd(mat) =
            hypre_TAlloc(NALU_HYPRE_BigInt, hypre_CSRMatrixNumCols(Ba), NALU_HYPRE_MEMORY_DEVICE);
                                          ...
                                          NALU_HYPRE_Int           type)
{
   ...
   hypre_CSRMatrix *A_offd   = hypre_ParCSRMatrixOffd(A);
   NALU_HYPRE_Int        num_cols_A_offd  = hypre_CSRMatrixNumCols(A_offd);
   NALU_HYPRE_BigInt    *h_col_map_offd_A = hypre_ParCSRMatrixColMapOffd(A);
   NALU_HYPRE_BigInt    *col_map_offd_A = hypre_ParCSRMatrixDeviceColMapOffd(A);
   ...
   if (col_map_offd_A == NULL)
   {
      ...
      hypre_TMemcpy(col_map_offd_A, h_col_map_offd_A, NALU_HYPRE_BigInt, num_cols_A_offd,
                    NALU_HYPRE_MEMORY_DEVICE, NALU_HYPRE_MEMORY_HOST);
   ...
   if (type != 0)
   {
      elmt_tols_diag = hypre_TAlloc(NALU_HYPRE_Real, hypre_CSRMatrixNumNonzeros(A_diag), NALU_HYPRE_MEMORY_DEVICE);
      elmt_tols_offd = hypre_TAlloc(NALU_HYPRE_Real, hypre_CSRMatrixNumNonzeros(A_offd), NALU_HYPRE_MEMORY_DEVICE);
   }
   ...
   if (type == -1)
   {
      NALU_HYPRE_GPU_LAUNCH( hypre_ParCSRMatrixDropSmallEntriesDevice_getElmtTols < -1 >, gDim, bDim,
   ...
   if (type == 2)
   {
      NALU_HYPRE_GPU_LAUNCH( hypre_ParCSRMatrixDropSmallEntriesDevice_getElmtTols<2>, gDim, bDim,
                        hypre_CSRMatrixNumRows(A_diag), tol, hypre_CSRMatrixI(A_diag),
                        hypre_CSRMatrixJ(A_diag), hypre_CSRMatrixData(A_diag), hypre_CSRMatrixI(A_offd),
                        hypre_CSRMatrixData(A_offd), elmt_tols_diag, elmt_tols_offd);
   }
   ...
   NALU_HYPRE_Int *tmp_j, *tmp_end, num_cols_A_offd_new;
   tmp_j = hypre_TAlloc(NALU_HYPRE_Int, hypre_CSRMatrixNumNonzeros(A_offd), NALU_HYPRE_MEMORY_DEVICE);
   hypre_TMemcpy(tmp_j, hypre_CSRMatrixJ(A_offd), NALU_HYPRE_Int, hypre_CSRMatrixNumNonzeros(A_offd),
                 NALU_HYPRE_MEMORY_DEVICE, NALU_HYPRE_MEMORY_DEVICE);
   NALU_HYPRE_THRUST_CALL( sort,
                      tmp_j,
                      tmp_j + hypre_CSRMatrixNumNonzeros(A_offd) );
   tmp_end = NALU_HYPRE_THRUST_CALL( unique,
                                tmp_j,
                                tmp_j + hypre_CSRMatrixNumNonzeros(A_offd) );
   ...
   if (num_cols_A_offd_new < num_cols_A_offd)
   {
      ...
      NALU_HYPRE_Int *offd_mark = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_A_offd, NALU_HYPRE_MEMORY_DEVICE);
      NALU_HYPRE_BigInt *col_map_offd_A_new = hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_A_offd_new,
                                                      NALU_HYPRE_MEMORY_DEVICE);

      NALU_HYPRE_THRUST_CALL( scatter,
                         thrust::counting_iterator<NALU_HYPRE_Int>(0),
                         thrust::counting_iterator<NALU_HYPRE_Int>(num_cols_A_offd_new),
                         tmp_j,
                         offd_mark );
      NALU_HYPRE_THRUST_CALL( gather,
      ...
      hypre_TFree(col_map_offd_A, NALU_HYPRE_MEMORY_DEVICE);
      hypre_TFree(h_col_map_offd_A, NALU_HYPRE_MEMORY_HOST);

      hypre_ParCSRMatrixDeviceColMapOffd(A) = col_map_offd_A_new;
      hypre_ParCSRMatrixColMapOffd(A) = hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_A_offd_new,
                                                     NALU_HYPRE_MEMORY_HOST);
      hypre_TMemcpy(hypre_ParCSRMatrixColMapOffd(A), col_map_offd_A_new, NALU_HYPRE_BigInt,
                    num_cols_A_offd_new,
                    NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
   }
   ...
   if (type != 0)
   {
      hypre_TFree(elmt_tols_diag, NALU_HYPRE_MEMORY_DEVICE);
      hypre_TFree(elmt_tols_offd, NALU_HYPRE_MEMORY_DEVICE);
   }
                                   ...
                                   NALU_HYPRE_Int            data )
{
   ...
   if (num_procs > 1)
   {
      ...
      hypre_CSRMatrixBigJ(A_offdT) = hypre_TAlloc(NALU_HYPRE_BigInt, hypre_CSRMatrixNumNonzeros(A_offdT),
                                                  NALU_HYPRE_MEMORY_DEVICE);
      ...
      tmp_bigj = hypre_TAlloc(NALU_HYPRE_BigInt, Aext_nnz, NALU_HYPRE_MEMORY_DEVICE);
      hypre_TMemcpy(tmp_bigj, hypre_CSRMatrixBigJ(Aext), NALU_HYPRE_BigInt, Aext_nnz, NALU_HYPRE_MEMORY_DEVICE,
                    NALU_HYPRE_MEMORY_DEVICE);
      ...
      col_map_offd_AT = hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_offd_AT, NALU_HYPRE_MEMORY_DEVICE);
      hypre_TMemcpy(col_map_offd_AT, tmp_bigj, NALU_HYPRE_BigInt, num_cols_offd_AT, NALU_HYPRE_MEMORY_DEVICE,
                    NALU_HYPRE_MEMORY_DEVICE);

      hypre_TFree(tmp_bigj, NALU_HYPRE_MEMORY_DEVICE);

      Aext_j = hypre_TAlloc(NALU_HYPRE_Int, Aext_nnz, NALU_HYPRE_MEMORY_DEVICE);
      ...
      hypre_CSRMatrixInitialize_v2(AT_offd, 0, NALU_HYPRE_MEMORY_DEVICE);
      hypreDevice_CsrRowIndicesToPtrs_v2(hypre_CSRMatrixNumRows(AT_offd), Aext_nnz, Aext_ii,
                                         hypre_CSRMatrixI(AT_offd));
      hypre_TFree(Aext_ii, NALU_HYPRE_MEMORY_DEVICE);
   }
   else
   {
   ...
   if (num_cols_offd_AT)
   {
      ...
      hypre_ParCSRMatrixColMapOffd(AT) = hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_offd_AT, NALU_HYPRE_MEMORY_HOST);
      hypre_TMemcpy(hypre_ParCSRMatrixColMapOffd(AT), col_map_offd_AT, NALU_HYPRE_BigInt, num_cols_offd_AT,
                    NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
   }
...
NALU_HYPRE_Int
hypre_ParCSRMatrixAddDevice( NALU_HYPRE_Complex        alpha,
                             hypre_ParCSRMatrix  *A,
                             ...
                             hypre_ParCSRMatrix **C_ptr )
{
   ...
   if (num_procs > 1)
   {
      ...
      NALU_HYPRE_BigInt *tmp = hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_offd_A + num_cols_offd_B,
                                       NALU_HYPRE_MEMORY_DEVICE);
      hypre_TMemcpy(tmp,                   hypre_ParCSRMatrixDeviceColMapOffd(A), NALU_HYPRE_BigInt,
                    num_cols_offd_A, NALU_HYPRE_MEMORY_DEVICE, NALU_HYPRE_MEMORY_DEVICE);
      hypre_TMemcpy(tmp + num_cols_offd_A, hypre_ParCSRMatrixDeviceColMapOffd(B), NALU_HYPRE_BigInt,
      ...
      NALU_HYPRE_BigInt *new_end = NALU_HYPRE_THRUST_CALL( unique, tmp, tmp + num_cols_offd_A + num_cols_offd_B );
      num_cols_offd_C = new_end - tmp;
      d_col_map_offd_C = hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_offd_C, NALU_HYPRE_MEMORY_DEVICE);
      hypre_TMemcpy(d_col_map_offd_C, tmp, NALU_HYPRE_BigInt, num_cols_offd_C, NALU_HYPRE_MEMORY_DEVICE,
                    NALU_HYPRE_MEMORY_DEVICE);
      ...
      hypre_CSRMatrixMemoryLocation(C_offd) = NALU_HYPRE_MEMORY_DEVICE;
   }
   else
   {
      C_offd = hypre_CSRMatrixCreate(hypre_CSRMatrixNumRows(A_offd), 0, 0);
      hypre_CSRMatrixInitialize_v2(C_offd, 0, NALU_HYPRE_MEMORY_DEVICE);
   }
   ...
   if (num_cols_offd_C)
   {
      ...
      hypre_ParCSRMatrixColMapOffd(C) = hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_offd_C, NALU_HYPRE_MEMORY_HOST);
      hypre_TMemcpy(hypre_ParCSRMatrixColMapOffd(C), d_col_map_offd_C, NALU_HYPRE_BigInt, num_cols_offd_C,
                    NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
   }
