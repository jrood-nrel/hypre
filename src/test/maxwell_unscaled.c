         ...
         else if ( strcmp(key, "MatrixSetValues:") == 0 )
         {
            ...
            if ((pdata.matrix_nentries % 10) == 0)
            {
               ...
               pdata.d_matrix_values =
                  nalu_hypre_TReAlloc_v2(pdata.d_matrix_values, NALU_HYPRE_Real, pdata.matrix_values_size,
                                    NALU_HYPRE_Real, size, memory_location);
                ...
                ProblemData  *data_ptr )
{
   ...
   for (part = 0; part < data.nparts; part++)
   {
      ...
      if (pdata.matrix_nentries == 0)
      {
         ...
         nalu_hypre_TFree(pdata.d_matrix_values, memory_location);
      }
      ...
      char     *argv[] )
{
   ...
   values = nalu_hypre_CTAlloc(NALU_HYPRE_Real, data.max_boxsize, NALU_HYPRE_MEMORY_HOST);
   ...
   for (part = 0; part < data.nparts; part++)
   {
      ...
      for (var = 0; var < pdata.nvars; var++)
      {
         ...
         for (i = 0; i < data.stencil_sizes[s]; i++)
         {
            ...
            nalu_hypre_TMemcpy(d_values, values, NALU_HYPRE_Real, data.max_boxsize,
      ...
      nalu_hypre_TMemcpy(pdata.d_graph_values, pdata.graph_values, NALU_HYPRE_Real, pdata.graph_values_size,
                    memory_location, NALU_HYPRE_MEMORY_HOST);
   ...
   for (part = 0; part < data.nparts; part++)
   {
      ...
      nalu_hypre_TMemcpy(pdata.d_matrix_values, pdata.matrix_values, NALU_HYPRE_Real, pdata.matrix_values_size,
                    memory_location, NALU_HYPRE_MEMORY_HOST);
