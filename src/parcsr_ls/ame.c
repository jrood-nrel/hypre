...
NALU_HYPRE_Int nalu_hypre_AMESetMaxPCGIter(void *esolver,
                                 NALU_HYPRE_Int maxit)
{
   nalu_hypre_AMEData *ame_data = (nalu_hypre_AMEData *) esolver;
...
NALU_HYPRE_Int nalu_hypre_AMESetup(void *esolver)
{
...
#endif
         {
            ...
            for (i = 0; i < ne; i++)
            {
               ...
               for (j = AdI[i]; j < AdI[i + 1]; j++)
                  if (AdJ[j] != i)
                  {
                     l1_norm += nalu_hypre_abs(AdA[j]);
                  }
               if (AoI)
                  for (j = AoI[i]; j < AoI[i + 1]; j++)
                  {
                     l1_norm += nalu_hypre_abs(AoA[j]);
                  }
               if (l1_norm < eps)
               {
                  edge_bc[i] = 1;
   ...
   /* Create AMG preconditioner and PCG-AMG solver for G^tMG */
   {
      ...
      NALU_HYPRE_PCGSetMaxIter(ame_data -> B2_G, ame_data -> pcg_maxit);
