...
void
hypre_ParMatmul_RowSizes( NALU_HYPRE_MemoryLocation memory_location,
                          ...
                          NALU_HYPRE_Int   num_cols_offd_C )
{
   ...
   *C_diag_i = hypre_CTAlloc(NALU_HYPRE_Int, num_rows_diag_A + 1, memory_location);
   *C_offd_i = hypre_CTAlloc(NALU_HYPRE_Int, num_rows_diag_A + 1, memory_location);

   jj_count_diag_array = hypre_CTAlloc(NALU_HYPRE_Int, num_threads, NALU_HYPRE_MEMORY_HOST);
   jj_count_offd_array = hypre_CTAlloc(NALU_HYPRE_Int, num_threads, NALU_HYPRE_MEMORY_HOST);
...
#endif
   {
      ...
      {
         B_marker = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_diag_B + num_cols_offd_C, NALU_HYPRE_MEMORY_HOST);
      }
   ...
   *C_offd_size = (*C_offd_i)[num_rows_diag_A];
   ...
   hypre_TFree(jj_count_diag_array, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(jj_count_offd_array, NALU_HYPRE_MEMORY_HOST);
                 ...
                 hypre_ParCSRMatrix  *B )
{
   ...
   hypre_profile_times[NALU_HYPRE_TIMER_ID_MATMUL] -= hypre_MPI_Wtime();
#endif

   /* ParCSRMatrix A */
   MPI_Comm            comm              = hypre_ParCSRMatrixComm(A);
   NALU_HYPRE_BigInt        nrows_A           = hypre_ParCSRMatrixGlobalNumRows(A);
   NALU_HYPRE_BigInt        ncols_A           = hypre_ParCSRMatrixGlobalNumCols(A);
   NALU_HYPRE_BigInt       *row_starts_A      = hypre_ParCSRMatrixRowStarts(A);
   NALU_HYPRE_Int           num_rownnz_A;
   NALU_HYPRE_Int          *rownnz_A = NULL;

   /* ParCSRMatrix B */
   NALU_HYPRE_BigInt        nrows_B           = hypre_ParCSRMatrixGlobalNumRows(B);
   NALU_HYPRE_BigInt        ncols_B           = hypre_ParCSRMatrixGlobalNumCols(B);
   ...
   NALU_HYPRE_Int          *A_diag_ir         = hypre_CSRMatrixRownnz(A_diag);
   NALU_HYPRE_Int           num_rownnz_diag_A = hypre_CSRMatrixNumRownnz(A_diag);
   ...
   NALU_HYPRE_Int          *A_offd_ir         = hypre_CSRMatrixRownnz(A_offd);
   NALU_HYPRE_Int           num_rownnz_offd_A = hypre_CSRMatrixNumRownnz(A_offd);
   NALU_HYPRE_Int           num_rows_offd_A   = hypre_CSRMatrixNumRows(A_offd);
   NALU_HYPRE_Int           num_cols_offd_A   = hypre_CSRMatrixNumCols(A_offd);

   /* B_diag */
   hypre_CSRMatrix    *B_diag            = hypre_ParCSRMatrixDiag(B);
   NALU_HYPRE_Complex      *B_diag_data       = hypre_CSRMatrixData(B_diag);
   NALU_HYPRE_Int          *B_diag_i          = hypre_CSRMatrixI(B_diag);
   NALU_HYPRE_Int          *B_diag_j          = hypre_CSRMatrixJ(B_diag);
   ...
   NALU_HYPRE_Complex      *B_offd_data       = hypre_CSRMatrixData(B_offd);
   NALU_HYPRE_Int          *B_offd_i          = hypre_CSRMatrixI(B_offd);
   NALU_HYPRE_Int          *B_offd_j          = hypre_CSRMatrixJ(B_offd);
   ...
   NALU_HYPRE_MemoryLocation memory_location_A = hypre_ParCSRMatrixMemoryLocation(A);
   NALU_HYPRE_MemoryLocation memory_location_B = hypre_ParCSRMatrixMemoryLocation(B);
   ...
   */
    ...
    * TODO */
   NALU_HYPRE_MemoryLocation memory_location_C = hypre_max(memory_location_A, memory_location_B);

   NALU_HYPRE_ANNOTATE_FUNC_BEGIN;

   max_num_threads = hypre_NumThreads();
   my_diag_array = hypre_CTAlloc(NALU_HYPRE_Int, max_num_threads, NALU_HYPRE_MEMORY_HOST);
   my_offd_array = hypre_CTAlloc(NALU_HYPRE_Int, max_num_threads, NALU_HYPRE_MEMORY_HOST);
   ...
   B_ext_diag_i = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_offd_A + 1, NALU_HYPRE_MEMORY_HOST);
   B_ext_offd_i = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_offd_A + 1, NALU_HYPRE_MEMORY_HOST);
   B_ext_diag_size = 0;
   B_ext_offd_size = 0;
   ...
   #pragma omp parallel
   {
      ...
      else
      {
         ...
         if (B_ext_diag_size)
         {
            B_ext_diag_j = hypre_CTAlloc(NALU_HYPRE_Int,  B_ext_diag_size, NALU_HYPRE_MEMORY_HOST);
            B_ext_diag_data = hypre_CTAlloc(NALU_HYPRE_Complex, B_ext_diag_size, NALU_HYPRE_MEMORY_HOST);
         }
         if (B_ext_offd_size)
         {
            B_ext_offd_j = hypre_CTAlloc(NALU_HYPRE_Int, B_ext_offd_size, NALU_HYPRE_MEMORY_HOST);
            B_big_offd_j = hypre_CTAlloc(NALU_HYPRE_BigInt, B_ext_offd_size, NALU_HYPRE_MEMORY_HOST);
            B_ext_offd_data = hypre_CTAlloc(NALU_HYPRE_Complex, B_ext_offd_size, NALU_HYPRE_MEMORY_HOST);
         }
   ...
   hypre_TFree(my_diag_array, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(my_offd_array, NALU_HYPRE_MEMORY_HOST);
...
#endif
   {
      ...
      else
      {
         ...
         {
            temp = hypre_CTAlloc(NALU_HYPRE_BigInt, B_ext_offd_size + num_cols_offd_B, NALU_HYPRE_MEMORY_HOST);
         }
      }
      ...
      if (ii == 0)
      {
         ...
         if (B_ext_offd_size || num_cols_offd_B)
         {
            ...
            if (num_cols_offd_C)
            {
               col_map_offd_C = hypre_CTAlloc(NALU_HYPRE_BigInt, num_cols_offd_C, NALU_HYPRE_MEMORY_HOST);
            }
   ...
   hypre_TFree(B_big_offd_j, NALU_HYPRE_MEMORY_HOST);
   ...
   last_col_diag_B = first_col_diag_B + (NALU_HYPRE_BigInt)num_cols_diag_B - 1;
   C_diag_data = hypre_CTAlloc(NALU_HYPRE_Complex, C_diag_size, memory_location_C);
   C_diag_j    = hypre_CTAlloc(NALU_HYPRE_Int, C_diag_size, memory_location_C);
   if (C_offd_size)
   {
      C_offd_data = hypre_CTAlloc(NALU_HYPRE_Complex, C_offd_size, memory_location_C);
      C_offd_j    = hypre_CTAlloc(NALU_HYPRE_Int, C_offd_size, memory_location_C);
   }
...
#endif
   {
      ...
      if (num_cols_diag_B || num_cols_offd_C)
      {
         B_marker = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_diag_B + num_cols_offd_C,
    ...
    *  Free various arrays
    *-----------------------------------------------------------------------*/
   hypre_TFree(B_ext_diag_i, NALU_HYPRE_MEMORY_HOST);
   if (B_ext_diag_size)
   {
      hypre_TFree(B_ext_diag_j, NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(B_ext_diag_data, NALU_HYPRE_MEMORY_HOST);
   }
   hypre_TFree(B_ext_offd_i, NALU_HYPRE_MEMORY_HOST);
   if (B_ext_offd_size)
   {
      hypre_TFree(B_ext_offd_j, NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(B_ext_offd_data, NALU_HYPRE_MEMORY_HOST);
   }
   if (num_cols_offd_B)
   {
...
)
{
   ...
   B_int_i = hypre_CTAlloc(NALU_HYPRE_Int,  send_map_starts[num_sends] + 1, NALU_HYPRE_MEMORY_HOST);
   B_ext_i = hypre_CTAlloc(NALU_HYPRE_Int,  num_rows_B_ext + 1, NALU_HYPRE_MEMORY_HOST);
   *pB_ext_i = B_ext_i;
   if ( find_row_map )
   {
      B_int_row_map = hypre_CTAlloc( NALU_HYPRE_BigInt,  send_map_starts[num_sends] + 1, NALU_HYPRE_MEMORY_HOST);
      B_ext_row_map = hypre_CTAlloc( NALU_HYPRE_BigInt,  num_rows_B_ext + 1, NALU_HYPRE_MEMORY_HOST);
   ...
   jdata_send_map_starts = hypre_CTAlloc(NALU_HYPRE_Int,  num_sends + 1, NALU_HYPRE_MEMORY_HOST);
   jdata_recv_vec_starts = hypre_CTAlloc(NALU_HYPRE_Int,  num_recvs + 1, NALU_HYPRE_MEMORY_HOST);
   jdata_send_map_starts[0] = B_int_i[0] = 0;

   /*NALU_HYPRE_Int prefix_sum_workspace[(hypre_NumThreads() + 1)*num_sends];*/
   prefix_sum_workspace = hypre_TAlloc(NALU_HYPRE_Int,  (hypre_NumThreads() + 1) * num_sends,
                                       NALU_HYPRE_MEMORY_HOST);
...
#endif
   {
      ...
      counts = hypre_TAlloc(NALU_HYPRE_Int,  num_sends, NALU_HYPRE_MEMORY_HOST);
...
#endif
      {
         ...
         B_int_j = hypre_TAlloc(NALU_HYPRE_BigInt,  jdata_send_map_starts[num_sends], NALU_HYPRE_MEMORY_HOST);
         if (data) { B_int_data = hypre_TAlloc(NALU_HYPRE_Complex,  jdata_send_map_starts[num_sends], NALU_HYPRE_MEMORY_HOST); }
      }
#ifdef NALU_HYPRE_USING_OPENMP
      #pragma omp barrier
#endif
   ...
   *pB_ext_j = hypre_TAlloc(NALU_HYPRE_BigInt,  *num_nonzeros, NALU_HYPRE_MEMORY_HOST);
   B_ext_j = *pB_ext_j;
   if (data)
   {
      *pB_ext_data = hypre_TAlloc(NALU_HYPRE_Complex,  *num_nonzeros, NALU_HYPRE_MEMORY_HOST);
   ...
   *comm_handle_idx = hypre_ParCSRCommHandleCreate(21, tmp_comm_pkg, B_int_j, B_ext_j);
   ...
   /* Free memory */
   hypre_TFree(jdata_send_map_starts, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(jdata_recv_vec_starts, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(tmp_comm_pkg, NALU_HYPRE_MEMORY_HOST);
   ...
   if (find_row_map)
   {
      hypre_TFree(B_int_row_map, NALU_HYPRE_MEMORY_HOST);
   }
   hypre_TFree(B_int_i, NALU_HYPRE_MEMORY_HOST);
                                       ...
                                       NALU_HYPRE_Int skip_fine, NALU_HYPRE_Int skip_same_sign )
{
   ...
   NALU_HYPRE_BigInt first_col_diag = hypre_ParCSRMatrixFirstColDiag(B);
   /*NALU_HYPRE_Int first_row_index = hypre_ParCSRMatrixFirstRowIndex(B);*/
   NALU_HYPRE_BigInt *col_map_offd = hypre_ParCSRMatrixColMapOffd(B);
   ...
   hypre_CSRMatrixMemoryLocation(B_ext) = NALU_HYPRE_MEMORY_HOST;
                                 ...
                                 NALU_HYPRE_Int            data )
{
   ...
   hypre_CSRMatrix         *A_diag   = hypre_ParCSRMatrixDiag(A);
   hypre_CSRMatrix         *A_offd   = hypre_ParCSRMatrixOffd(A);
   NALU_HYPRE_Int                num_cols = hypre_ParCSRMatrixNumCols(A);
   ...
   NALU_HYPRE_BigInt            *col_starts = hypre_ParCSRMatrixColStarts(A);
   ...
   if (num_procs > 1)
   {
      ...
      AT_buf_i = hypre_CTAlloc(NALU_HYPRE_Int, send_map_starts[num_sends], NALU_HYPRE_MEMORY_HOST);
      if (AT_tmp_i[num_cols_offd])
      {
         AT_big_j = hypre_CTAlloc(NALU_HYPRE_BigInt, AT_tmp_i[num_cols_offd], NALU_HYPRE_MEMORY_HOST);
      }
   ...
   AT_offd_i = hypre_CTAlloc(NALU_HYPRE_Int, num_cols + 1, memory_location);
   ...
   if (num_procs > 1)
   {
      ...
      tmp_send_map_starts = hypre_CTAlloc(NALU_HYPRE_Int, num_sends + 1, NALU_HYPRE_MEMORY_HOST);
      tmp_recv_vec_starts = hypre_CTAlloc(NALU_HYPRE_Int, num_recvs + 1, NALU_HYPRE_MEMORY_HOST);
      ...
      AT_buf_j = hypre_CTAlloc(NALU_HYPRE_BigInt, tmp_send_map_starts[num_sends], NALU_HYPRE_MEMORY_HOST);
      comm_handle = hypre_ParCSRCommHandleCreate(22, tmp_comm_pkg, AT_big_j,
                                                 AT_buf_j);
      hypre_ParCSRCommHandleDestroy(comm_handle);
      comm_handle = NULL;
      hypre_TFree(AT_big_j, NALU_HYPRE_MEMORY_HOST);

      if (data)
      {
         AT_buf_data = hypre_CTAlloc(NALU_HYPRE_Complex, tmp_send_map_starts[num_sends], NALU_HYPRE_MEMORY_HOST);
      ...
      hypre_TFree(tmp_recv_vec_starts, NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(tmp_send_map_starts, NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(tmp_comm_pkg, NALU_HYPRE_MEMORY_HOST);
      hypre_CSRMatrixDestroy(AT_tmp);

      if (AT_offd_i[num_cols])
      {
         AT_offd_j = hypre_CTAlloc(NALU_HYPRE_Int, AT_offd_i[num_cols], memory_location);
         AT_big_j = hypre_CTAlloc(NALU_HYPRE_BigInt, AT_offd_i[num_cols], NALU_HYPRE_MEMORY_HOST);
         if (data)
         {
            AT_offd_data = hypre_CTAlloc(NALU_HYPRE_Complex,  AT_offd_i[num_cols], memory_location);
         }
      }
      else
      {
      ...
      if (num_cols_offd_AT)
      {
         col_map_offd_AT = hypre_CTAlloc(NALU_HYPRE_BigInt, num_cols_offd_AT, NALU_HYPRE_MEMORY_HOST);
      }
      else
      {
         col_map_offd_AT = NULL;
      ...
      hypre_TFree(AT_buf_i, NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(AT_buf_j, NALU_HYPRE_MEMORY_HOST);
      if (data)
      {
         hypre_TFree(AT_buf_data, NALU_HYPRE_MEMORY_HOST);
      }
   ...
   AT = hypre_CTAlloc(hypre_ParCSRMatrix, 1, NALU_HYPRE_MEMORY_HOST);
                                   ...
                                   NALU_HYPRE_Int           G_type )
{
   ...
   else
   {
      ...
      counts = hypre_TAlloc(NALU_HYPRE_Int, nrows_G, NALU_HYPRE_MEMORY_HOST);
      for (i = 0; i < nrows_G; i++) { counts[i] = 0; }
      for (i = 0; i < T_diag_i[ncols_G]; i++) { counts[T_diag_j[i]]++; }
      G_diag_i = hypre_TAlloc(NALU_HYPRE_Int, (nrows_G + 1), NALU_HYPRE_MEMORY_HOST);
      G_diag_j = hypre_TAlloc(NALU_HYPRE_Int, T_diag_i[ncols_G], NALU_HYPRE_MEMORY_HOST);
      G_diag_i[0] = 0;
   ...
   GT_diag_mat = hypre_TAlloc(NALU_HYPRE_Int, 2 * ncols_G, NALU_HYPRE_MEMORY_HOST);
   ...
   nodes_marked = hypre_TAlloc(NALU_HYPRE_Int, nrows_G, NALU_HYPRE_MEMORY_HOST);
   edges_marked = hypre_TAlloc(NALU_HYPRE_Int, ncols_G, NALU_HYPRE_MEMORY_HOST);
   for (i = 0; i < nrows_G; i++) { nodes_marked[i] = 0; }
   for (i = 0; i < ncols_G; i++) { edges_marked[i] = 0; }
   queue = hypre_TAlloc(NALU_HYPRE_Int, nrows_G, NALU_HYPRE_MEMORY_HOST);
   queue_head = 0;
   queue_tail = 1;
   queue[0] = 0;
   nodes_marked[0] = 1;
   ...
   hypre_TFree(nodes_marked, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(queue, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(GT_diag_mat, NALU_HYPRE_MEMORY_HOST);
   ...
   if (nprocs > 1)
   {
      ...
      {
         n_proc_array = 0;
         proc_array = hypre_TAlloc(NALU_HYPRE_Int, (nsends + nrecvs), NALU_HYPRE_MEMORY_HOST);
      ...
      pgraph_i = hypre_TAlloc(NALU_HYPRE_Int, (nprocs + 1), NALU_HYPRE_MEMORY_HOST);
      recv_cnts = hypre_TAlloc(NALU_HYPRE_Int, nprocs, NALU_HYPRE_MEMORY_HOST);
      hypre_MPI_Allgather(&n_proc_array, 1, NALU_HYPRE_MPI_INT, recv_cnts, 1,
      ...
      pgraph_j = hypre_TAlloc(NALU_HYPRE_Int, pgraph_i[nprocs], NALU_HYPRE_MEMORY_HOST);
      hypre_MPI_Allgatherv(proc_array, n_proc_array, NALU_HYPRE_MPI_INT, pgraph_j,
                           recv_cnts, pgraph_i, NALU_HYPRE_MPI_INT, comm);
      hypre_TFree(recv_cnts, NALU_HYPRE_MEMORY_HOST);

      /* BFS on the processor graph to determine parent and children */

      nodes_marked = hypre_TAlloc(NALU_HYPRE_Int, nprocs, NALU_HYPRE_MEMORY_HOST);
      for (i = 0; i < nprocs; i++) { nodes_marked[i] = -1; }
      queue = hypre_TAlloc(NALU_HYPRE_Int, nprocs, NALU_HYPRE_MEMORY_HOST);
      queue_head = 0;
      queue_tail = 1;
      node = 0;
      queue[0] = node;
      ...
      {
         children = hypre_TAlloc(NALU_HYPRE_Int, n_children, NALU_HYPRE_MEMORY_HOST);
         n_children = 0;
         for (i = 0; i < nprocs; i++)
      ...
      hypre_TFree(nodes_marked, NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(queue, NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(pgraph_i, NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(pgraph_j, NALU_HYPRE_MEMORY_HOST);
   }
   ...
   if (n_children > 0)
   {
      hypre_TFree(children, NALU_HYPRE_MEMORY_HOST);
   }
   ...
   t_indices = hypre_TAlloc(NALU_HYPRE_Int, (tree_size + 1), NALU_HYPRE_MEMORY_HOST);
   t_indices[0] = tree_size;
   tree_size = 1;
   for (i = 0; i < ncols_G; i++)
      if (edges_marked[i] == 1) { t_indices[tree_size++] = i; }
   (*indices) = t_indices;
   hypre_TFree(edges_marked, NALU_HYPRE_MEMORY_HOST);
   if (G_type != 0)
   {
      hypre_TFree(G_diag_i, NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(G_diag_j, NALU_HYPRE_MEMORY_HOST);
   }
}
                                           ...
                                           hypre_ParCSRMatrix ***submatrices )
{
   ...
   nrows_A = (NALU_HYPRE_Int) hypre_ParCSRMatrixGlobalNumRows(A_csr);
   ...
   if (nprocs > 1)
   {
      hypre_error_w_msg(NALU_HYPRE_ERROR_GENERIC, "ExtractSubmatrices: cannot handle nprocs > 1 yet.\n");
      exit(1);
   }
   ...
   proc_offsets1 = hypre_TAlloc(NALU_HYPRE_Int, (nprocs + 1), NALU_HYPRE_MEMORY_HOST);
   proc_offsets2 = hypre_TAlloc(NALU_HYPRE_Int, (nprocs + 1), NALU_HYPRE_MEMORY_HOST);
   hypre_MPI_Allgather(&nindices, 1, NALU_HYPRE_MPI_INT, proc_offsets1, 1,
                       NALU_HYPRE_MPI_INT, comm);
   k = 0;
   for (i = 0; i < nprocs; i++)
   {
   ...
   exp_indices = hypre_TAlloc(NALU_HYPRE_Int, nrows_A, NALU_HYPRE_MEMORY_HOST);
   ...
   for (i = 0; i < nindices; i++)
   {
      ...
      {
         hypre_error_w_msg(NALU_HYPRE_ERROR_GENERIC, "ExtractSubmatrices: wrong index %d %d\n");
         exit(1);
      }
   }
   k = 0;
   ...
   A11_csr = hypre_ParCSRMatrixCreate(comm, global_nrows, global_ncols,
                                      ...
                                      nnz_diag, nnz_offd);
   nrows = nindices;
   diag_i = hypre_CTAlloc(NALU_HYPRE_Int,  nrows + 1, NALU_HYPRE_MEMORY_HOST);
   diag_j = hypre_CTAlloc(NALU_HYPRE_Int,  nnz_diag, NALU_HYPRE_MEMORY_HOST);
   diag_a = hypre_CTAlloc(NALU_HYPRE_Complex,  nnz_diag, NALU_HYPRE_MEMORY_HOST);
   nnz = 0;
   row = 0;
   diag_i[0] = 0;
   for (i = 0; i < nrows_A; i++)
   ...
   offd_i = hypre_CTAlloc(NALU_HYPRE_Int,  nrows + 1, NALU_HYPRE_MEMORY_HOST);
   ...
   row_starts = hypre_CTAlloc(NALU_HYPRE_BigInt,  nprocs + 1, NALU_HYPRE_MEMORY_HOST);
   col_starts = hypre_CTAlloc(NALU_HYPRE_BigInt,  nprocs + 1, NALU_HYPRE_MEMORY_HOST);
   for (i = 0; i <= nprocs; i++)
   {
   ...
   hypre_CSRMatrixJ(offd) = NULL;
   hypre_CSRMatrixData(offd) = NULL;
   hypre_TFree(row_starts, NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_TFree(proc_offsets1, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(proc_offsets2, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(exp_indices, NALU_HYPRE_MEMORY_HOST);
}
                                              ...
                                              hypre_ParCSRMatrix ***submatrices )
{
   ...
   for (i = 0; i < nindices; i++)
   {
      ...
      {
         hypre_error_w_msg(NALU_HYPRE_ERROR_GENERIC, "ExtractRowSubmatrices: wrong index %d %d\n");
         exit(1);
      }
   }
   k = 0;
   ...
   offd_i = hypre_CTAlloc(NALU_HYPRE_Int,  nrows + 1, NALU_HYPRE_MEMORY_HOST);
   offd_j = hypre_CTAlloc(NALU_HYPRE_Int,  nnz_offd, NALU_HYPRE_MEMORY_HOST);
   offd_a = hypre_CTAlloc(NALU_HYPRE_Complex,  nnz_offd, NALU_HYPRE_MEMORY_HOST);
   nnz = 0;
   row = 0;
   offd_i[0] = 0;
   for (i = 0; i < nrows_A; i++)
                           ...
                           hypre_ParCSRMatrix **C_ptr)
{
   ...
   NALU_HYPRE_Int             num_rows       = hypre_CSRMatrixNumRows(B_diag);
   NALU_HYPRE_Int            *B_diag_i       = hypre_CSRMatrixI(B_diag);
   NALU_HYPRE_Int            *B_diag_j       = hypre_CSRMatrixJ(B_diag);
   NALU_HYPRE_Complex        *B_diag_data    = hypre_CSRMatrixData(B_diag);
   ...
   D_tmp = hypre_CTAlloc(NALU_HYPRE_Complex, num_rows, NALU_HYPRE_MEMORY_HOST);

   if (num_cols_offd_A)
   {
      map_to_B = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_offd_A, NALU_HYPRE_MEMORY_HOST);
      cnt = 0;
      for (i = 0; i < num_cols_offd_A; i++)
      {
   ...
   for (ii = 0; ii < num_threads; ii++)
   {
      ...
      A_marker = hypre_CTAlloc(NALU_HYPRE_Int,  nmax, NALU_HYPRE_MEMORY_HOST);
      ...
      hypre_TFree(A_marker, NALU_HYPRE_MEMORY_HOST);
   ...
   recv_procs_C      = hypre_CTAlloc(NALU_HYPRE_Int, num_recvs_B, NALU_HYPRE_MEMORY_HOST);
   recv_vec_starts_C = hypre_CTAlloc(NALU_HYPRE_Int, num_recvs_B + 1, NALU_HYPRE_MEMORY_HOST);
   send_procs_C      = hypre_CTAlloc(NALU_HYPRE_Int, num_sends_B, NALU_HYPRE_MEMORY_HOST);
   send_map_starts_C = hypre_CTAlloc(NALU_HYPRE_Int, num_sends_B + 1, NALU_HYPRE_MEMORY_HOST);
   send_map_elmts_C  = hypre_CTAlloc(NALU_HYPRE_Int, send_map_starts_B[num_sends_B], NALU_HYPRE_MEMORY_HOST);
   ...
   if (num_cols_offd_A)
   {
      hypre_TFree(map_to_B, NALU_HYPRE_MEMORY_HOST);
   }
                  ...
                  hypre_ParCSRMatrix  *B)
{
   ...
   */
   ...
   if (C_ext_size || num_cols_offd_B)
   {
      ...
      C_ext_diag_i = hypre_CTAlloc(NALU_HYPRE_Int,  C_ext_num_rows + 1, NALU_HYPRE_MEMORY_HOST);
      C_ext_offd_i = hypre_CTAlloc(NALU_HYPRE_Int,  C_ext_num_rows + 1, NALU_HYPRE_MEMORY_HOST);
      temp = hypre_CTAlloc(NALU_HYPRE_BigInt,  C_ext_size + num_cols_offd_B, NALU_HYPRE_MEMORY_HOST);
      C_ext_diag_size = 0;
      C_ext_offd_size = 0;
      ...
      if (C_ext_diag_size)
      {
         C_ext_diag_j = hypre_CTAlloc(NALU_HYPRE_Int,  C_ext_diag_size, NALU_HYPRE_MEMORY_HOST);
         C_ext_diag_data = hypre_CTAlloc(NALU_HYPRE_Complex,  C_ext_diag_size, NALU_HYPRE_MEMORY_HOST);
      }
      if (C_ext_offd_size)
      {
         C_ext_offd_j = hypre_CTAlloc(NALU_HYPRE_Int,  C_ext_offd_size, NALU_HYPRE_MEMORY_HOST);
         C_ext_offd_data = hypre_CTAlloc(NALU_HYPRE_Complex,  C_ext_offd_size, NALU_HYPRE_MEMORY_HOST);
      }
   ...
   if (C_ext_size || num_cols_offd_B)
   {
      C_diag_i = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_diag_A + 1, memory_location_C);
      C_offd_i = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_diag_A + 1, memory_location_C);

      C_diag_array = hypre_CTAlloc(NALU_HYPRE_Int,  max_num_threads, NALU_HYPRE_MEMORY_HOST);
      C_offd_array = hypre_CTAlloc(NALU_HYPRE_Int,  max_num_threads, NALU_HYPRE_MEMORY_HOST);
...
#endif
      {
         ...
         B_marker = hypre_CTAlloc(NALU_HYPRE_Int,  num_cols_diag_B, NALU_HYPRE_MEMORY_HOST);
         B_marker_offd = hypre_CTAlloc(NALU_HYPRE_Int,  num_cols_offd_C, NALU_HYPRE_MEMORY_HOST);
         ...
         hypre_TFree(B_marker, NALU_HYPRE_MEMORY_HOST);
         hypre_TFree(B_marker_offd, NALU_HYPRE_MEMORY_HOST);

      } /*end parallel region */

      hypre_TFree(C_diag_array, NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(C_offd_array, NALU_HYPRE_MEMORY_HOST);
   }
   ...
   C = hypre_CTAlloc(hypre_ParCSRMatrix, 1, NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_ParCSRMatrixLastRowIndex(C) = first_row_index + (NALU_HYPRE_BigInt)local_num_rows - 1;
   hypre_ParCSRMatrixLastColDiag(C) = first_col_diag + (NALU_HYPRE_BigInt)local_num_cols - 1;
   ...
   if (num_cols_offd_C)
   {
      ...
      P_marker = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_offd_C, NALU_HYPRE_MEMORY_HOST);
      for (i = 0; i < num_cols_offd_C; i++)
      {
         P_marker[i] = -1;
      }
      ...
      {
         new_col_map_offd_C = hypre_CTAlloc(NALU_HYPRE_BigInt, jj_count_offd, NALU_HYPRE_MEMORY_HOST);
         jj_count_offd = 0;
         for (i = 0; i < num_cols_offd_C; i++)
         {
            if (!P_marker[i])
         ...
         hypre_TFree(col_map_offd_C, NALU_HYPRE_MEMORY_HOST);
   ...
   if (C_ext_size || num_cols_offd_B)
   {
      hypre_TFree(C_ext_diag_i, NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(C_ext_offd_i, NALU_HYPRE_MEMORY_HOST);
   }

   if (C_ext_diag_size)
   {
      hypre_TFree(C_ext_diag_j, NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(C_ext_diag_data, NALU_HYPRE_MEMORY_HOST);
   }

   if (C_ext_offd_size)
   {
      hypre_TFree(C_ext_offd_j, NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(C_ext_offd_data, NALU_HYPRE_MEMORY_HOST);
   }
   ...
   if ( hypre_GetExecPolicy2(memory_location_A, memory_location_B) == NALU_HYPRE_EXEC_DEVICE )
   {
                          ...
                          hypre_ParCSRMatrix  *A)
{
   ...
   NALU_HYPRE_BigInt nrow_global = hypre_ParVectorGlobalSize(b);
   NALU_HYPRE_BigInt first_row   = hypre_ParVectorFirstIndex(b);
   NALU_HYPRE_BigInt last_row    = hypre_ParVectorLastIndex(b);
   ...
   NALU_HYPRE_BigInt end_row_block   = hypre_min( (last_row / (NALU_HYPRE_BigInt)blockSize + 1) *
   ...
   NALU_HYPRE_Complex   *b_local_data = hypre_VectorData(b_local);
   /* number of sends (#procs) */
   NALU_HYPRE_Int num_sends = hypre_ParCSRCommPkgNumSends(comm_pkg);
   /* number of rows to send */
   NALU_HYPRE_Int num_rows_send = hypre_ParCSRCommPkgSendMapStart(comm_pkg, num_sends);
   /* number of recvs (#procs) */
   NALU_HYPRE_Int num_recvs = hypre_ParCSRCommPkgNumRecvs(comm_pkg);
   /* number of rows to recv */
   NALU_HYPRE_Int num_rows_recv = hypre_ParCSRCommPkgRecvVecStart(comm_pkg, num_recvs);
   hypre_ParCSRCommHandle  *comm_handle;
   ...
   NALU_HYPRE_Complex   *bnew_local_data = hypre_VectorData(bnew_local);

   /* send and recv b */
   NALU_HYPRE_Complex *send_b = hypre_TAlloc(NALU_HYPRE_Complex, num_rows_send, NALU_HYPRE_MEMORY_HOST);
   NALU_HYPRE_Complex *recv_b = hypre_TAlloc(NALU_HYPRE_Complex, num_rows_recv, NALU_HYPRE_MEMORY_HOST);
   ...
   for (block_start = first_row_block; block_start < end_row_block; block_start += blockSize)
   {
      ...
      block_end = hypre_min(block_start + (NALU_HYPRE_BigInt)blockSize, nrow_global);
   ...
   hypre_TFree(send_b, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(recv_b, NALU_HYPRE_MEMORY_HOST);
   *bs = bnew;
                          ...
                          hypre_ParCSRMatrix  **As)
{
   ...
   /* diag part of A */
   hypre_CSRMatrix *A_diag   = hypre_ParCSRMatrixDiag(A);
   NALU_HYPRE_Real      *A_diag_a = hypre_CSRMatrixData(A_diag);
   NALU_HYPRE_Int       *A_diag_i = hypre_CSRMatrixI(A_diag);
   NALU_HYPRE_Int       *A_diag_j = hypre_CSRMatrixJ(A_diag);
   /* off-diag part of A */
   hypre_CSRMatrix *A_offd   = hypre_ParCSRMatrixOffd(A);
   NALU_HYPRE_Real      *A_offd_a = hypre_CSRMatrixData(A_offd);
   NALU_HYPRE_Int       *A_offd_i = hypre_CSRMatrixI(A_offd);
   NALU_HYPRE_Int       *A_offd_j = hypre_CSRMatrixJ(A_offd);
   ...
   NALU_HYPRE_Int nrow_local = hypre_CSRMatrixNumRows(A_diag);
   NALU_HYPRE_BigInt first_row  = hypre_ParCSRMatrixFirstRowIndex(A);
   NALU_HYPRE_BigInt last_row   = hypre_ParCSRMatrixLastRowIndex(A);
   NALU_HYPRE_BigInt end_row    = first_row + (NALU_HYPRE_BigInt)nrow_local; /* one past-the-last */

   NALU_HYPRE_Int ncol_local = hypre_CSRMatrixNumCols(A_diag);
   NALU_HYPRE_BigInt first_col  = hypre_ParCSRMatrixFirstColDiag(A);
   /* NALU_HYPRE_Int last_col   = hypre_ParCSRMatrixLastColDiag(A); */
   NALU_HYPRE_BigInt end_col    = first_col + (NALU_HYPRE_BigInt)ncol_local;

   NALU_HYPRE_BigInt nrow_global = hypre_ParCSRMatrixGlobalNumRows(A);
   NALU_HYPRE_BigInt ncol_global = hypre_ParCSRMatrixGlobalNumCols(A);
   NALU_HYPRE_BigInt *row_starts = hypre_ParCSRMatrixRowStarts(A);
   void *request;
   ...
   NALU_HYPRE_Real *dense_all = hypre_CTAlloc(NALU_HYPRE_Complex, num_blocks * blockSize * blockSize,
                                         NALU_HYPRE_MEMORY_HOST);
   NALU_HYPRE_Real *dense = dense_all;
   NALU_HYPRE_Int *IPIV  = hypre_TAlloc(NALU_HYPRE_Int, blockSize, NALU_HYPRE_MEMORY_HOST);
   ...
   A_diag_i_new = hypre_CTAlloc(NALU_HYPRE_Int,     nrow_local + 1, NALU_HYPRE_MEMORY_HOST);
   A_diag_j_new = hypre_CTAlloc(NALU_HYPRE_Int,     nnz_diag_alloc, NALU_HYPRE_MEMORY_HOST);
   A_diag_a_new = hypre_CTAlloc(NALU_HYPRE_Complex, nnz_diag_alloc, NALU_HYPRE_MEMORY_HOST);
   A_offd_i_new = hypre_CTAlloc(NALU_HYPRE_Int,     nrow_local + 1, NALU_HYPRE_MEMORY_HOST);
   A_offd_j_new = hypre_CTAlloc(NALU_HYPRE_Int,     nnz_offd_alloc, NALU_HYPRE_MEMORY_HOST);
   A_offd_a_new = hypre_CTAlloc(NALU_HYPRE_Complex, nnz_offd_alloc, NALU_HYPRE_MEMORY_HOST);
   ...
   ext_indices = hypre_CTAlloc(NALU_HYPRE_BigInt, num_ext_rows, NALU_HYPRE_MEMORY_HOST);
   j = 0;
   ...
   hypre_TFree(ext_indices, NALU_HYPRE_MEMORY_HOST);

   A_ext_i = hypre_CSRMatrixI(A_ext);
   A_ext_j = hypre_CSRMatrixBigJ(A_ext);
   A_ext_a = hypre_CSRMatrixData(A_ext);
   A_ext_nnz = A_ext_i[num_ext_rows];
   ext_offd = hypre_CTAlloc(NALU_HYPRE_BigInt, A_ext_nnz, NALU_HYPRE_MEMORY_HOST);
   ...
   col_map_offd_A_new = hypre_CTAlloc(NALU_HYPRE_BigInt, num_cols_A_offd + k, NALU_HYPRE_MEMORY_HOST);
   if (k)
   {
      /* map offd to offd_new */
      offd2new = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_A_offd, NALU_HYPRE_MEMORY_HOST);
   }
   hypre_union2(num_cols_A_offd, col_map_offd_A, k, ext_offd,
                &num_cols_A_offd_new, col_map_offd_A_new, offd2new, NULL);
   hypre_TFree(ext_offd, NALU_HYPRE_MEMORY_HOST);
   /*
   ...
   /* marker for diag */
   marker_diag = hypre_TAlloc(NALU_HYPRE_Int, ncol_local, NALU_HYPRE_MEMORY_HOST);
   for (i = 0; i < ncol_local; i++)
   {
      marker_diag[i] = -1;
   }
   /* marker for newoffd */
   marker_newoffd = hypre_TAlloc(NALU_HYPRE_Int, num_cols_A_offd_new, NALU_HYPRE_MEMORY_HOST);
        ...
        block_start += (NALU_HYPRE_BigInt)blockSize)
   {
      ...
      if (lapack_info == 0)
      {
         ...
         if (lwork_opt > dgetri_lwork)
         {
            ...
            dgetri_work = hypre_TReAlloc(dgetri_work, NALU_HYPRE_Complex, dgetri_lwork, NALU_HYPRE_MEMORY_HOST);
         }
      ...
      for (big_i = block_start; big_i < block_end; big_i++)
      {
         ...
         if (square2)
         {
            ...
            if (nnz_diag_new == nnz_diag_alloc)
            {
               ...
               A_diag_j_new = hypre_TReAlloc(A_diag_j_new, NALU_HYPRE_Int,     nnz_diag_alloc, NALU_HYPRE_MEMORY_HOST);
               A_diag_a_new = hypre_TReAlloc(A_diag_a_new, NALU_HYPRE_Complex, nnz_diag_alloc, NALU_HYPRE_MEMORY_HOST);
            }
               ...
               for (ii = A_offd_i[rid]; ii < A_offd_i[rid + 1]; ii++)
               {
                  ...
                  if (marker_newoffd[col_new] < offd_i_start)
                  {
                     ...
                     if (nnz_offd_new == nnz_offd_alloc)
                     {
                        ...
                        A_offd_j_new = hypre_TReAlloc(A_offd_j_new, NALU_HYPRE_Int,     nnz_offd_alloc, NALU_HYPRE_MEMORY_HOST);
                        A_offd_a_new = hypre_TReAlloc(A_offd_a_new, NALU_HYPRE_Complex, nnz_offd_alloc, NALU_HYPRE_MEMORY_HOST);
                     }
   ...
   /* done with all rows */
   /* resize properly */
   A_diag_j_new = hypre_TReAlloc(A_diag_j_new, NALU_HYPRE_Int,     nnz_diag_new, NALU_HYPRE_MEMORY_HOST);
   A_diag_a_new = hypre_TReAlloc(A_diag_a_new, NALU_HYPRE_Complex, nnz_diag_new, NALU_HYPRE_MEMORY_HOST);
   A_offd_j_new = hypre_TReAlloc(A_offd_j_new, NALU_HYPRE_Int,     nnz_offd_new, NALU_HYPRE_MEMORY_HOST);
   A_offd_a_new = hypre_TReAlloc(A_offd_a_new, NALU_HYPRE_Complex, nnz_offd_new, NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_ParCSRMatrixDNumNonzeros(Anew) = (NALU_HYPRE_Real) hypre_ParCSRMatrixNumNonzeros(Anew);
   ...
   *As = Anew;
   ...
   {
      hypre_TFree(dense_all, NALU_HYPRE_MEMORY_HOST);
   }
   */
   ...
   hypre_TFree(dgetri_work, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(marker_diag, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(marker_newoffd, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(offd2new, NALU_HYPRE_MEMORY_HOST);
                                 ...
                                 void                **request_ptr)
{
   ...
   send_i = hypre_TAlloc(NALU_HYPRE_Int, num_rows_send, NALU_HYPRE_MEMORY_HOST);
   recv_i = hypre_CTAlloc(NALU_HYPRE_Int, num_rows_recv + 1, NALU_HYPRE_MEMORY_HOST);
   ...
   send_j = hypre_TAlloc(NALU_HYPRE_BigInt, num_nnz_send, NALU_HYPRE_MEMORY_HOST);
   if (want_data)
   {
      send_a = hypre_TAlloc(NALU_HYPRE_Complex, num_nnz_send, NALU_HYPRE_MEMORY_HOST);
   }

   send_i_offset = hypre_TAlloc(NALU_HYPRE_Int, num_rows_send + 1, NALU_HYPRE_MEMORY_HOST);
   send_i_offset[0] = 0;
   hypre_TMemcpy(send_i_offset + 1, send_i, NALU_HYPRE_Int, num_rows_send,
   ...
   send_jstarts = hypre_TAlloc(NALU_HYPRE_Int, num_sends + 1, NALU_HYPRE_MEMORY_HOST);
#ifdef NALU_HYPRE_USING_OPENMP
   ...
   recv_j = hypre_CTAlloc(NALU_HYPRE_BigInt, num_nnz_recv, NALU_HYPRE_MEMORY_HOST);
   if (want_data)
   {
      recv_a = hypre_CTAlloc(NALU_HYPRE_Complex, num_nnz_recv, NALU_HYPRE_MEMORY_HOST);
   }
   recv_jstarts = hypre_CTAlloc(NALU_HYPRE_Int, num_recvs + 1, NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_CSRMatrixMemoryLocation(A_ext) = NALU_HYPRE_MEMORY_HOST;
   hypre_CSRMatrixI   (A_ext) = recv_i;
   hypre_CSRMatrixBigJ(A_ext) = recv_j;
   hypre_CSRMatrixData(A_ext) = recv_a;

   /* output */
   vrequest = hypre_TAlloc(void *, 4, NALU_HYPRE_MEMORY_HOST);
   ...
   *request_ptr = (void *) vrequest;
   ...
   /* free */
   hypre_TFree(send_i, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(send_i_offset, NALU_HYPRE_MEMORY_HOST);
...
hypre_ParcsrGetExternalRowsWait(void *vrequest)
{
   ...
   NALU_HYPRE_BigInt           *send_j        = (NALU_HYPRE_BigInt *) hypre_ParCSRCommHandleSendData(
                                              comm_handle_j);

   if (comm_handle_a)
   {
      NALU_HYPRE_Complex *send_a = (NALU_HYPRE_Complex *) hypre_ParCSRCommHandleSendData(comm_handle_a);
      hypre_ParCSRCommHandleDestroy(comm_handle_a);
      hypre_TFree(send_a, NALU_HYPRE_MEMORY_HOST);
   }

   hypre_ParCSRCommHandleDestroy(comm_handle_j);
   hypre_TFree(send_j, NALU_HYPRE_MEMORY_HOST);

   hypre_TFree(hypre_ParCSRCommPkgSendMapStarts(comm_pkg_j), NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(hypre_ParCSRCommPkgRecvVecStarts(comm_pkg_j), NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(comm_pkg_j, NALU_HYPRE_MEMORY_HOST);
...
NALU_HYPRE_Int
hypre_ParCSRMatrixAddHost( NALU_HYPRE_Complex        alpha,
                           hypre_ParCSRMatrix  *A,
                           ...
                           hypre_ParCSRMatrix **C_ptr )
{
   ...
   /* NALU_HYPRE_BigInt      num_rows_B = hypre_ParCSRMatrixGlobalNumRows(B); */
   /* NALU_HYPRE_BigInt      num_cols_B = hypre_ParCSRMatrixGlobalNumCols(B); */

   /* diag part of A */
   hypre_CSRMatrix    *A_diag   = hypre_ParCSRMatrixDiag(A);
   NALU_HYPRE_Int     *rownnz_diag_A = hypre_CSRMatrixRownnz(A_diag);
   NALU_HYPRE_Int  num_rownnz_diag_A = hypre_CSRMatrixNumRownnz(A_diag);
   NALU_HYPRE_Int    num_rows_diag_A = hypre_CSRMatrixNumRows(A_diag);
   NALU_HYPRE_Int    num_cols_diag_A = hypre_CSRMatrixNumCols(A_diag);
   ...
   /* off-diag part of A */
   hypre_CSRMatrix    *A_offd   = hypre_ParCSRMatrixOffd(A);
   NALU_HYPRE_Int     *rownnz_offd_A = hypre_CSRMatrixRownnz(A_offd);
   NALU_HYPRE_Int  num_rownnz_offd_A = hypre_CSRMatrixNumRownnz(A_offd);
   NALU_HYPRE_Int    num_rows_offd_A = hypre_CSRMatrixNumRows(A_offd);
   NALU_HYPRE_Int    num_cols_offd_A = hypre_CSRMatrixNumCols(A_offd);
   NALU_HYPRE_BigInt *col_map_offd_A = hypre_ParCSRMatrixColMapOffd(A);
   ...
   /* diag part of B */
   hypre_CSRMatrix    *B_diag   = hypre_ParCSRMatrixDiag(B);
   NALU_HYPRE_Int     *rownnz_diag_B = hypre_CSRMatrixRownnz(B_diag);
   NALU_HYPRE_Int  num_rownnz_diag_B = hypre_CSRMatrixNumRownnz(B_diag);
   NALU_HYPRE_Int    num_rows_diag_B = hypre_CSRMatrixNumRows(B_diag);
   /* NALU_HYPRE_Int    num_cols_diag_B = hypre_CSRMatrixNumCols(B_diag); */

   /* off-diag part of B */
   hypre_CSRMatrix    *B_offd   = hypre_ParCSRMatrixOffd(B);
   NALU_HYPRE_Int     *rownnz_offd_B = hypre_CSRMatrixRownnz(B_offd);
   NALU_HYPRE_Int  num_rownnz_offd_B = hypre_CSRMatrixNumRownnz(B_offd);
   NALU_HYPRE_Int    num_rows_offd_B = hypre_CSRMatrixNumRows(B_offd);
   NALU_HYPRE_Int    num_cols_offd_B = hypre_CSRMatrixNumCols(B_offd);
   NALU_HYPRE_BigInt *col_map_offd_B = hypre_ParCSRMatrixColMapOffd(B);
   ...
   */
   ...
   /* Allocate memory */
   twspace  = hypre_TAlloc(NALU_HYPRE_Int, hypre_NumThreads(), NALU_HYPRE_MEMORY_HOST);
   C_diag_i = hypre_CTAlloc(NALU_HYPRE_Int, num_rows_diag_A + 1, memory_location_C);
   C_offd_i = hypre_CTAlloc(NALU_HYPRE_Int, num_rows_offd_A + 1, memory_location_C);
   col_map_offd_C = hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_offd_C, NALU_HYPRE_MEMORY_HOST);

   /* Compute num_cols_offd_C, A2C_offd, and B2C_offd*/
   A2C_offd = hypre_TAlloc(NALU_HYPRE_Int, num_cols_offd_A, NALU_HYPRE_MEMORY_HOST);
   B2C_offd = hypre_TAlloc(NALU_HYPRE_Int, num_cols_offd_B, NALU_HYPRE_MEMORY_HOST);
...
#endif
   {
      ...
      marker_diag = hypre_TAlloc(NALU_HYPRE_Int, num_cols_diag_A, NALU_HYPRE_MEMORY_HOST);
      ...
      marker_offd = hypre_TAlloc(NALU_HYPRE_Int, num_cols_offd_C, NALU_HYPRE_MEMORY_HOST);
      ...
      hypre_TFree(marker_offd, NALU_HYPRE_MEMORY_HOST);
   } /* end of omp parallel region */

   /* Free memory */
   hypre_TFree(twspace, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(A2C_offd, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(B2C_offd, NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_ParCSRMatrixDNumNonzeros(C) = (NALU_HYPRE_Real) hypre_ParCSRMatrixNumNonzeros(C);
...
NALU_HYPRE_Int
hypre_ParCSRMatrixAdd( NALU_HYPRE_Complex        alpha,
                       hypre_ParCSRMatrix  *A,
                       NALU_HYPRE_Complex        beta,
                       hypre_ParCSRMatrix  *B,
                       hypre_ParCSRMatrix **C_ptr )
{
   ...
   if ( hypre_GetExecPolicy2( hypre_ParCSRMatrixMemoryLocation(A),
                              hypre_ParCSRMatrixMemoryLocation(B) ) == NALU_HYPRE_EXEC_DEVICE )
   {
...
hypre_ParCSRMatrixFnorm( hypre_ParCSRMatrix *A )
{
   ...
   hypre_MPI_Allreduce(&local_result, &result, 1, NALU_HYPRE_MPI_REAL, hypre_MPI_SUM, comm);
                           ...
                           NALU_HYPRE_Real          *norm )
{
   ...
   hypre_MPI_Allreduce(&maxsum, norm, 1, NALU_HYPRE_MPI_REAL, hypre_MPI_MAX, comm);
                                ...
                                void                **request_ptr)
{
   ...
   NALU_HYPRE_Int  num_recvs        = hypre_ParCSRCommPkgNumRecvs(comm_pkg_A);
   NALU_HYPRE_Int *recv_procs       = hypre_ParCSRCommPkgRecvProcs(comm_pkg_A);
   NALU_HYPRE_Int *recv_vec_starts  = hypre_ParCSRCommPkgRecvVecStarts(comm_pkg_A);
   NALU_HYPRE_Int  num_sends        = hypre_ParCSRCommPkgNumSends(comm_pkg_A);
   NALU_HYPRE_Int *send_procs       = hypre_ParCSRCommPkgSendProcs(comm_pkg_A);
   NALU_HYPRE_Int *send_map_starts  = hypre_ParCSRCommPkgSendMapStarts(comm_pkg_A);

   NALU_HYPRE_Int  num_elmts_send   = send_map_starts[num_sends];
   NALU_HYPRE_Int  num_elmts_recv   = recv_vec_starts[num_recvs];

   NALU_HYPRE_Int     *B_ext_i      = B_ext ? hypre_CSRMatrixI(B_ext) : NULL;
   NALU_HYPRE_BigInt  *B_ext_j      = B_ext ? hypre_CSRMatrixBigJ(B_ext) : NULL;
   NALU_HYPRE_Complex *B_ext_data   = B_ext ? hypre_CSRMatrixData(B_ext) : NULL;
   NALU_HYPRE_Int      B_ext_ncols  = B_ext ? hypre_CSRMatrixNumCols(B_ext) : 0;
   NALU_HYPRE_Int      B_ext_nrows  = B_ext ? hypre_CSRMatrixNumRows(B_ext) : 0;
   NALU_HYPRE_Int     *B_ext_rownnz = hypre_CTAlloc(NALU_HYPRE_Int, B_ext_nrows, NALU_HYPRE_MEMORY_HOST);
   ...
   NALU_HYPRE_Int       *B_int_i     = hypre_TAlloc(NALU_HYPRE_Int, B_int_nrows + 1, NALU_HYPRE_MEMORY_HOST);
   ...
   jdata_send_map_starts = hypre_TAlloc(NALU_HYPRE_Int, num_sends + 1, NALU_HYPRE_MEMORY_HOST);
   ...
   jdata_recv_vec_starts = hypre_TAlloc(NALU_HYPRE_Int, num_recvs + 1, NALU_HYPRE_MEMORY_HOST);
   ...
   B_int_j    = hypre_TAlloc(NALU_HYPRE_BigInt,  B_int_nnz, NALU_HYPRE_MEMORY_HOST);
   B_int_data = hypre_TAlloc(NALU_HYPRE_Complex, B_int_nnz, NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_CSRMatrixMemoryLocation(B_int) = NALU_HYPRE_MEMORY_HOST;
   ...
   *request_ptr = (void *) vrequest;
   ...
   hypre_TFree(B_ext_rownnz, NALU_HYPRE_MEMORY_HOST);
                                      ...
                                      NALU_HYPRE_Real           strength_thresh)
{
   ...
   NALU_HYPRE_Int           num_cols_A_offd = hypre_CSRMatrixNumCols(A_offd);
   //NALU_HYPRE_Int          *col_map_offd_A  = hypre_ParCSRMatrixColMapOffd(A);
   ...
   if (row_set == -1 || col_set == -1)
   {
      ...
      hypre_MPI_Scan(&big_nf_local, fpts_starts + 1, 1, NALU_HYPRE_MPI_BIG_INT, hypre_MPI_SUM, comm);
   ...
   sub_idx_diag = hypre_TAlloc(NALU_HYPRE_Int, A_nlocal, NALU_HYPRE_MEMORY_HOST);
   ...
   num_sends = hypre_ParCSRCommPkgNumSends(comm_pkg);
   send_buf_data = hypre_TAlloc(NALU_HYPRE_BigInt, hypre_ParCSRCommPkgSendMapStart(comm_pkg, num_sends),
                                NALU_HYPRE_MEMORY_HOST);
   k = 0;
   for (i = 0; i < num_sends; i++)
   {
      /* start pos of elements sent to send_proc[i] */
      NALU_HYPRE_Int si = hypre_ParCSRCommPkgSendMapStart(comm_pkg, i);
      NALU_HYPRE_Int ei = hypre_ParCSRCommPkgSendMapStart(comm_pkg, i + 1);
      /* loop through all elems to send_proc[i] */
      for (j = si; j < ei; j++)
      {
         /* j1: local idx */
         NALU_HYPRE_BigInt j1 = sub_idx_diag[hypre_ParCSRCommPkgSendMapElmt(comm_pkg, j)];
         if (j1 != -1)
         {
   ...
   /* recv buffer */
   sub_idx_offd = hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_A_offd, NALU_HYPRE_MEMORY_HOST);
   ...
   col_map_offd_B = hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_B_offd, NALU_HYPRE_MEMORY_HOST);
   ...
   B_maxel_row = hypre_TAlloc(NALU_HYPRE_Real, B_nrow_local, NALU_HYPRE_MEMORY_HOST);
   B_diag_i = hypre_TAlloc(NALU_HYPRE_Int, B_nrow_local + 1, NALU_HYPRE_MEMORY_HOST);
   B_offd_i = hypre_TAlloc(NALU_HYPRE_Int, B_nrow_local + 1, NALU_HYPRE_MEMORY_HOST);
   ...
   B_diag_j = hypre_TAlloc(NALU_HYPRE_Int,     B_nnz_diag, NALU_HYPRE_MEMORY_HOST);
   B_diag_a = hypre_TAlloc(NALU_HYPRE_Complex, B_nnz_diag, NALU_HYPRE_MEMORY_HOST);
   B_offd_j = hypre_TAlloc(NALU_HYPRE_Int,     B_nnz_offd, NALU_HYPRE_MEMORY_HOST);
   B_offd_a = hypre_TAlloc(NALU_HYPRE_Complex, B_nnz_offd, NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_CSRMatrixMemoryLocation(B_diag) = NALU_HYPRE_MEMORY_HOST;
   ...
   hypre_CSRMatrixMemoryLocation(B_offd) = NALU_HYPRE_MEMORY_HOST;
   ...
   hypre_ParCSRMatrixDNumNonzeros(B) = (NALU_HYPRE_Real) hypre_ParCSRMatrixNumNonzeros(B);

   hypre_MatvecCommPkgCreate(B);

   *B_ptr = B;

   hypre_TFree(B_maxel_row, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(send_buf_data, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(sub_idx_diag, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(sub_idx_offd, NALU_HYPRE_MEMORY_HOST);
                                        ...
                                        NALU_HYPRE_Int           type)
{
   ...
   marker_offd = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_A_offd, NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_ParCSRMatrixDNumNonzeros(A) = (NALU_HYPRE_Real) hypre_ParCSRMatrixNumNonzeros(A);
...
hypre_ParCSRMatrixReorder(hypre_ParCSRMatrix *A)
{
   ...
   if (nrows_A != ncols_A)
   {
      hypre_error_w_msg(NALU_HYPRE_ERROR_GENERIC, " Error! Matrix should be square!\n");
      return hypre_error_flag;
   }
                                 ...
                                 hypre_ParVector    *par_x )
{
   ...
   NALU_HYPRE_Int           x_num_vectors = hypre_VectorNumVectors(x);
   NALU_HYPRE_Int           x_vecstride   = hypre_VectorVectorStride(x);

   /* Local vector y info */
   NALU_HYPRE_Complex      *y_data        = hypre_VectorData(y);
   NALU_HYPRE_Int           y_vecstride   = hypre_VectorVectorStride(y);

   /* Local matrix A info */
   NALU_HYPRE_Complex      *A_data        = hypre_CSRMatrixData(A_diag);
                             ...
                             hypre_ParVector    *par_x )
{
   ...
   NALU_HYPRE_Int           y_num_vectors = hypre_VectorNumVectors(y);
   NALU_HYPRE_Int           y_vecstride   = hypre_VectorVectorStride(y);
   ...
   {
      hypre_error_w_msg(NALU_HYPRE_ERROR_GENERIC, "Error! incompatible number of vectors!\n");
      return hypre_error_flag;
   }

   if (num_rows != x_size)
   {
      hypre_error_w_msg(NALU_HYPRE_ERROR_GENERIC, "Error! incompatible x size!\n");
      return hypre_error_flag;
   }

   if (x_size > 0 && x_vecstride <= 0)
   {
      hypre_error_w_msg(NALU_HYPRE_ERROR_GENERIC, "Error! non-positive x vector stride!\n");
      return hypre_error_flag;
   }

   if (y_size > 0 && y_vecstride <= 0)
   {
      hypre_error_w_msg(NALU_HYPRE_ERROR_GENERIC, "Error! non-positive y vector stride!\n");
      return hypre_error_flag;
   }

   if (num_rows != y_size)
   {
      hypre_error_w_msg(NALU_HYPRE_ERROR_GENERIC, "Error! incompatible y size!\n");
      return hypre_error_flag;
   }
...
#if defined(NALU_HYPRE_USING_CUDA) || defined(NALU_HYPRE_USING_HIP)
   NALU_HYPRE_ExecutionPolicy exec = hypre_GetExecPolicy1( hypre_ParCSRMatrixMemoryLocation(par_A) );
