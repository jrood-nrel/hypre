                   ...
                   NALU_HYPRE_Int job)
{
   ...
   normr0 = nalu_hypre_sqrt(nalu_hypre_ddot(&n, v, &one, v, &one));
   ...
   while (i < kdim)
   {
      ...
      H[i + (i - 1)*kdim] = t = nalu_hypre_sqrt(nalu_hypre_ddot(&n, w, &one, w, &one));
      if (nalu_hypre_abs(t) > EPSILON)
      {
         t = 1.0 / t;
      ...
      NALU_HYPRE_Complex gam = nalu_hypre_sqrt(hii * hii + hii1 * hii1);

      if (nalu_hypre_cabs(gam) < EPSILON)
      {
         gam = EPSIMAC;
      }
      c[i - 1] = hii / gam;
