                                     ...
                                     NALU_HYPRE_Int *OUT_marker)
{
   ...
   e_num_sends = nalu_hypre_ParCSRCommPkgNumSends(extend_comm_pkg);


   index = nalu_hypre_max(nalu_hypre_ParCSRCommPkgSendMapStart(comm_pkg, num_sends),
                     nalu_hypre_ParCSRCommPkgSendMapStart(extend_comm_pkg, e_num_sends));

   int_buf_data = nalu_hypre_CTAlloc(NALU_HYPRE_Int,  index, NALU_HYPRE_MEMORY_HOST);
   ...
   for (i = begin; i < end; ++i)
   {
      int_buf_data[i - begin] =
         IN_marker[nalu_hypre_ParCSRCommPkgSendMapElmt(comm_pkg, i)];
   }
   ...
   begin = nalu_hypre_ParCSRCommPkgSendMapStart(extend_comm_pkg, 0);
   end = nalu_hypre_ParCSRCommPkgSendMapStart(extend_comm_pkg, e_num_sends);
#ifdef NALU_HYPRE_USING_OPENMP
   #pragma omp parallel for NALU_HYPRE_SMP_SCHEDULE
#endif
   for (i = begin; i < end; ++i)
   {
      int_buf_data[i - begin] =
         IN_marker[nalu_hypre_ParCSRCommPkgSendMapElmt(extend_comm_pkg, i)];
   }

   comm_handle = nalu_hypre_ParCSRCommHandleCreate( 11, extend_comm_pkg, int_buf_data,
                                               e_out_marker);
                                     ...
                                     NALU_HYPRE_BigInt *OUT_marker)
{
   ...
   int_buf_data = nalu_hypre_CTAlloc(NALU_HYPRE_BigInt,  index, NALU_HYPRE_MEMORY_HOST);
   ...
   for (i = begin; i < end; ++i)
   {
      int_buf_data[i - begin] = offset +
                                (NALU_HYPRE_BigInt) IN_marker[nalu_hypre_ParCSRCommPkgSendMapElmt(comm_pkg, i)];
   }

   comm_handle = nalu_hypre_ParCSRCommHandleCreate( 21, comm_pkg, int_buf_data,
                                               OUT_marker);
   ...
   {
      int_buf_data[i - begin] = offset +
                                (NALU_HYPRE_BigInt) IN_marker[nalu_hypre_ParCSRCommPkgSendMapElmt(extend_comm_pkg, i)];
   }

   comm_handle = nalu_hypre_ParCSRCommHandleCreate( 21, extend_comm_pkg, int_buf_data,
                                               e_out_marker);
...
NALU_HYPRE_Int nalu_hypre_ssort(NALU_HYPRE_BigInt *data, NALU_HYPRE_Int n)
{
   ...
   if (n > 0)
      for (i = n - 1; i > 0; i--)
      {
         si = nalu_hypre_index_of_minimum(data, i + 1);
         if (i != si)
         {
            nalu_hypre_swap_int(data, i, si);
            change = 1;
         }
      }
   return change;
...
/* Auxilary function for nalu_hypre_ssort */
...
static NALU_HYPRE_Int nalu_hypre_new_offd_nodes(NALU_HYPRE_BigInt **found, NALU_HYPRE_Int num_cols_A_offd,
                                      ...
                                      NALU_HYPRE_Int *CF_marker_offd)
{
   ...
   nalu_hypre_UnorderedBigIntMap col_map_offd_inverse;
   nalu_hypre_UnorderedBigIntMapCreate(&col_map_offd_inverse, 2 * num_cols_A_offd, 16 * nalu_hypre_NumThreads());

   #pragma omp parallel for NALU_HYPRE_SMP_SCHEDULE
   for (i = 0; i < num_cols_A_offd; i++)
   {
      nalu_hypre_UnorderedBigIntMapPutIfAbsent(&col_map_offd_inverse, col_map_offd[i], i);
   }
   ...
   nalu_hypre_UnorderedBigIntSetCreate(&set, size_offP, 16 * nalu_hypre_NumThreads());
      ...
      for (i = 0; i < num_cols_A_offd; i++)
      {
         if (CF_marker_offd[i] < 0)
         {
            for (j = A_ext_i[i]; j < A_ext_i[i + 1]; j++)
            {
               ...
               {
                  if (!nalu_hypre_UnorderedBigIntSetContains(&set, big_i1))
                  {
                     NALU_HYPRE_Int k = nalu_hypre_UnorderedBigIntMapGet(&col_map_offd_inverse, big_i1);
                     if (-1 == k)
                     {
                        nalu_hypre_UnorderedBigIntSetPut(&set, big_i1);
                     }
                     else
                     {
                        A_ext_j[j] = -k - 1;
   ...
   nalu_hypre_UnorderedBigIntMapDestroy(&col_map_offd_inverse);
   NALU_HYPRE_BigInt *tmp_found = nalu_hypre_UnorderedBigIntSetCopyToArray(&set, &newoff);
   ...
   nalu_hypre_UnorderedBigIntMap tmp_found_inverse;
   if (newoff > 0)
   {
      nalu_hypre_big_sort_and_create_inverse_map(tmp_found, newoff, &tmp_found, &tmp_found_inverse);
   }
   ...
   for (i = 0; i < num_cols_A_offd; i++)
   {
      if (CF_marker_offd[i] < 0)
      {
         for (kk = Sop_i[i]; kk < Sop_i[i + 1]; kk++)
         {
            ...
            {
               got_loc = nalu_hypre_UnorderedBigIntMapGet(&tmp_found_inverse, big_k1);
   ...
   if (newoff)
   {
      nalu_hypre_UnorderedBigIntMapDestroy(&tmp_found_inverse);
   }
   ...
   tmp_found = nalu_hypre_CTAlloc(NALU_HYPRE_BigInt, size_offP, NALU_HYPRE_MEMORY_HOST);
   ...
   for (i = 0; i < num_cols_A_offd; i++)
   {
      if (CF_marker_offd[i] < 0)
      {
         for (j = A_ext_i[i]; j < A_ext_i[i + 1]; j++)
         {
            ...
            if (big_i1 < col_1 || big_i1 >= col_n)
            {
               ifound = nalu_hypre_BigBinarySearch(col_map_offd, big_i1, num_cols_A_offd);
               if (ifound == -1)
               {
   ...
   if (newoff > 0)
   {
      nalu_hypre_BigQsort0(tmp_found, 0, newoff - 1);
      ifound = tmp_found[0];
      min = 1;
      for (i = 1; i < newoff; i++)
      {
   ...
   for (i = 0; i < num_cols_A_offd; i++)
   {
      if (CF_marker_offd[i] < 0)
      {
         for (kk = Sop_i[i]; kk < Sop_i[i + 1]; kk++)
         {
            ...
            if (big_k1 > -1 && (big_k1 < col_1 || big_k1 >= col_n))
            {
               got_loc = nalu_hypre_BigBinarySearch(tmp_found, big_k1, newoff);
               if (got_loc > -1)
               {
                  loc_col = got_loc + num_cols_A_offd;
               }
...
NALU_HYPRE_Int nalu_hypre_exchange_marker(nalu_hypre_ParCSRCommPkg *comm_pkg,
                                NALU_HYPRE_Int *IN_marker,
                                NALU_HYPRE_Int *OUT_marker)
{
   ...
   NALU_HYPRE_Int begin = nalu_hypre_ParCSRCommPkgSendMapStart(comm_pkg, 0);
   NALU_HYPRE_Int end = nalu_hypre_ParCSRCommPkgSendMapStart(comm_pkg, num_sends);
   NALU_HYPRE_Int *int_buf_data = nalu_hypre_CTAlloc(NALU_HYPRE_Int, end, NALU_HYPRE_MEMORY_HOST);
   ...
   nalu_hypre_ParCSRCommHandle *comm_handle = nalu_hypre_ParCSRCommHandleCreate( 11, comm_pkg, int_buf_data,
                                                                       OUT_marker);
...
NALU_HYPRE_Int nalu_hypre_exchange_interp_data(
   NALU_HYPRE_Int **CF_marker_offd,
...
{
#ifdef NALU_HYPRE_PROFILE
   nalu_hypre_profile_times[NALU_HYPRE_TIMER_ID_EXCHANGE_INTERP_DATA] -= nalu_hypre_MPI_Wtime();
#endif
    ...
    * A_offd and S_offd.
    *---------------------------------------------------------------------*/
   *CF_marker_offd = nalu_hypre_TAlloc(NALU_HYPRE_Int, num_cols_A_offd, NALU_HYPRE_MEMORY_HOST);
   nalu_hypre_exchange_marker(comm_pkg, CF_marker, *CF_marker_offd);

   nalu_hypre_ParCSRCommHandle *comm_handle_a_idx, *comm_handle_a_data;
   *A_ext         = nalu_hypre_ParCSRMatrixExtractBExt_Overlap(A, A, 1, &comm_handle_a_idx,
                                                          &comm_handle_a_data,
                                                          CF_marker, *CF_marker_offd, skip_fine_or_same_sign, skip_fine_or_same_sign);
   NALU_HYPRE_Int *A_ext_i        = nalu_hypre_CSRMatrixI(*A_ext);
   NALU_HYPRE_BigInt *A_ext_j        = nalu_hypre_CSRMatrixBigJ(*A_ext);
   NALU_HYPRE_Int  A_ext_rows     = nalu_hypre_CSRMatrixNumRows(*A_ext);

   nalu_hypre_ParCSRCommHandle *comm_handle_s_idx;
   *Sop           = nalu_hypre_ParCSRMatrixExtractBExt_Overlap(S, A, 0, &comm_handle_s_idx, NULL, CF_marker,
                                                          *CF_marker_offd, skip_fine_or_same_sign, 0);
   NALU_HYPRE_Int *Sop_i          = nalu_hypre_CSRMatrixI(*Sop);
   NALU_HYPRE_BigInt *Sop_j       = nalu_hypre_CSRMatrixBigJ(*Sop);
   NALU_HYPRE_Int  Soprows        = nalu_hypre_CSRMatrixNumRows(*Sop);

   NALU_HYPRE_Int *send_idx = (NALU_HYPRE_Int *)comm_handle_s_idx->send_data;
   nalu_hypre_ParCSRCommHandleDestroy(comm_handle_s_idx);
   nalu_hypre_TFree(send_idx, NALU_HYPRE_MEMORY_HOST);

   send_idx = (NALU_HYPRE_Int *)comm_handle_a_idx->send_data;
   nalu_hypre_ParCSRCommHandleDestroy(comm_handle_a_idx);
   nalu_hypre_TFree(send_idx, NALU_HYPRE_MEMORY_HOST);
   ...
   nalu_hypre_profile_times[NALU_HYPRE_TIMER_ID_EXCHANGE_INTERP_DATA] += nalu_hypre_MPI_Wtime();
#endif
   NALU_HYPRE_Int newoff = nalu_hypre_new_offd_nodes(&found, A_ext_rows, A_ext_i, A_ext_j,
   ...
   nalu_hypre_ParCSRFindExtendCommPkg(nalu_hypre_ParCSRMatrixComm(A),
   ...
   *CF_marker_offd = nalu_hypre_TReAlloc(*CF_marker_offd, NALU_HYPRE_Int, *full_off_procNodes,
                                    NALU_HYPRE_MEMORY_HOST);
   nalu_hypre_exchange_marker(*extend_comm_pkg, CF_marker, *CF_marker_offd + A_ext_rows);
   ...
   if (num_functions > 1)
   {
      ...
      nalu_hypre_alt_insert_new_nodes(comm_pkg, *extend_comm_pkg, dof_func,
   ...
   nalu_hypre_ParCSRCommHandleDestroy(comm_handle_a_data);
                               ...
                               NALU_HYPRE_Int *tmp_CF_marker_offd, NALU_HYPRE_BigInt *fine_to_coarse_offd)
{
   ...
   nalu_hypre_profile_times[NALU_HYPRE_TIMER_ID_RENUMBER_COLIDX] -= nalu_hypre_MPI_Wtime();
#endif
   NALU_HYPRE_Int     n_fine = nalu_hypre_CSRMatrixNumRows(P->diag);
   ...
   {
      P_marker = nalu_hypre_TAlloc(NALU_HYPRE_Int, full_off_procNodes, NALU_HYPRE_MEMORY_HOST);
   }
...
#endif
   {
      ...
      nalu_hypre_GetSimpleThreadPartition(&i_begin, &i_end, full_off_procNodes);
      ...
      nalu_hypre_prefix_sum(&local_num_cols_P_offd, &num_cols_P_offd, prefix_sum_workspace);
...
#endif
      {
         if (num_cols_P_offd)
         {
            col_map_offd_P = nalu_hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_P_offd, NALU_HYPRE_MEMORY_HOST);
         }
      }
#ifdef NALU_HYPRE_USING_OPENMP
      #pragma omp barrier
   ...
   nalu_hypre_UnorderedBigIntMap col_map_offd_P_inverse;
   nalu_hypre_big_sort_and_create_inverse_map(col_map_offd_P, num_cols_P_offd, &col_map_offd_P,
   ...
   for (i = 0; i < full_off_procNodes; i++)
   {
      P_marker[i] = nalu_hypre_UnorderedBigIntMapGet(&col_map_offd_P_inverse, fine_to_coarse_offd[i]);
   }

   if (num_cols_P_offd)
   {
      nalu_hypre_UnorderedBigIntMapDestroy(&col_map_offd_P_inverse);
   }
   ...
   if (num_cols_P_offd)
   {
      ...
      nalu_hypre_CSRMatrixNumCols(P->offd) = num_cols_P_offd;
   }
