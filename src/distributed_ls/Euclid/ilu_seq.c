...
void compute_scaling_private(NALU_HYPRE_Int row, NALU_HYPRE_Int len, NALU_HYPRE_Real *AVAL, Euclid_dh ctx)
{
  ...
  for (j=0; j<len; ++j) tmp = MAX( tmp, nalu_hypre_abs(AVAL[j]) );
  if (tmp) {
...
NALU_HYPRE_Real fixPivot_private(NALU_HYPRE_Int row, NALU_HYPRE_Int len, float *vals)
{
  ...
  for (i=0; i<len; ++i) {
    float tmp = nalu_hypre_abs(vals[i]);
    max = MAX(max, tmp);
  }
                 ...
                 NALU_HYPRE_Int *o2n_col, Euclid_dh ctx, bool debug)
{
  ...
  for (j=0; j<len; ++j) {
    ...
    if (nalu_hypre_abs(val) > thresh || col == localRow) {  /* sparsification */
      ++count;
...
#if 0
  if (nalu_hypre_abs(work[i]) <= pivotTol) {
...
void ilut_seq(Euclid_dh ctx)
{
  ...
  for (i=from; i<to; ++i) {
    ...
    while (count--) {
      val = work[col];
      if (col == i || nalu_hypre_abs(val) > droptol) {
        cval[idx] = col;   
        aval[idx++] = val;
        work[col] = 0.0;
      }
                     ...
                     REAL_DH *work, Euclid_dh ctx, bool debug)
{
  ...
  while (list[head] < localRow) {
    ...
    if (pc != 0.0) {
      ...
      if (nalu_hypre_abs(mult) > droptol) {
        work[row] = mult;
