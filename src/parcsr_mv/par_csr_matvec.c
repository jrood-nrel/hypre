...
NALU_HYPRE_Int
hypre_ParCSRMatrixMatvecOutOfPlaceHost( NALU_HYPRE_Complex       alpha,
...
#if defined(NALU_HYPRE_USING_PERSISTENT_COMM)
   hypre_VectorData(x_tmp) = (NALU_HYPRE_Complex *)
                             hypre_ParCSRCommHandleRecvDataBuffer(persistent_comm_handle);
   hypre_SeqVectorSetDataOwner(x_tmp, 0);
#endif

   hypre_SeqVectorInitialize_v2(x_tmp, NALU_HYPRE_MEMORY_HOST);
...
#if defined(NALU_HYPRE_USING_PERSISTENT_COMM)
   x_buf_data = (NALU_HYPRE_Complex *) hypre_ParCSRCommHandleSendDataBuffer(persistent_comm_handle);

#else
   x_buf_data = hypre_TAlloc(NALU_HYPRE_Complex,
...
#ifdef NALU_HYPRE_USING_PERSISTENT_COMM
   hypre_ParCSRPersistentCommHandleWait(persistent_comm_handle, NALU_HYPRE_MEMORY_HOST, x_tmp_data);
#else
...
NALU_HYPRE_Int
hypre_ParCSRMatrixMatvecOutOfPlace( NALU_HYPRE_Complex       alpha,
                                    hypre_ParCSRMatrix *A,
                                    hypre_ParVector    *x,
                                    NALU_HYPRE_Complex       beta,
                                    hypre_ParVector    *b,
...
NALU_HYPRE_Int
hypre_ParCSRMatrixMatvec( NALU_HYPRE_Complex       alpha,
                          hypre_ParCSRMatrix *A,
                          hypre_ParVector    *x,
                          NALU_HYPRE_Complex       beta,
                          hypre_ParVector    *y )
...
NALU_HYPRE_Int
hypre_ParCSRMatrixMatvecTHost( NALU_HYPRE_Complex       alpha,
                               hypre_ParCSRMatrix *A,
                               hypre_ParVector    *x,
                               NALU_HYPRE_Complex       beta,
                               hypre_ParVector    *y )
{
...
#if defined(NALU_HYPRE_USING_PERSISTENT_COMM)
   hypre_VectorData(y_tmp) = (NALU_HYPRE_Complex *)
                             hypre_ParCSRCommHandleSendDataBuffer(persistent_comm_handle);
   hypre_SeqVectorSetDataOwner(y_tmp, 0);
#endif

   hypre_SeqVectorInitialize_v2(y_tmp, NALU_HYPRE_MEMORY_HOST);
...
#if defined(NALU_HYPRE_USING_PERSISTENT_COMM)
   y_buf_data = (NALU_HYPRE_Complex *) hypre_ParCSRCommHandleRecvDataBuffer(persistent_comm_handle);

#else
   y_buf_data = hypre_TAlloc(NALU_HYPRE_Complex,
                             hypre_ParCSRCommPkgSendMapStart(comm_pkg, num_sends),
                             NALU_HYPRE_MEMORY_HOST);
#endif
   ...
   /* Non-blocking communication starts */
#if defined(NALU_HYPRE_USING_PERSISTENT_COMM)
   hypre_ParCSRPersistentCommHandleStart(persistent_comm_handle, NALU_HYPRE_MEMORY_HOST, y_tmp_data);
...
NALU_HYPRE_Int
hypre_ParCSRMatrixMatvecT( NALU_HYPRE_Complex       alpha,
                           hypre_ParCSRMatrix *A,
                           hypre_ParVector    *x,
                           NALU_HYPRE_Complex       beta,
                           hypre_ParVector    *y )
...
NALU_HYPRE_Int
hypre_ParCSRMatrixMatvec_FF( NALU_HYPRE_Complex       alpha,
                             hypre_ParCSRMatrix *A,
                             hypre_ParVector    *x,
                             NALU_HYPRE_Complex       beta,
                             hypre_ParVector    *y,
                             ...
                             NALU_HYPRE_Int           fpt )
{
   ...
   if (num_procs > 1)
   {
      ...
      num_sends = hypre_ParCSRCommPkgNumSends(comm_pkg);
      if (num_sends)
         x_buf_data = hypre_CTAlloc(NALU_HYPRE_Complex,  hypre_ParCSRCommPkgSendMapStart
   ...
   if (num_procs > 1)
   {
      ...
      if (num_sends)
         int_buf_data = hypre_CTAlloc(NALU_HYPRE_Int,  hypre_ParCSRCommPkgSendMapStart
                                      (comm_pkg,  num_sends), NALU_HYPRE_MEMORY_HOST);
