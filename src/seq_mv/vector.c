                               ...
                               NALU_HYPRE_Int     marker_val)
{
   ...
   {
      nalu_hypre_error_w_msg(NALU_HYPRE_ERROR_GENERIC, "Error: sizes of y and b do not match!\n");
      return nalu_hypre_error_flag;
   }

   if (nalu_hypre_VectorSize(x) < nalu_hypre_VectorSize(y))
   {
      nalu_hypre_error_w_msg(NALU_HYPRE_ERROR_GENERIC, "Error: x_size is smaller than y_size!\n");
      return nalu_hypre_error_flag;
   }
