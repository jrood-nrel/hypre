      ...
      for (i = 0; i <= n; i++)
         for (j = 0; j <= n; j++)
            for (k = 0; k <= n; k++)
            {
               ...
               NALU_HYPRE_SStructVectorSetValues(xcoord, part, index, var, &xyzval[0]);
               NALU_HYPRE_SStructVectorSetValues(ycoord, part, index, var, &xyzval[1]);
               NALU_HYPRE_SStructVectorSetValues(zcoord, part, index, var, &xyzval[2]);
            }
