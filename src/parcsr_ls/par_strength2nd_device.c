...
{
   NALU_HYPRE_Int           S_nr_local = hypre_ParCSRMatrixNumRows(S);
   hypre_CSRMatrix    *S_diag     = hypre_ParCSRMatrixDiag(S);
   hypre_CSRMatrix    *S_offd     = hypre_ParCSRMatrixOffd(S);
   NALU_HYPRE_Int           S_diag_nnz = hypre_CSRMatrixNumNonzeros(S_diag);
   NALU_HYPRE_Int           S_offd_nnz = hypre_CSRMatrixNumNonzeros(S_offd);
   ...
   */
   ...
   /* give S data arrays */
   hypre_CSRMatrixData(S_diag) = hypre_TAlloc(NALU_HYPRE_Complex, S_diag_nnz, NALU_HYPRE_MEMORY_DEVICE );
   hypreDevice_ComplexFilln( hypre_CSRMatrixData(S_diag),
                             S_diag_nnz,
                             1.0 );

   hypre_CSRMatrixData(S_offd) = hypre_TAlloc(NALU_HYPRE_Complex, S_offd_nnz, NALU_HYPRE_MEMORY_DEVICE );
   ...
   hypre_CSRMatrixInitialize_v2(Id, 0, NALU_HYPRE_MEMORY_DEVICE);
   ...
   hypre_ParCSRMatrixDNumNonzeros(S_CX) = (NALU_HYPRE_Real) hypre_ParCSRMatrixNumNonzeros(S_CX);
   */
   ...
   hypre_TFree(hypre_CSRMatrixData(hypre_ParCSRMatrixDiag(S2)), NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(hypre_CSRMatrixData(hypre_ParCSRMatrixOffd(S2)), NALU_HYPRE_MEMORY_DEVICE);
