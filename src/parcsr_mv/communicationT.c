...
( NALU_HYPRE_Int * rowmin, NALU_HYPRE_Int * rowmax, NALU_HYPRE_BigInt column, hypre_ParCSRMatrix * A )
...
)
{
   ...
   */
   ...
   recv_buf = hypre_CTAlloc(NALU_HYPRE_BigInt,  displs[num_procs], NALU_HYPRE_MEMORY_HOST);
   tmp = hypre_CTAlloc(NALU_HYPRE_BigInt,  local_info, NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_MPI_Allgatherv(tmp, local_info, NALU_HYPRE_MPI_BIG_INT,
   ...
   */
   ...
   send_map_elmts = hypre_CTAlloc(NALU_HYPRE_Int,  num_elmts, NALU_HYPRE_MEMORY_HOST);
   row_marker = hypre_CTAlloc(NALU_HYPRE_Int, num_rows_diag, NALU_HYPRE_MEMORY_HOST);
   ...
   /* Compute recv_vec_starts.
      ...
      hypre_MPI_ISend,hypre_MPI_IRecv of each size, separately between each pair of procs.
        ...
        implementations
   */
   send_buf = hypre_CTAlloc( NALU_HYPRE_Int,  3 * num_sends, NALU_HYPRE_MEMORY_HOST);
   all_num_sends3 = hypre_CTAlloc( NALU_HYPRE_Int,  num_procs, NALU_HYPRE_MEMORY_HOST);

   /* scatter-gather num_sends, to set up the size for the main comm. step */
   i = 3 * num_sends;
   hypre_MPI_Allgather( &i, 1, NALU_HYPRE_MPI_INT, all_num_sends3, 1, NALU_HYPRE_MPI_INT, comm );
   displs[0] = 0;
   for ( p = 0; p < num_procs; ++p )
   {
      displs[p + 1] = displs[p] + all_num_sends3[p];
   };
   recv_sz_buf = hypre_CTAlloc( NALU_HYPRE_Int,  displs[num_procs], NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_MPI_Allgatherv( send_buf, 3 * num_sends, NALU_HYPRE_MPI_INT,
   ...
   hypre_TFree(all_num_sends3, NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_TFree(recv_sz_buf, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(row_marker, NALU_HYPRE_MEMORY_HOST);
...
hypre_MatTCommPkgCreate ( hypre_ParCSRMatrix *A)
{
   ...
   NALU_HYPRE_Int         num_rows_diag  = hypre_CSRMatrixNumRows(hypre_ParCSRMatrixDiag(A));
   NALU_HYPRE_Int         num_cols_diag  = hypre_CSRMatrixNumCols(hypre_ParCSRMatrixDiag(A));
