...
static void ExchangeDiagEntries(MPI_Comm comm, Matrix *mat, NALU_HYPRE_Int reqlen, 
  NALU_HYPRE_Int *reqind, NALU_HYPRE_Real *diags, NALU_HYPRE_Int *num_requests, hypre_MPI_Request *requests,
  NALU_HYPRE_Int *replies_list)
{
    ...
    *num_requests = 0;
    ...
    for (i=0; i<reqlen; i=j) /* j is set below */
    {
        ...
        hypre_MPI_Isend(&reqind[i], j-i, NALU_HYPRE_MPI_INT, this_pe, DIAG_INDS_TAG,
            comm, &request);
...
static void ExchangeDiagEntriesServer(MPI_Comm comm, Matrix *mat, 
  NALU_HYPRE_Real *local_diags, NALU_HYPRE_Int num_requests, Mem *mem, hypre_MPI_Request *requests)
{
	...
	hypre_MPI_Get_count(&status, NALU_HYPRE_MPI_INT, &count);
        ...
        /*hypre_MPI_Recv(recvbuf, count, NALU_HYPRE_MPI_INT, hypre_MPI_ANY_SOURCE, */
        hypre_MPI_Recv(recvbuf, count, NALU_HYPRE_MPI_INT, source, 
...
DiagScale *DiagScaleCreate(Matrix *A, Numbering *numb)
{
    ...
    DiagScale *p = hypre_TAlloc(DiagScale, 1, NALU_HYPRE_MEMORY_HOST);

    /* Storage for local diagonal entries */
    p->local_diags = (NALU_HYPRE_Real *) 
        hypre_TAlloc(NALU_HYPRE_Real, (A->end_row - A->beg_row + 1) , NALU_HYPRE_MEMORY_HOST);
    ...
    if (len)
    {
        ind = hypre_TAlloc(NALU_HYPRE_Int, len , NALU_HYPRE_MEMORY_HOST);
        hypre_TMemcpy(ind,  &numb->local_to_global[numb->num_loc], NALU_HYPRE_Int, len, NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_HOST);

        /* buffer for receiving diagonal values from other processors */
        p->ext_diags = hypre_TAlloc(NALU_HYPRE_Real, len , NALU_HYPRE_MEMORY_HOST);
    }
    ...
    requests2 = NULL;
    if (num_replies)
        requests2 = hypre_TAlloc(hypre_MPI_Request, num_replies , NALU_HYPRE_MEMORY_HOST);
    ...
    temp = NULL;
    if (len)
        temp = hypre_TAlloc(NALU_HYPRE_Real, len , NALU_HYPRE_MEMORY_HOST);
    for (j=0; j<len; j++)
	temp[ind[j]-p->offset] = p->ext_diags[j];

    hypre_TFree(ind,NALU_HYPRE_MEMORY_HOST);
    hypre_TFree(p->ext_diags,NALU_HYPRE_MEMORY_HOST);
    p->ext_diags = temp;

    /* Wait for all sends */
    hypre_MPI_Waitall(num_replies, requests2, statuses);
    hypre_TFree(requests2,NALU_HYPRE_MEMORY_HOST);
    MemDestroy(mem);
...
{
    hypre_TFree(p->local_diags,NALU_HYPRE_MEMORY_HOST);
    hypre_TFree(p->ext_diags,NALU_HYPRE_MEMORY_HOST);
