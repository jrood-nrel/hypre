                                          ...
                                          NALU_HYPRE_Int           type)
{
   ...
   if (type == -1)
   {
      NALU_HYPRE_GPU_LAUNCH( nalu_hypre_ParCSRMatrixDropSmallEntriesDevice_getElmtTols < -1 >, gDim, bDim,
                        nalu_hypre_CSRMatrixNumRows(A_diag), tol, nalu_hypre_CSRMatrixI(A_diag),
                        nalu_hypre_CSRMatrixJ(A_diag), nalu_hypre_CSRMatrixData(A_diag), nalu_hypre_CSRMatrixI(A_offd),
                        nalu_hypre_CSRMatrixData(A_offd), elmt_tols_diag, elmt_tols_offd);
   }
   if (type == 1)
   {
      NALU_HYPRE_GPU_LAUNCH( nalu_hypre_ParCSRMatrixDropSmallEntriesDevice_getElmtTols<1>, gDim, bDim,
   ...
   tmp_j = nalu_hypre_TAlloc(NALU_HYPRE_Int, nalu_hypre_CSRMatrixNumNonzeros(A_offd), NALU_HYPRE_MEMORY_DEVICE);
   nalu_hypre_TMemcpy(tmp_j, nalu_hypre_CSRMatrixJ(A_offd), NALU_HYPRE_Int, nalu_hypre_CSRMatrixNumNonzeros(A_offd),
                 NALU_HYPRE_MEMORY_DEVICE, NALU_HYPRE_MEMORY_DEVICE);
   NALU_HYPRE_THRUST_CALL( sort,
                      tmp_j,
                      tmp_j + nalu_hypre_CSRMatrixNumNonzeros(A_offd) );
   tmp_end = NALU_HYPRE_THRUST_CALL( unique,
                                tmp_j,
                                tmp_j + nalu_hypre_CSRMatrixNumNonzeros(A_offd) );
   num_cols_A_offd_new = tmp_end - tmp_j;

   nalu_hypre_assert(num_cols_A_offd_new <= num_cols_A_offd);

   if (num_cols_A_offd_new < num_cols_A_offd)
   {
      nalu_hypre_CSRMatrixNumCols(A_offd) = num_cols_A_offd_new;

      NALU_HYPRE_Int *offd_mark = nalu_hypre_CTAlloc(NALU_HYPRE_Int, num_cols_A_offd, NALU_HYPRE_MEMORY_DEVICE);
      NALU_HYPRE_BigInt *col_map_offd_A_new = nalu_hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_A_offd_new,
                                                      NALU_HYPRE_MEMORY_DEVICE);
      ...
      NALU_HYPRE_THRUST_CALL( gather,
                         ...
                         nalu_hypre_CSRMatrixJ(A_offd) + nalu_hypre_CSRMatrixNumNonzeros(A_offd),
                         offd_mark,
                         nalu_hypre_CSRMatrixJ(A_offd) );
      NALU_HYPRE_THRUST_CALL( gather,
                         tmp_j,
      ...
      nalu_hypre_TFree(h_col_map_offd_A, NALU_HYPRE_MEMORY_HOST);

      nalu_hypre_ParCSRMatrixDeviceColMapOffd(A) = col_map_offd_A_new;
      nalu_hypre_ParCSRMatrixColMapOffd(A) = nalu_hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_A_offd_new,
                                                     NALU_HYPRE_MEMORY_HOST);
      nalu_hypre_TMemcpy(nalu_hypre_ParCSRMatrixColMapOffd(A), col_map_offd_A_new, NALU_HYPRE_BigInt,
                    num_cols_A_offd_new,
                    NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
   }
                             ...
                             nalu_hypre_ParCSRMatrix **C_ptr )
{
   ...
   if (num_cols_offd_C)
   {
      ...
      nalu_hypre_ParCSRMatrixColMapOffd(C) = nalu_hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_offd_C, NALU_HYPRE_MEMORY_HOST);
      nalu_hypre_TMemcpy(nalu_hypre_ParCSRMatrixColMapOffd(C), d_col_map_offd_C, NALU_HYPRE_BigInt, num_cols_offd_C,
                    NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
   }
