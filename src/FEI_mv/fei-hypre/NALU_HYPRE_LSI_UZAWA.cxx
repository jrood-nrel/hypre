...
typedef struct NALU_HYPRE_LSI_Uzawa_Struct
{
   void *precon;
}
NALU_HYPRE_LSI_UzawaStruct;
...
extern "C" int NALU_HYPRE_LSI_UzawaCreate(MPI_Comm mpi_comm, NALU_HYPRE_Solver *solver)
{
   (void) mpi_comm;
...
extern "C" int NALU_HYPRE_LSI_UzawaDestroy(NALU_HYPRE_Solver solver)
{
   int err=0;

   NALU_HYPRE_LSI_UzawaStruct *cprecon = (NALU_HYPRE_LSI_UzawaStruct *) solver;
   if ( cprecon == NULL ) err = 1;
   else
   {
      NALU_HYPRE_LSI_Uzawa *precon = (NALU_HYPRE_LSI_Uzawa *) cprecon->precon;
...
extern "C" int NALU_HYPRE_LSI_UzawaSetParams(NALU_HYPRE_Solver solver, char *params)
{
   int err=0;
...
extern "C" int NALU_HYPRE_LSI_UzawaSetMaxIterations(NALU_HYPRE_Solver solver, int iter)
{
   int err=0;
...
extern "C" int NALU_HYPRE_LSI_UzawaSetTolerance(NALU_HYPRE_Solver solver, double tol)
{
   int err=0;
...
extern "C" int NALU_HYPRE_LSI_UzawaGetNumIterations(NALU_HYPRE_Solver solver, int *iter)
{
   int err=0;
...
extern "C"
int NALU_HYPRE_LSI_UzawaSetup(NALU_HYPRE_Solver solver, NALU_HYPRE_ParCSRMatrix Amat,
...
extern "C"
int NALU_HYPRE_LSI_UzawaSolve(NALU_HYPRE_Solver solver, NALU_HYPRE_ParCSRMatrix Amat,
                         NALU_HYPRE_ParVector b, NALU_HYPRE_ParVector x)
{
   int err=0;
...
NALU_HYPRE_LSI_Uzawa::NALU_HYPRE_LSI_Uzawa(MPI_Comm comm)
{
   Amat_                     = NULL;
   A11mat_                   = NULL;
   A12mat_                   = NULL;
...
NALU_HYPRE_LSI_Uzawa::~NALU_HYPRE_LSI_Uzawa()
{
   Amat_    = NULL;
   mpiComm_ = 0;
   if ( procA22Sizes_ != NULL ) delete [] procA22Sizes_;
   if ( A11mat_       != NULL ) NALU_HYPRE_ParCSRMatrixDestroy(A11mat_);
   if ( A12mat_       != NULL ) NALU_HYPRE_ParCSRMatrixDestroy(A12mat_);
   if ( S22mat_       != NULL ) NALU_HYPRE_ParCSRMatrixDestroy(S22mat_);
}
...
int NALU_HYPRE_LSI_Uzawa::setParams(char *params)
{
   ...
   {
      printf("NALU_HYPRE_LSI_Uzawa::parameters not for me.\n");
      return 1;
   }
   ...
   else if ( !strcmp(param2, "outputLevel") )
   {
      ...
      if ( outputLevel_ > 0 )
         printf("NALU_HYPRE_LSI_Uzawa::outputLevel = %d.\n", outputLevel_);
   }
   else if ( !strcmp(param2, "modified") )
   {
      modifiedScheme_ = 1;
      if ( outputLevel_ > 0 ) printf("NALU_HYPRE_LSI_Uzawa::3 level scheme.\n");
   }
   else if ( !strcmp(param2, "A11Solver") )
   {
      ...
      if ( !strcmp(param3, "none") )
      {
         ...
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11 solver = cg\n");
      }
      ...
      else if ( !strcmp(param3, "gmres") )
      {
         ...
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11 solver = gmres\n");
      }
   }
   else if ( !strcmp(param2, "S22Solver") )
   {
      ...
      if ( !strcmp(param3, "none") )
      {
         ...
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22 solver = cg\n");
      }
      else if ( !strcmp(param3, "cg") )
      {
      ...
      else if ( !strcmp(param3, "gmres") )
      {
         ...
         if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22 solver = gmres\n");
      }
   }
   ...
   else if ( !strcmp(param2, "A11Tolerance") )
   {
      ...
      if (outputLevel_ > 0)
         printf("NALU_HYPRE_LSI_Uzawa::A11 tol = %e\n", A11Params_.Tol_);
   }
   else if ( !strcmp(param2, "S22Tolerance") )
   {
      ...
      if (outputLevel_ > 0)
         printf("NALU_HYPRE_LSI_Uzawa::S22 tol = %e\n", S22Params_.Tol_);
   }
   else if ( !strcmp(param2, "A11MaxIterations") )
   {
      ...
      if (outputLevel_ > 0)
         printf("NALU_HYPRE_LSI_Uzawa::A11 maxiter = %d\n", A11Params_.MaxIter_);
   }
   else if ( !strcmp(param2, "S22MaxIterations") )
   {
      ...
      if (outputLevel_ > 0)
         printf("NALU_HYPRE_LSI_Uzawa::S22 maxiter = %d\n", S22Params_.MaxIter_);
   }
   else if ( !strcmp(param2, "A11Precon") )
   {
      ...
      if ( !strcmp(param3, "diagonal") )
      {
         ...
         if (outputLevel_ > 0)
            printf("NALU_HYPRE_LSI_Uzawa::A11 precon = diagonal\n");
      }
      else if ( !strcmp(param3, "parasails") )
      {
         A11Params_.PrecondID_ = 2;
         if (outputLevel_ > 0)
            printf("NALU_HYPRE_LSI_Uzawa::A11 precon = parasails\n");
      }
      else if ( !strcmp(param3, "boomeramg") )
      {
         A11Params_.PrecondID_ = 3;
         if (outputLevel_ > 0)
            printf("NALU_HYPRE_LSI_Uzawa::A11 precon = boomeramg\n");
      }
      else if ( !strcmp(param3, "pilut") )
      {
         A11Params_.PrecondID_ = 4;
         if (outputLevel_ > 0)
            printf("NALU_HYPRE_LSI_Uzawa::A11 precon = pilut\n");
      }
      else if ( !strcmp(param3, "euclid") )
      {
         A11Params_.PrecondID_ = 5;
         if (outputLevel_ > 0)
            printf("NALU_HYPRE_LSI_Uzawa::A11 precon = euclid\n");
      }
      else if ( !strcmp(param3, "mli") )
      {
         A11Params_.PrecondID_ = 6;
         if (outputLevel_ > 0)
            printf("NALU_HYPRE_LSI_Uzawa::A11 precon = MLISA\n");
      }
   }
   else if ( !strcmp(param2, "S22Precon") )
   {
      ...
      if ( !strcmp(param3, "diagonal") )
      {
         ...
         if (outputLevel_ > 0)
            printf("NALU_HYPRE_LSI_Uzawa::S22 precon = diagonal\n");
      }
      else if ( !strcmp(param3, "parasails") )
      {
         S22Params_.PrecondID_ = 2;
         if (outputLevel_ > 0)
            printf("NALU_HYPRE_LSI_Uzawa::S22 precon = parasails\n");
      }
      else if ( !strcmp(param3, "boomeramg") )
      {
         S22Params_.PrecondID_ = 3;
         if (outputLevel_ > 0)
            printf("NALU_HYPRE_LSI_Uzawa::S22 precon = boomeramg\n");
      }
      else if ( !strcmp(param3, "pilut") )
      {
         S22Params_.PrecondID_ = 4;
         if (outputLevel_ > 0)
            printf("NALU_HYPRE_LSI_Uzawa::S22 precon = pilut\n");
      }
      else if ( !strcmp(param3, "euclid") )
      {
         S22Params_.PrecondID_ = 5;
         if (outputLevel_ > 0)
            printf("NALU_HYPRE_LSI_Uzawa::S22 precon = euclid\n");
      }
      else if ( !strcmp(param3, "mli") )
      {
         S22Params_.PrecondID_ = 6;
         if (outputLevel_ > 0)
            printf("NALU_HYPRE_LSI_Uzawa::S22 precon = MLISA\n");
      }
   }
   else if ( !strcmp(param2, "A11PreconPSNlevels") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconPSNLevels\n");
   }
   else if ( !strcmp(param2, "S22PreconPSNlevels") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconPSNLevels\n");
   }
   else if ( !strcmp(param2, "A11PreconPSThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconPSThresh\n");
   }
   else if ( !strcmp(param2, "S22PreconPSThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconPSThresh\n");
   }
   else if ( !strcmp(param2, "A11PreconPSFilter") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconPSFilter\n");
   }
   else if ( !strcmp(param2, "S22PreconPSFilter") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconPSFilter\n");
   }
   else if ( !strcmp(param2, "A11PreconAMGThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconAMGThresh\n");
   }
   else if ( !strcmp(param2, "S22PreconAMGThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconAMGThresh\n");
   }
   else if ( !strcmp(param2, "A11PreconAMGNumSweeps") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconAMGNSweeps\n");
   }
   else if ( !strcmp(param2, "S22PreconAMGNumSweeps") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconAMGNSweeps\n");
   }
   else if ( !strcmp(param2, "A11PreconAMGSystemSize") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconAMGSystemSize\n");
   }
   else if ( !strcmp(param2, "S22PreconAMGSystemSize") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconAMGSystemSize\n");
   }
   else if ( !strcmp(param2, "A11PreconEuclidNLevels") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconEuclidNLevels\n");
   }
   else if ( !strcmp(param2, "S22PreconEuclidNLevels") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconEuclidNLevels\n");
   }
   else if ( !strcmp(param2, "A11PreconEuclidThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconEuclidThresh\n");
   }
   else if ( !strcmp(param2, "S22PreconEuclidThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconEuclidThresh\n");
   }
   else if ( !strcmp(param2, "A11PreconPilutFillin") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconPilutFillin\n");
   }
   else if ( !strcmp(param2, "S22PreconPilutFillin") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconPilutFillin\n");
   }
   else if ( !strcmp(param2, "A11PreconPilutDropTol") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconPilutDropTol\n");
   }
   else if ( !strcmp(param2, "S22PreconPilutDropTol") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconPilutDropTol\n");
   }
   else if ( !strcmp(param2, "A11PreconMLIThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconMLIThresh\n");
   }
   else if ( !strcmp(param2, "S22PreconMLIThresh") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconMLIThresh\n");
   }
   else if ( !strcmp(param2, "A11PreconMLINumSweeps") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconMLINSweeps\n");
   }
   else if ( !strcmp(param2, "S22PreconMLINumSweeps") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconMLINSweeps\n");
   }
   else if ( !strcmp(param2, "A11PreconMLIPweight") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconMLIPweight\n");
   }
   else if ( !strcmp(param2, "S22PreconMLIPweight") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconMLIPweight\n");
   }
   else if ( !strcmp(param2, "A11PreconMLINodeDOF") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconMLINodeDOF\n");
   }
   else if ( !strcmp(param2, "S22PreconMLINodeDOF") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconMLINodeDOF\n");
   }
   else if ( !strcmp(param2, "A11PreconMLINullDim") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::A11PreconMLINullDim\n");
   }
   else if ( !strcmp(param2, "S22PreconMLINullDim") )
   {
      ...
      if (outputLevel_ > 0) printf("NALU_HYPRE_LSI_Uzawa::S22PreconMLINullDim\n");
   }
   else
   {
      printf("NALU_HYPRE_LSI_Uzawa:: string not recognized %s\n", params);
   }
   return 0;
}
...
int NALU_HYPRE_LSI_Uzawa::setMaxIterations(int niter)
{
...
int NALU_HYPRE_LSI_Uzawa::setTolerance(double tol)
{
   tolerance_ = tol;
   return 0;
}
...
int NALU_HYPRE_LSI_Uzawa::getNumIterations(int &iter)
{
   iter = numIterations_;
   return 0;
}
...
int NALU_HYPRE_LSI_Uzawa::setup(NALU_HYPRE_ParCSRMatrix A, NALU_HYPRE_ParVector x,
                           NALU_HYPRE_ParVector b)
{
   ...
   if ( mypid == 0 && outputLevel_ >= 1 )
      printf("%4d : NALU_HYPRE_LSI_Uzawa begins....\n", mypid);
   ...
   if ( mypid == 0 && outputLevel_ >= 1 )
      printf("%4d : NALU_HYPRE_LSI_Uzawa ends.\n", mypid);
   return 0;
}
...
int NALU_HYPRE_LSI_Uzawa::solve(NALU_HYPRE_ParVector b, NALU_HYPRE_ParVector x)
{
   ...
   NALU_HYPRE_ParCSRMatrixGetRowPartitioning( Amat_, &procNRows );
   startRow   = procNRows[mypid];
   endRow     = procNRows[mypid+1] - 1;
   localNRows = endRow - startRow + 1;
   ierr = NALU_HYPRE_IJVectorCreate(mpiComm_, startRow, endRow, &IJR);
   ...
   ierr = NALU_HYPRE_IJVectorCreate(mpiComm_, startRow, endRow, &IJF1);
   ...
   ierr = NALU_HYPRE_IJVectorCreate(mpiComm_, startRow, endRow, &IJF2);
   ...
   if ( maxIterations_ > 1 )
   {
      NALU_HYPRE_ParVectorCopy( b, r_csr );
      NALU_HYPRE_ParCSRMatrixMatvec( -1.0, Amat_, x, 1.0, r_csr );
   ...
   while ( numIterations_ < maxIterations_ && rnorm >= rnorm0 )
   {
      ...
      NALU_HYPRE_ParVectorCopy( f1_csr, t1_csr );
      NALU_HYPRE_ParCSRMatrixMatvec( -1.0, A11mat_, u1_csr, 1.0, t1_csr );
      NALU_HYPRE_ParCSRMatrixMatvec( -1.0, A12mat_, u2_csr, 1.0, t1_csr );

      if ( A11Params_.SolverID_ == 1 )
         NALU_HYPRE_ParCSRPCGSolve(A11Solver_, A11mat_, t1_csr, u1_csr);
      ...
      {
         NALU_HYPRE_ParVectorCopy( f2_csr, t2_csr );
         NALU_HYPRE_ParCSRMatrixMatvecT( 1.0, A12mat_, u1_csr, -1.0, t2_csr );

         if ( S22Params_.SolverID_ == 1 )
            NALU_HYPRE_ParCSRPCGSolve(S22Solver_, S22mat_, t2_csr, v2_csr);
         else if ( S22Params_.SolverID_ == 2 )
            NALU_HYPRE_ParCSRGMRESSolve(S22Solver_, S22mat_, t2_csr, v2_csr);
         else
         {
            NALU_HYPRE_ParVectorCopy( t2_csr, v2_csr );
         ...
         NALU_HYPRE_ParVectorCopy( f1_csr, t1_csr );
         NALU_HYPRE_ParCSRMatrixMatvec( -1.0, A11mat_, v1_csr, 1.0, t1_csr );
         NALU_HYPRE_ParCSRMatrixMatvec( -1.0, A12mat_, u2_csr, 1.0, t1_csr );
   ...
   NALU_HYPRE_IJVectorDestroy(IJR);
   NALU_HYPRE_IJVectorDestroy(IJF1);
   NALU_HYPRE_IJVectorDestroy(IJF2);
   NALU_HYPRE_IJVectorDestroy(IJU1);
   NALU_HYPRE_IJVectorDestroy(IJU2);
   NALU_HYPRE_IJVectorDestroy(IJT1);
   NALU_HYPRE_IJVectorDestroy(IJT2);
   NALU_HYPRE_IJVectorDestroy(IJV2);
   return 0;
}
...
int NALU_HYPRE_LSI_Uzawa::findA22BlockSize()
{
   ...
   for ( irow = endRow; irow >= startRow; irow-- )
   {
      ...
      NALU_HYPRE_ParCSRMatrixRestoreRow(Amat_,irow,&rowSize,&colInd,&colVal);
...
int NALU_HYPRE_LSI_Uzawa::buildBlockMatrices()
{
   int  ierr=0;
...
int NALU_HYPRE_LSI_Uzawa::buildA11A12Mat()
{
   ...
   NALU_HYPRE_IJMatrix     IJA11, IJA12;
   ...
   for ( irow = startRow; irow <= newEndRow ; irow++ )
   {
      ...
      NALU_HYPRE_ParCSRMatrixGetRow(Amat_,irow,&rowSize,&colInd,NULL);
      for ( jcol = 0;  jcol < rowSize;  jcol++ )
      {
      ...
      NALU_HYPRE_ParCSRMatrixRestoreRow(Amat_,irow,&rowSize,&colInd,NULL);
   }
   ...
   NALU_HYPRE_IJMatrixSetObjectType(IJA12, -1);
   NALU_HYPRE_IJMatrixDestroy(IJA12);
   delete [] A11MatSize;
   delete [] A12MatSize;
   delete [] A11ColInd;
   delete [] A11ColVal;
   ...
   if ( outputLevel_ >= 3 )
   {
      ...
      while ( ncnt < nprocs )
      {
         if ( mypid == ncnt )
         {
            ...
            {
               NALU_HYPRE_ParCSRMatrixGetRow(A11mat_,irow,&rowSize,&colInd,&colVal);
               for ( jcol = 0; jcol < rowSize; jcol++ )
                  if ( colVal[jcol] != 0.0 )
                     printf("%6d  %6d  %25.16e \n",irow+1,colInd[jcol]+1,
                            colVal[jcol]);
               NALU_HYPRE_ParCSRMatrixRestoreRow(A11mat_, irow, &rowSize,
                                            &colInd, &colVal);
            }
            printf("====================================================\n");
         }
   ...
   if ( outputLevel_ >= 3 )
   {
      ...
      while ( ncnt < nprocs )
      {
         if ( mypid == ncnt )
         {
            ...
            for (irow = A11StartRow;irow < A11StartRow+A11NRows;irow++)
            {
               NALU_HYPRE_ParCSRMatrixGetRow(A12mat_,irow,&rowSize,&colInd,&colVal);
               for ( jcol = 0; jcol < rowSize; jcol++ )
                  if ( colVal[jcol] != 0.0 )
                     printf("%6d  %6d  %25.16e \n",irow+1,colInd[jcol]+1,
                            colVal[jcol]);
               NALU_HYPRE_ParCSRMatrixRestoreRow(A12mat_, irow, &rowSize,
                                            &colInd, &colVal);
            }
            printf("====================================================\n");
         }
...
int NALU_HYPRE_LSI_Uzawa::buildS22Mat()
{
   ...
   if ( S22Scheme_ == 1 )
   {
      ...
      NALU_HYPRE_ParaSailsCreate(mpiComm_, &parasails);
      NALU_HYPRE_ParaSailsSetParams(parasails, 0.1, 1);
      NALU_HYPRE_ParaSailsSetFilter(parasails, 0.1);
      NALU_HYPRE_ParaSailsSetLogging(parasails, 1);
      NALU_HYPRE_ParaSailsSetup(parasails, A11mat_, NULL, NULL);
      NALU_HYPRE_ParaSailsBuildIJMatrix(parasails, &ainvA11);
   }
   else
   {
      //---------------------------------------------------------------
      ...
      NALU_HYPRE_ParCSRMatrixGetRowPartitioning( A11mat_, &procNRows );
      A11StartRow = procNRows[mypid];
      A11NRows    = procNRows[mypid+1] - A11StartRow;

      ierr  = NALU_HYPRE_IJMatrixCreate(mpiComm_,A11StartRow,
   ...
   NALU_HYPRE_IJMatrixDestroy(ainvA11);
   return 0;
}
...
int NALU_HYPRE_LSI_Uzawa::setupPrecon(NALU_HYPRE_Solver *precon,NALU_HYPRE_ParCSRMatrix Amat,
                                 NALU_HYPRE_Uzawa_PARAMS paramPtr)
{
   ...
   {
      case 2 :
          NALU_HYPRE_ParCSRParaSailsCreate( mpiComm_, precon );
          if (paramPtr.SolverID_ == 0) NALU_HYPRE_ParCSRParaSailsSetSym(*precon,1);
          else                         NALU_HYPRE_ParCSRParaSailsSetSym(*precon,0);
          NALU_HYPRE_ParCSRParaSailsSetParams(*precon, paramPtr.PSThresh_,
                                         paramPtr.PSNLevels_);
          NALU_HYPRE_ParCSRParaSailsSetFilter(*precon, paramPtr.PSFilter_);
          break;
      case 3 :
          ...
          break;
      case 4 :
          NALU_HYPRE_ParCSRPilutCreate( mpiComm_, precon );
          NALU_HYPRE_ParCSRPilutSetMaxIter( *precon, 1 );
          NALU_HYPRE_ParCSRPilutSetFactorRowSize(*precon,paramPtr.PilutFillin_);
          NALU_HYPRE_ParCSRPilutSetDropTolerance(*precon,paramPtr.PilutDropTol_);
          break;
      case 5 :
      ...
      case 6 :
#ifdef HAVE_MLI
          NALU_HYPRE_LSI_MLICreate(mpiComm_, precon);
...
int NALU_HYPRE_LSI_Uzawa::setupSolver(NALU_HYPRE_Solver *solver,NALU_HYPRE_ParCSRMatrix Amat,
                          NALU_HYPRE_ParVector fvec, NALU_HYPRE_ParVector xvec,
                          NALU_HYPRE_Solver precon, NALU_HYPRE_Uzawa_PARAMS paramPtr)
{
   //------------------------------------------------------------------
   ...
   {
      case 1 :
          NALU_HYPRE_ParCSRPCGCreate(mpiComm_, solver);
          NALU_HYPRE_ParCSRPCGSetMaxIter(*solver, paramPtr.MaxIter_ );
          NALU_HYPRE_ParCSRPCGSetTol(*solver, paramPtr.Tol_);
          ...
          NALU_HYPRE_ParCSRPCGSetup(*solver, Amat, fvec, xvec);
          break;

      case 2 :
          NALU_HYPRE_ParCSRGMRESCreate(mpiComm_, solver);
          NALU_HYPRE_ParCSRGMRESSetMaxIter(*solver, paramPtr.MaxIter_ );
          NALU_HYPRE_ParCSRGMRESSetTol(*solver, paramPtr.Tol_);
          ...
          NALU_HYPRE_ParCSRGMRESSetup(*solver, Amat, fvec, xvec);
          break;
   }
   return 0;
}
