   ...
   for (i = 0; i < max_iter; i++)
   {
      ...
      if (num_levels > 1)
      {
         ...
         for (l = 1; l <= (num_levels - 2); l++)
         {
            ...
            if (hypre_StructGridDataLocation(hypre_StructVectorGrid(r_l[l])) == NALU_HYPRE_MEMORY_HOST)
            {
               hypre_SetDeviceOff();
            }
#endif
         ...
         {
#if 0 //defined(NALU_HYPRE_USING_CUDA) || defined(NALU_HYPRE_USING_HIP)
            if (hypre_StructGridDataLocation(hypre_StructVectorGrid(e_l[l])) == NALU_HYPRE_MEMORY_DEVICE)
            {
               hypre_SetDeviceOn();
            }
#endif
...
#if 0 //defined(NALU_HYPRE_USING_CUDA) || defined(NALU_HYPRE_USING_HIP)
         if (hypre_StructGridDataLocation(hypre_StructVectorGrid(e_l[0])) == NALU_HYPRE_MEMORY_DEVICE)
         {
            hypre_SetDeviceOn();
         }
#endif
