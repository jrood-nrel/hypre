      ...
      for (ly = 0; ly < num_grids[1]; ly++)
      {
         for (lx = 0; lx < num_grids[0]; lx++)
         {
            ...
            if (lx < (num_grids[0] - 1))
            {
               ...
               nalu_hypre_SetIndex3(stridePR, 1, nalu_hypre_pow2(ly), nalu_hypre_pow2(lz));
         ...
         if (ly < (num_grids[1] - 1))
         {
            ...
            A_a[ci] = nalu_hypre_SparseMSGCreateRAPOp(RTy_a[ly], A_a[fi], Py_a[ly],
                                                 grid_a[ci], 1);
            nalu_hypre_StructMatrixInitialize(A_a[ci]);
            nalu_hypre_SetIndex3(stridePR, 1, 1, nalu_hypre_pow2(lz));
   ...
   for (ly = 0; ly < (num_grids[1] - 1); ly++)
   {
      ...
      for (lz = 0; lz < num_grids[2]; lz++)
      {
         for (lx = 0; lx < num_grids[0]; lx++)
         {
            ...
            nalu_hypre_SetIndex3(stridePR, nalu_hypre_pow2(lx), 1, nalu_hypre_pow2(lz));
   ...
   for (lz = 0; lz < (num_grids[2] - 1); lz++)
   {
      ...
      for (ly = 0; ly < num_grids[1]; ly++)
      {
         for (lx = 0; lx < num_grids[0]; lx++)
         {
            ...
            nalu_hypre_SetIndex3(stridePR, nalu_hypre_pow2(lx), nalu_hypre_pow2(ly), 1);
