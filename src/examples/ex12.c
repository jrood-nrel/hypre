...
/*
   ...
   Compile with: make ex12 (may need to edit NALU_HYPRE_DIR in Makefile)
...
int main (int argc, char *argv[])
{
   ...
   int precond_id  = 1;
   int object_type = NALU_HYPRE_STRUCT;
      ...
      the piece of the grid that it owns.  */
   {
      ...
      NALU_HYPRE_SStructGridCreate(MPI_COMM_WORLD, 2, nparts, &grid);
   ...
   /* 4. Set up a SStruct Matrix */
   {
      ...
      /* Set the object type (by default NALU_HYPRE_SSTRUCT). This determines the
         ...
         NALU_HYPRE_STRUCT, and for BoomerAMG we need NALU_HYPRE_PARCSR (set above). */
      ...
      if (myid == 0)
      {
         ...
         NALU_HYPRE_SStructMatrixSetBoxValues(A, part, ilower, iupper, var, nentries,
      ...
      if (myid == 0)
      {
         ...
         }
         {
            ...
            NALU_HYPRE_SStructMatrixSetBoxValues(A, part, ilower, iupper, var, 1,
                                            stencil_indices, values);
         }
         {
   ...
   if (precond_id == 1) /* PFMG */
   {
      ...
      NALU_HYPRE_StructPFMGSetNumPreRelax(precond, 2);
      NALU_HYPRE_StructPFMGSetNumPostRelax(precond, 2);
      /* non-Galerkin coarse grid (more efficient for this problem) */
      NALU_HYPRE_StructPFMGSetRAPType(precond, 1);
      /* R/B Gauss-Seidel */
      NALU_HYPRE_StructPFMGSetRelaxType(precond, 2);
      /* skip relaxation on some levels (more efficient for this problem) */
      NALU_HYPRE_StructPFMGSetSkipRelax(precond, 1);


      /* Set preconditioner and solve */
      NALU_HYPRE_StructPCGSetPrecond(solver, NALU_HYPRE_StructPFMGSolve,
                                NALU_HYPRE_StructPFMGSetup, precond);
   ...
   else if (precond_id == 2) /* BoomerAMG */
   {
      ...
      /* Set PCG parameters */
      NALU_HYPRE_ParCSRPCGSetTol(solver, 1.0e-06);
      NALU_HYPRE_ParCSRPCGSetPrintLevel(solver, 2);
      NALU_HYPRE_ParCSRPCGSetMaxIter(solver, 50);
      ...
      NALU_HYPRE_BoomerAMGSetTol(precond, 0.0);
      ...
      NALU_HYPRE_ParCSRPCGSetPrecond(solver, NALU_HYPRE_BoomerAMGSolve,
                                NALU_HYPRE_BoomerAMGSetup, precond);
      NALU_HYPRE_ParCSRPCGSetup(solver, parA, parb, parx);
      NALU_HYPRE_ParCSRPCGSolve(solver, parA, parb, parx);
