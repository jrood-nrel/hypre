...
void testEigen()
{
   ...
   matrix = hypre_TAlloc(double,  mDim * mDim , NALU_HYPRE_MEMORY_HOST);
   fp = fopen("test.m", "r");
   if ( fp == NULL )
   {
   ...
   evectors = hypre_TAlloc(double,  mDim * mDim , NALU_HYPRE_MEMORY_HOST);
   evalues  = hypre_TAlloc(double,  mDim , NALU_HYPRE_MEMORY_HOST);
   daux1    = hypre_TAlloc(double,  mDim , NALU_HYPRE_MEMORY_HOST);
   daux2    = hypre_TAlloc(double,  mDim , NALU_HYPRE_MEMORY_HOST);
   ...
   hypre_TFree(evectors, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(evalues, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(daux1, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(daux2, NALU_HYPRE_MEMORY_HOST);
}
...
void testMergeSort()
{
   ...
   listLengs = hypre_TAlloc(int,  nlist , NALU_HYPRE_MEMORY_HOST);
   list  = hypre_TAlloc(int*,  nlist , NALU_HYPRE_MEMORY_HOST);
   list2 = hypre_TAlloc(int*,  nlist , NALU_HYPRE_MEMORY_HOST);
   for ( i = 0; i < nlist; i++ )
   {
      list[i] = hypre_TAlloc(int,  maxLeng , NALU_HYPRE_MEMORY_HOST);
      list2[i] = hypre_TAlloc(int,  maxLeng , NALU_HYPRE_MEMORY_HOST);
   }
   listLengs[0] = 5;
   list[0][0] = 4;
   list[0][1] = 5;
...
/*
   ...
   checkList = hypre_TAlloc(int,  nlist * maxLeng , NALU_HYPRE_MEMORY_HOST);
   for ( i = 0; i < nlist; i++ )
   ...
   for ( i = 0; i < nlist; i++ )
   {
      hypre_TFree(list[i], NALU_HYPRE_MEMORY_HOST);
      hypre_TFree(list2[i], NALU_HYPRE_MEMORY_HOST);
   }
   hypre_TFree(checkList , NALU_HYPRE_MEMORY_HOST);
*/
   hypre_TFree(listLengs, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(list, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(list2, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(newList, NALU_HYPRE_MEMORY_HOST);
}
