                             ...
                             hypre_ParCSRMatrix   **S_ptr)
{
   ...
   S_diag_i = hypre_TAlloc(NALU_HYPRE_Int, num_variables + 1, memory_location);
   S_offd_i = hypre_TAlloc(NALU_HYPRE_Int, num_variables + 1, memory_location);
   S_temp_diag_j = hypre_TAlloc(NALU_HYPRE_Int, num_nonzeros_diag, NALU_HYPRE_MEMORY_DEVICE);
   S_temp_offd_j = hypre_TAlloc(NALU_HYPRE_Int, num_nonzeros_offd, NALU_HYPRE_MEMORY_DEVICE);

   if (num_functions > 1)
   {
      dof_func_offd_dev = hypre_TAlloc(NALU_HYPRE_Int, num_cols_offd, NALU_HYPRE_MEMORY_DEVICE);
   }
   ...
   if (num_functions > 1)
   {
      NALU_HYPRE_Int *int_buf_data = hypre_TAlloc(NALU_HYPRE_Int, hypre_ParCSRCommPkgSendMapStart(comm_pkg,
   ...
   if (abs_soc)
   {
      NALU_HYPRE_GPU_LAUNCH( hypre_BoomerAMGCreateSabs_rowcount, gDim, bDim,
   ...
   {
      NALU_HYPRE_GPU_LAUNCH( hypre_BoomerAMGCreateS_rowcount, gDim, bDim,
                        num_variables, max_row_sum, strength_threshold,
                        A_diag_data, A_diag_i, A_diag_j,
                        A_offd_data, A_offd_i, A_offd_j,
                        S_temp_diag_j, S_temp_offd_j,
   ...
   hypre_Memset(S_diag_i + num_variables, 0, sizeof(NALU_HYPRE_Int), NALU_HYPRE_MEMORY_DEVICE);
   hypre_Memset(S_offd_i + num_variables, 0, sizeof(NALU_HYPRE_Int), NALU_HYPRE_MEMORY_DEVICE);
   ...
   hypre_TMemcpy(&S_num_nonzeros_diag, &S_diag_i[num_variables], NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST,
                 memory_location);
   hypre_TMemcpy(&S_num_nonzeros_offd, &S_offd_i[num_variables], NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST,
                 memory_location);

   S_diag_j = hypre_TAlloc(NALU_HYPRE_Int, S_num_nonzeros_diag, memory_location);
   S_offd_j = hypre_TAlloc(NALU_HYPRE_Int, S_num_nonzeros_offd, memory_location);
   ...
   hypre_ParCSRMatrixColMapOffd(S) = hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_offd, NALU_HYPRE_MEMORY_HOST);
   ...
   *S_ptr = S;
   ...
   hypre_TFree(dof_func_offd_dev, NALU_HYPRE_MEMORY_DEVICE);
   /*
   hypre_TFree(S_temp_diag_j,     NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(S_temp_offd_j,     NALU_HYPRE_MEMORY_DEVICE);
   */
                                                 ...
                                                 NALU_HYPRE_Int  *jS_offd )
{
   ...
   /* compute row of S */
   NALU_HYPRE_Int all_weak = max_row_sum < 1.0 && fabs(row_sum) > fabs(diag) * max_row_sum;
                                   ...
                                   hypre_ParCSRMatrix *S)
{
   if (!hypre_ParCSRMatrixSocDiagJ(S))
   {
      ...
      NALU_HYPRE_Int nnz_diag = hypre_CSRMatrixNumNonzeros(A_diag);
      NALU_HYPRE_Int *soc_diag = hypre_TAlloc(NALU_HYPRE_Int, nnz_diag, NALU_HYPRE_MEMORY_DEVICE);
   ...
   if (!hypre_ParCSRMatrixSocOffdJ(S))
   {
      ...
      NALU_HYPRE_Int nnz_offd = hypre_CSRMatrixNumNonzeros(A_offd);
      NALU_HYPRE_Int *soc_offd = hypre_TAlloc(NALU_HYPRE_Int, nnz_offd, NALU_HYPRE_MEMORY_DEVICE);
                                                    ...
                                                    NALU_HYPRE_Int  *jS_offd )
{
   ...
   for (NALU_HYPRE_Int i = p_diag + lane; i < q_diag; i += NALU_HYPRE_WARP_SIZE)
   {
      ...
      if ( num_functions == 1 || row == col ||
           read_only_load(&dof_func[row]) == read_only_load(&dof_func[col]) )
      {
         const NALU_HYPRE_Real v = hypre_cabs( read_only_load(&A_diag_data[i]) );
         row_sum += v;
         if (row == col)
         {
            diag = v;
   ...
   for (NALU_HYPRE_Int i = p_offd + lane; i < q_offd; i += NALU_HYPRE_WARP_SIZE)
   {
      if ( num_functions == 1 ||
           read_only_load(&dof_func[row]) == read_only_load(&dof_func_offd[read_only_load(&A_offd_j[i])]) )
      {
         const NALU_HYPRE_Real v = hypre_cabs( read_only_load(&A_offd_data[i]) );
         row_sum += v;
         row_scale = hypre_max(row_scale, v);
      }
   }
   ...
   /* compute row of S */
   NALU_HYPRE_Int all_weak = max_row_sum < 1.0 && fabs(row_sum) < fabs(diag) * (2.0 - max_row_sum);
...
hypre_BoomerAMGCorrectCFMarkerDevice(hypre_IntArray *CF_marker, hypre_IntArray *new_CF_marker)
{
   ...
   NALU_HYPRE_Int n_fine     = hypre_IntArraySize(CF_marker);
   NALU_HYPRE_Int n_coarse   = hypre_IntArraySize(new_CF_marker);

   NALU_HYPRE_Int *indices   = hypre_CTAlloc(NALU_HYPRE_Int, n_coarse, NALU_HYPRE_MEMORY_DEVICE);
   NALU_HYPRE_Int *CF_C      = hypre_CTAlloc(NALU_HYPRE_Int, n_coarse, NALU_HYPRE_MEMORY_DEVICE);
   ...
   hypre_TFree(indices, NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(CF_C, NALU_HYPRE_MEMORY_DEVICE);
