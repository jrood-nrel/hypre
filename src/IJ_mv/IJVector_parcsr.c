...
NALU_HYPRE_Int
hypre_IJVectorInitializePar_v2(hypre_IJVector *vector, NALU_HYPRE_MemoryLocation memory_location)
{
   MPI_Comm            comm         = hypre_IJVectorComm(vector);
   ...
   NALU_HYPRE_Int           print_level  = hypre_IJVectorPrintLevel(vector);
   NALU_HYPRE_Int           num_vectors  = hypre_IJVectorNumComponents(vector);

   NALU_HYPRE_BigInt       *partitioning = hypre_ParVectorPartitioning(par_vector);
   ...
   hypre_VectorSize(local_vector) = (NALU_HYPRE_Int)(partitioning[1] - partitioning[0]);
...
hypre_IJVectorZeroValuesPar(hypre_IJVector *vector)
{
   ...
   hypre_assert(hypre_VectorSize(local_vector) == (NALU_HYPRE_Int)(vec_stop - vec_start));
                              ...
                              NALU_HYPRE_Int       component)
{
   NALU_HYPRE_Int        print_level = hypre_IJVectorPrintLevel(vector);
   hypre_ParVector *par_vector  = (hypre_ParVector*) hypre_IJVectorObject(vector);
   NALU_HYPRE_Int        num_vectors = hypre_ParVectorNumVectors(par_vector);
                           ...
                           const NALU_HYPRE_Complex  *values)
{
   ...
   NALU_HYPRE_BigInt *IJpartitioning = hypre_IJVectorPartitioning(vector);
   hypre_ParVector *par_vector = (hypre_ParVector*) hypre_IJVectorObject(vector);
   MPI_Comm comm = hypre_IJVectorComm(vector);
   NALU_HYPRE_Int component;
   hypre_Vector *local_vector;
                             ...
                             const NALU_HYPRE_Complex  *values)
{
   ...
   if (indices)
   {
      ...
      NALU_HYPRE_BigInt *off_proc_i = hypre_AuxParVectorOffProcI(aux_vector);
      NALU_HYPRE_Complex *off_proc_data = hypre_AuxParVectorOffProcData(aux_vector);
      NALU_HYPRE_Int k;
      ...
      for (j = 0; j < num_values; j++)
      {
         ...
         if (i < vec_start || i > vec_stop)
         {
            ...
            else if (current_num_elmts + 1 > max_off_proc_elmts)
            {
               ...
               off_proc_i = hypre_TReAlloc(off_proc_i, NALU_HYPRE_BigInt, max_off_proc_elmts, NALU_HYPRE_MEMORY_HOST);
...
hypre_IJVectorAssemblePar(hypre_IJVector *vector)
{
   ...
   if (aux_vector)
   {
      ...
      hypre_MPI_Allreduce(&current_num_elmts, &off_proc_elmts, 1, NALU_HYPRE_MPI_INT,
                          hypre_MPI_SUM, comm);
      if (off_proc_elmts)
      {
                           ...
                           NALU_HYPRE_Complex       *values)
{
   ...
   NALU_HYPRE_BigInt     jmin           = hypre_IJVectorGlobalFirstRow(vector);
   hypre_ParVector *par_vector     = (hypre_ParVector*) hypre_IJVectorObject(vector);
   NALU_HYPRE_Int        print_level    = hypre_IJVectorPrintLevel(vector);
   ...
   hypre_ParVectorGetValues2(par_vector, num_values, (NALU_HYPRE_BigInt *) indices, jmin, values);
                                      ...
                                      NALU_HYPRE_Complex        *off_proc_data)
{
   ...
   {
      NALU_HYPRE_BigInt  *off_proc_i_h    = hypre_TAlloc(NALU_HYPRE_BigInt,  current_num_elmts, NALU_HYPRE_MEMORY_HOST);
      NALU_HYPRE_Complex *off_proc_data_h = hypre_TAlloc(NALU_HYPRE_Complex, current_num_elmts, NALU_HYPRE_MEMORY_HOST);

      hypre_TMemcpy(off_proc_i_h,    off_proc_i,    NALU_HYPRE_BigInt,  current_num_elmts, NALU_HYPRE_MEMORY_HOST,
   ...
   a_proc_id = hypre_CTAlloc(NALU_HYPRE_Int,  current_num_elmts, NALU_HYPRE_MEMORY_HOST);
   orig_order =  hypre_CTAlloc(NALU_HYPRE_Int,  current_num_elmts, NALU_HYPRE_MEMORY_HOST);
   real_proc_id = hypre_CTAlloc(NALU_HYPRE_Int,  current_num_elmts, NALU_HYPRE_MEMORY_HOST);
   row_list =   hypre_CTAlloc(NALU_HYPRE_BigInt,  current_num_elmts, NALU_HYPRE_MEMORY_HOST);
   ...
   us_real_proc_id =  hypre_CTAlloc(NALU_HYPRE_Int,  current_num_elmts, NALU_HYPRE_MEMORY_HOST);
   for (i = 0; i < current_num_elmts; i++)
   {
   ...
   for (i = 0; i < num_recvs; i++)
   {
      ...
      hypre_TMemcpy( &row_count,  recv_data_ptr, NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_HOST);
      ...
      for (j = 0; j < row_count; j++) /* for each row: unpack info */
      {
         ...
         /* value */
         hypre_TMemcpy( &value,  recv_data_ptr, NALU_HYPRE_Complex, 1, NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_HOST);
         recv_data_ptr = (void *) ((char *)recv_data_ptr + obj_size_bytes);
         indx++;
