                          ...
                          hypre_ParCSRMatrix  *B )
{
   ...
   if (num_procs > 1)
   {
      ...
      t1 = hypre_MPI_Wtime();
#endif
      in_range<NALU_HYPRE_Int> pred(0, hypre_ParCSRMatrixNumCols(B) - 1);
#if defined(NALU_HYPRE_USING_SYCL)
      ...
      NALU_HYPRE_Int nnz_C_diag = NALU_HYPRE_THRUST_CALL( count_if,
                                                ...
                                                pred );
#endif
      NALU_HYPRE_Int nnz_C_offd = hypre_CSRMatrixNumNonzeros(Cbar) - nnz_C_diag;

      C_diag = hypre_CSRMatrixCreate(hypre_ParCSRMatrixNumRows(A), hypre_ParCSRMatrixNumCols(B),
                                     nnz_C_diag);
      hypre_CSRMatrixInitialize_v2(C_diag, 0, NALU_HYPRE_MEMORY_DEVICE);
      NALU_HYPRE_Int     *C_diag_ii = hypre_TAlloc(NALU_HYPRE_Int, nnz_C_diag, NALU_HYPRE_MEMORY_DEVICE);
      NALU_HYPRE_Int     *C_diag_j = hypre_CSRMatrixJ(C_diag);
      NALU_HYPRE_Complex *C_diag_a = hypre_CSRMatrixData(C_diag);

      NALU_HYPRE_Int *Cbar_ii = hypreDevice_CsrRowPtrsToIndices(hypre_ParCSRMatrixNumRows(A),
      ...
      hypre_TFree(C_diag_ii, NALU_HYPRE_MEMORY_DEVICE);
      ...
      NALU_HYPRE_Int     *C_offd_ii = hypre_TAlloc(NALU_HYPRE_Int, nnz_C_offd, NALU_HYPRE_MEMORY_DEVICE);
      NALU_HYPRE_Int     *C_offd_j = hypre_CSRMatrixJ(C_offd);
      NALU_HYPRE_Complex *C_offd_a = hypre_CSRMatrixData(C_offd);
#if defined(NALU_HYPRE_USING_SYCL)
      ...
      hypre_TFree(C_offd_ii, NALU_HYPRE_MEMORY_DEVICE);
      ...
      hypre_TFree(Cbar_ii, NALU_HYPRE_MEMORY_DEVICE);
   ...
   if (num_cols_offd_C)
   {
      ...
      hypre_TMemcpy(hypre_ParCSRMatrixColMapOffd(C), col_map_offd_C, NALU_HYPRE_BigInt, num_cols_offd_C,
                             ...
                             NALU_HYPRE_Int            keep_transpose)
{
   ...
   if (num_procs > 1)
   {
      ...
      t1 = hypre_MPI_Wtime();
#endif
      hypre_TMemcpy(&local_nnz_Cbar, hypre_CSRMatrixI(Cbar) + hypre_ParCSRMatrixNumCols(A), NALU_HYPRE_Int, 1,
                    NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
      ...
      hypre_CSRMatrixMemoryLocation(Cint) = NALU_HYPRE_MEMORY_DEVICE;
      ...
      hypre_CSRMatrixBigJ(Cint) = hypre_TAlloc(NALU_HYPRE_BigInt, hypre_CSRMatrixNumNonzeros(Cint),
                                               NALU_HYPRE_MEMORY_DEVICE);

      RAP_functor<1, NALU_HYPRE_BigInt> func1( hypre_ParCSRMatrixNumCols(B),
      ...
      hypre_TFree(hypre_CSRMatrixBigJ(Cint), NALU_HYPRE_MEMORY_DEVICE);
      hypre_TFree(Cint, NALU_HYPRE_MEMORY_HOST);

      hypre_TMemcpy(hypre_CSRMatrixI(Cbar) + hypre_ParCSRMatrixNumCols(A), &local_nnz_Cbar, NALU_HYPRE_Int, 1,
                               ...
                               NALU_HYPRE_Int           keep_transpose )
{
   ...
   if (num_procs > 1)
   {
      ...
      hypre_TMemcpy(&local_nnz_Cbar, hypre_CSRMatrixI(Cbar) + hypre_ParCSRMatrixNumCols(R), NALU_HYPRE_Int, 1,
                    NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
      ...
      RAP_functor<1, NALU_HYPRE_BigInt> func1(hypre_ParCSRMatrixNumCols(P), hypre_ParCSRMatrixFirstColDiag(P),
                                         col_map_offd);
#if defined(NALU_HYPRE_USING_SYCL)
      NALU_HYPRE_ONEDPL_CALL( std::transform,
                         hypre_CSRMatrixJ(Cbar) + local_nnz_Cbar,
      ...
      hypre_TMemcpy(hypre_CSRMatrixI(Cbar) + hypre_ParCSRMatrixNumCols(R), &local_nnz_Cbar, NALU_HYPRE_Int, 1,
                    NALU_HYPRE_MEMORY_DEVICE, NALU_HYPRE_MEMORY_HOST);
      ...
      hypre_TFree(col_map_offd, NALU_HYPRE_MEMORY_DEVICE);
   }
   else
   {
                                     ...
                                     NALU_HYPRE_BigInt       **col_map_offd_C_ptr )
{
   ...
   NALU_HYPRE_Int        Cext_nnz = hypre_CSRMatrixNumNonzeros(Cext);
   ...
   hypre_CSRMatrixMemoryLocation(Cbar_local) = NALU_HYPRE_MEMORY_DEVICE;

   if (!Cext_nnz)
   {
      num_cols_offd_C = num_cols_offd;
      col_map_offd_C = hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_offd, NALU_HYPRE_MEMORY_DEVICE);
      hypre_TMemcpy(col_map_offd_C, col_map_offd, NALU_HYPRE_BigInt, num_cols_offd,
   ...
   else
   {
      ...
      {
         hypre_CSRMatrixJ(Cext) = hypre_TAlloc(NALU_HYPRE_Int, Cext_nnz, NALU_HYPRE_MEMORY_DEVICE);
      }

      NALU_HYPRE_BigInt *Cext_bigj = hypre_CSRMatrixBigJ(Cext);
      NALU_HYPRE_BigInt *big_work  = hypre_TAlloc(NALU_HYPRE_BigInt, Cext_nnz, NALU_HYPRE_MEMORY_DEVICE);
      NALU_HYPRE_Int    *work      = hypre_TAlloc(NALU_HYPRE_Int, Cext_nnz, NALU_HYPRE_MEMORY_DEVICE);
      ...
      hypre_TFree(big_work,      NALU_HYPRE_MEMORY_DEVICE);
      hypre_TFree(work,          NALU_HYPRE_MEMORY_DEVICE);
      hypre_TFree(map_offd_to_C, NALU_HYPRE_MEMORY_DEVICE);
      hypre_TFree(Cext_bigj,     NALU_HYPRE_MEMORY_DEVICE);
      ...
      NALU_HYPRE_Int  num_elemt = hypre_ParCSRCommPkgSendMapStart(comm_pkg, num_sends);
      ...
      hypre_CSRMatrix *IE = hypre_CSRMatrixCreate(num_rows, num_rows + num_elemt,
                                                  num_rows + num_elemt);
      hypre_CSRMatrixMemoryLocation(IE) = NALU_HYPRE_MEMORY_DEVICE;

      NALU_HYPRE_Int     *ie_ii = hypre_TAlloc(NALU_HYPRE_Int, num_rows + num_elemt, NALU_HYPRE_MEMORY_DEVICE);
      NALU_HYPRE_Int     *ie_j  = hypre_TAlloc(NALU_HYPRE_Int, num_rows + num_elemt, NALU_HYPRE_MEMORY_DEVICE);
      NALU_HYPRE_Complex *ie_a  = NULL;

      if (hypre_HandleSpgemmUseVendor(hypre_handle()))
      {
         ie_a = hypre_TAlloc(NALU_HYPRE_Complex, num_rows + num_elemt, NALU_HYPRE_MEMORY_DEVICE);
#if defined(NALU_HYPRE_USING_SYCL)
      ...
      hypre_TFree(ie_ii, NALU_HYPRE_MEMORY_DEVICE);
   ...
   // split into diag and offd
   NALU_HYPRE_Int local_nnz_C = hypre_CSRMatrixNumNonzeros(Cz);

   NALU_HYPRE_Int     *zmp_i = hypreDevice_CsrRowPtrsToIndices(num_rows, local_nnz_C, hypre_CSRMatrixI(Cz));
   NALU_HYPRE_Int     *zmp_j = hypre_CSRMatrixJ(Cz);
   NALU_HYPRE_Complex *zmp_a = hypre_CSRMatrixData(Cz);
   ...
   // free
   hypre_TFree(Cbar_local, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(zmp_i, NALU_HYPRE_MEMORY_DEVICE);
