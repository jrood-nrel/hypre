                                    ...
                                    hypre_CommPkg            **comm_pkg_ptr )
{
   ...
   sprocesses   = hypre_CTAlloc(NALU_HYPRE_Int *,  hypre_BoxArrayArraySize(send_rboxes), NALU_HYPRE_MEMORY_HOST);
   send_rboxnums = hypre_CTAlloc(NALU_HYPRE_Int *,  hypre_BoxArrayArraySize(send_rboxes),
                                 NALU_HYPRE_MEMORY_HOST);

   hypre_ForBoxArrayI(i, sendboxes)
   {
      boxarray = hypre_BoxArrayArrayBoxArray(sendboxes, i);
      sprocesses[i]   = hypre_CTAlloc(NALU_HYPRE_Int,  hypre_BoxArraySize(boxarray), NALU_HYPRE_MEMORY_HOST);
      send_rboxnums[i] = hypre_CTAlloc(NALU_HYPRE_Int,  hypre_BoxArraySize(boxarray), NALU_HYPRE_MEMORY_HOST);
   ...
   rprocesses  = hypre_CTAlloc(NALU_HYPRE_Int *,  hypre_BoxArrayArraySize(recvboxes), NALU_HYPRE_MEMORY_HOST);

   /* dummy pointer for CommInfoCreate */
   recv_rboxnums = hypre_CTAlloc(NALU_HYPRE_Int *,  hypre_BoxArrayArraySize(recvboxes), NALU_HYPRE_MEMORY_HOST);

   hypre_ForBoxArrayI(i, recvboxes)
   {
      boxarray = hypre_BoxArrayArrayBoxArray(recvboxes, i);
      rprocesses[i] = hypre_CTAlloc(NALU_HYPRE_Int,  hypre_BoxArraySize(boxarray), NALU_HYPRE_MEMORY_HOST);
      recv_rboxnums[i] = hypre_CTAlloc(NALU_HYPRE_Int,  hypre_BoxArraySize(boxarray), NALU_HYPRE_MEMORY_HOST);
