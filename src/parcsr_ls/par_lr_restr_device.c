                                           ...
                                           nalu_hypre_ParCSRMatrix  **R_ptr)
{
   ...
   NALU_HYPRE_ONEDPL_CALL( std::copy,
                      count,
                      count + nalu_hypre_CSRMatrixNumRows(Dinv_diag) + 1,
                      nalu_hypre_CSRMatrixI(Dinv_diag) );
   NALU_HYPRE_ONEDPL_CALL( std::copy,
                      count,
                      count + nalu_hypre_CSRMatrixNumRows(Dinv_diag),
...
#if defined(NALU_HYPRE_USING_SYCL)
      hypreSycl_gather( nalu_hypre_ParCSRCommPkgDeviceSendMapElmts(comm_pkg_Z),
