...
extern "C++"
{
                  ...
                  LOOP_BODY loop_body)
   {
      ...
      const sycl::range<1> bDim = nalu_hypre_GetDefaultDeviceBlockDimension();
      const sycl::range<1> gDim = nalu_hypre_GetDefaultDeviceGridDimension(length, "thread", bDim);
...
{                                                                                                     \
   ...
   BoxLoopforall(nalu_hypre__tot, [=] (sycl::nd_item<1> item)                                              \
   {                                                                                                  \
...
{                                                                                                     \
   ...
   ReductionBoxLoopforall( [=,nalu_hypre_unused_var=sum_var] (sycl::nd_item<1> item, auto &sum_var)        \
   {                                                                                                  \
      NALU_HYPRE_Int idx = (NALU_HYPRE_Int) item.get_global_linear_id();                                        \
      if (idx < nalu_hypre__tot)                                                                           \
      {                                                                                               \
