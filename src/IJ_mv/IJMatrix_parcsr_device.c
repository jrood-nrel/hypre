...
__global__ void
hypreGPUKernel_IJMatrixValues_dev1(nalu_hypre_DeviceItem &item, NALU_HYPRE_Int n, NALU_HYPRE_Int *rowind,
...
NALU_HYPRE_Int
nalu_hypre_IJMatrixSetAddValuesParCSRDevice( nalu_hypre_IJMatrix       *matrix,
                                        ...
                                        const char           *action )
{
   ...
   if (ncols)
   {
      row_ptr = nalu_hypre_TAlloc(NALU_HYPRE_Int, nrows + 1, NALU_HYPRE_MEMORY_DEVICE);
      nalu_hypre_TMemcpy(row_ptr, ncols, NALU_HYPRE_Int, nrows, NALU_HYPRE_MEMORY_DEVICE, NALU_HYPRE_MEMORY_DEVICE);
      /* RL: have to init the last entry !!! */
      nalu_hypre_Memset(row_ptr + nrows, 0, sizeof(NALU_HYPRE_Int), NALU_HYPRE_MEMORY_DEVICE);
      hypreDevice_IntegerExclusiveScan(nrows + 1, row_ptr);
      nalu_hypre_TMemcpy(&nelms, row_ptr + nrows, NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
   }
   else
   {
      nelms = nrows;
   ...
   if (nelms <= 0)
   {
      nalu_hypre_TFree(row_ptr, NALU_HYPRE_MEMORY_DEVICE);
      return nalu_hypre_error_flag;
   }

   if (!aux_matrix)
   {
      nalu_hypre_AuxParCSRMatrixCreate(&aux_matrix, num_local_rows, num_local_cols, NULL);
      nalu_hypre_AuxParCSRMatrixInitialize_v2(aux_matrix, NALU_HYPRE_MEMORY_DEVICE);
      nalu_hypre_IJMatrixTranslator(matrix) = aux_matrix;
   }

   NALU_HYPRE_BigInt   stack_elmts_max      = nalu_hypre_AuxParCSRMatrixMaxStackElmts(aux_matrix);
   NALU_HYPRE_BigInt   stack_elmts_current  = nalu_hypre_AuxParCSRMatrixCurrentStackElmts(aux_matrix);
   NALU_HYPRE_BigInt   stack_elmts_required = stack_elmts_current + (NALU_HYPRE_BigInt) nelms;
   NALU_HYPRE_BigInt  *stack_i              = nalu_hypre_AuxParCSRMatrixStackI(aux_matrix);
   NALU_HYPRE_BigInt  *stack_j              = nalu_hypre_AuxParCSRMatrixStackJ(aux_matrix);
   NALU_HYPRE_Complex *stack_data           = nalu_hypre_AuxParCSRMatrixStackData(aux_matrix);
   char          *stack_sora           = nalu_hypre_AuxParCSRMatrixStackSorA(aux_matrix);

   if ( stack_elmts_max < stack_elmts_required )
   {
      NALU_HYPRE_BigInt stack_elmts_max_new =
         nalu_hypre_max(nalu_hypre_AuxParCSRMatrixUsrOnProcElmts (aux_matrix), 0) +
         nalu_hypre_max(nalu_hypre_AuxParCSRMatrixUsrOffProcElmts(aux_matrix), 0);

      if ( nalu_hypre_AuxParCSRMatrixUsrOnProcElmts (aux_matrix) < 0 ||
           nalu_hypre_AuxParCSRMatrixUsrOffProcElmts(aux_matrix) < 0 )
      {
         stack_elmts_max_new = nalu_hypre_max(num_local_rows * nalu_hypre_AuxParCSRMatrixInitAllocFactor(aux_matrix),
                                         stack_elmts_max_new);
         stack_elmts_max_new = nalu_hypre_max(stack_elmts_max * nalu_hypre_AuxParCSRMatrixGrowFactor(aux_matrix),
                                         stack_elmts_max_new);
      }
      stack_elmts_max_new = nalu_hypre_max(stack_elmts_required, stack_elmts_max_new);

      nalu_hypre_AuxParCSRMatrixStackI(aux_matrix)    = stack_i    = nalu_hypre_TReAlloc_v2(stack_i,
                                                                                  NALU_HYPRE_BigInt,  stack_elmts_max, NALU_HYPRE_BigInt,  stack_elmts_max_new, NALU_HYPRE_MEMORY_DEVICE);
      nalu_hypre_AuxParCSRMatrixStackJ(aux_matrix)    = stack_j    = nalu_hypre_TReAlloc_v2(stack_j,
                                                                                  NALU_HYPRE_BigInt,  stack_elmts_max, NALU_HYPRE_BigInt,  stack_elmts_max_new, NALU_HYPRE_MEMORY_DEVICE);
      nalu_hypre_AuxParCSRMatrixStackData(aux_matrix) = stack_data = nalu_hypre_TReAlloc_v2(stack_data,
                                                                                  NALU_HYPRE_Complex, stack_elmts_max, NALU_HYPRE_Complex, stack_elmts_max_new, NALU_HYPRE_MEMORY_DEVICE);
      nalu_hypre_AuxParCSRMatrixStackSorA(aux_matrix) = stack_sora = nalu_hypre_TReAlloc_v2(stack_sora,
                                                                                  char, stack_elmts_max,          char, stack_elmts_max_new, NALU_HYPRE_MEMORY_DEVICE);
      nalu_hypre_AuxParCSRMatrixMaxStackElmts(aux_matrix) = stack_elmts_max_new;
   }
   ...
   {
      nalu_hypre_TMemcpy(stack_i + stack_elmts_current, rows, NALU_HYPRE_BigInt, nelms, NALU_HYPRE_MEMORY_DEVICE,
   ...
   if (row_indexes)
   {
      ...
      nalu_hypre_TMemcpy(&len1, &row_indexes[nrows - 1], NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
      if (ncols)
      {
         nalu_hypre_TMemcpy(&len, &ncols[nrows - 1], NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
      }
      else
      {
         len = 1;
      ...
      len += len1;
      NALU_HYPRE_Int *indicator = nalu_hypre_CTAlloc(NALU_HYPRE_Int, len, NALU_HYPRE_MEMORY_DEVICE);
      ...
      dim3 gDim = nalu_hypre_GetDefaultDeviceGridDimension(len1, "thread", bDim);
      ...
      nalu_hypre_assert(nnz_tmp == nelms);

      nalu_hypre_TFree(indicator, NALU_HYPRE_MEMORY_DEVICE);
   }
   else
   {
      nalu_hypre_TMemcpy(stack_j    + stack_elmts_current, cols,   NALU_HYPRE_BigInt,  nelms, NALU_HYPRE_MEMORY_DEVICE,
                    NALU_HYPRE_MEMORY_DEVICE);
      nalu_hypre_TMemcpy(stack_data + stack_elmts_current, values, NALU_HYPRE_Complex, nelms, NALU_HYPRE_MEMORY_DEVICE,
                    NALU_HYPRE_MEMORY_DEVICE);
   }

   nalu_hypre_AuxParCSRMatrixCurrentStackElmts(aux_matrix) += (NALU_HYPRE_BigInt) nelms;
...
struct nalu_hypre_IJMatrixAssembleFunctor
{
...
#else
template<typename T1, typename T2>
struct nalu_hypre_IJMatrixAssembleFunctor : public
...
/* helper routine used in nalu_hypre_IJMatrixAssembleParCSRDevice:
 ...
 */
NALU_HYPRE_Int
nalu_hypre_IJMatrixAssembleSortAndReduce1(NALU_HYPRE_Int  N0, NALU_HYPRE_BigInt  *I0, NALU_HYPRE_BigInt  *J0, char  *X0,
                                     ...
                                     NALU_HYPRE_Int *N1, NALU_HYPRE_BigInt **I1, NALU_HYPRE_BigInt **J1, char **X1, NALU_HYPRE_Complex **A1 )
{
   ...
   NALU_HYPRE_BigInt  *J = nalu_hypre_TAlloc(NALU_HYPRE_BigInt,  N0, NALU_HYPRE_MEMORY_DEVICE);
   ...
   /*
   dim3 bDim = nalu_hypre_GetDefaultDeviceBlockDimension();
   dim3 gDim = nalu_hypre_GetDefaultDeviceGridDimension(N0, "thread", bDim);
   ...
   */
   ...
   auto new_end = NALU_HYPRE_ONEDPL_CALL( oneapi::dpl::reduce_by_segment,
                                     ...
                                     nalu_hypre_IJMatrixAssembleFunctor<char, NALU_HYPRE_Complex>()          /* binary_op */);
...
#if defined(NALU_HYPRE_USING_SYCL)
template<typename T1, typename T2>
struct nalu_hypre_IJMatrixAssembleFunctor2
{
   typedef std::tuple<T1, T2> Tuple;
...
#else
template<typename T1, typename T2>
struct nalu_hypre_IJMatrixAssembleFunctor2 : public
   thrust::binary_function< thrust::tuple<T1, T2>, thrust::tuple<T1, T2>, thrust::tuple<T1, T2> >
{
   typedef thrust::tuple<T1, T2> Tuple;
...
NALU_HYPRE_Int
nalu_hypre_IJMatrixAssembleSortAndReduce2(NALU_HYPRE_Int  N0, NALU_HYPRE_Int  *I0, NALU_HYPRE_Int  *J0, char  *X0,
                                     NALU_HYPRE_Complex  *A0,
                                     ...
                                     NALU_HYPRE_Int  opt )
{
   ...
   NALU_HYPRE_Int     *I = nalu_hypre_TAlloc(NALU_HYPRE_Int,     N0, NALU_HYPRE_MEMORY_DEVICE);
   NALU_HYPRE_Int     *J = nalu_hypre_TAlloc(NALU_HYPRE_Int,     N0, NALU_HYPRE_MEMORY_DEVICE);
   char          *X = nalu_hypre_TAlloc(char,          N0, NALU_HYPRE_MEMORY_DEVICE);
   NALU_HYPRE_Complex *A = nalu_hypre_TAlloc(NALU_HYPRE_Complex, N0, NALU_HYPRE_MEMORY_DEVICE);
   ...
   auto new_end = NALU_HYPRE_ONEDPL_CALL( oneapi::dpl::reduce_by_segment,
                                     ...
                                     nalu_hypre_IJMatrixAssembleFunctor2<char, NALU_HYPRE_Complex>()   /* binary_op */);
   ...
   *A1 = A;
   ...
   nalu_hypre_TFree(X, NALU_HYPRE_MEMORY_DEVICE);

   return nalu_hypre_error_flag;
}

NALU_HYPRE_Int
nalu_hypre_IJMatrixAssembleSortAndReduce3(NALU_HYPRE_Int  N0, NALU_HYPRE_BigInt  *I0, NALU_HYPRE_BigInt  *J0,  char *X0,
                                     NALU_HYPRE_Complex  *A0,
                                     NALU_HYPRE_Int *N1)
{
   ...
   *N1 = thrust::get<0>(new_end2.get_iterator_tuple()) - I0;
   ...
   nalu_hypre_TFree(J, NALU_HYPRE_MEMORY_DEVICE);
...
#if 0
NALU_HYPRE_Int
nalu_hypre_IJMatrixAssembleSortAndRemove(NALU_HYPRE_Int N0, NALU_HYPRE_BigInt *I0, NALU_HYPRE_BigInt *J0, char *X0,
                                    NALU_HYPRE_Complex *A0)
{
   hypreDevice_StableSortTupleByTupleKey(N0, I0, J0, X0, A0, 0);
...
NALU_HYPRE_Int
nalu_hypre_IJMatrixAssembleParCSRDevice(nalu_hypre_IJMatrix *matrix)
{
   ...
   NALU_HYPRE_Int      nelms      = nalu_hypre_AuxParCSRMatrixCurrentStackElmts(aux_matrix);
   NALU_HYPRE_BigInt  *stack_i    = nalu_hypre_AuxParCSRMatrixStackI(aux_matrix);
   NALU_HYPRE_BigInt  *stack_j    = nalu_hypre_AuxParCSRMatrixStackJ(aux_matrix);
   NALU_HYPRE_Complex *stack_data = nalu_hypre_AuxParCSRMatrixStackData(aux_matrix);
   char          *stack_sora = nalu_hypre_AuxParCSRMatrixStackSorA(aux_matrix);
   ...
   if (nelms_off_max)
   {
      ...
      if (nelms_off)
      {
         ...
         off_proc_j          = nalu_hypre_TAlloc(NALU_HYPRE_BigInt,  nelms_off, NALU_HYPRE_MEMORY_DEVICE);
         ...
         nalu_hypre_AuxParCSRMatrixCurrentStackElmts(aux_matrix) = nelms_on;

         nalu_hypre_TFree(is_on_proc, NALU_HYPRE_MEMORY_DEVICE);

         /* sort and reduce */
         nalu_hypre_IJMatrixAssembleSortAndReduce3(nelms_off, off_proc_i, off_proc_j, off_proc_sora,
                                              off_proc_data, &new_nnz);
         // new_nnz = nalu_hypre_IJMatrixAssembleSortAndRemove(nelms_off, off_proc_i, off_proc_j, off_proc_sora, off_proc_data);
      ...
      nalu_hypre_IJMatrixAssembleOffProcValsParCSR(matrix, -1, -1, new_nnz, NALU_HYPRE_MEMORY_DEVICE, off_proc_i,
                                              off_proc_j, off_proc_data);

      nalu_hypre_TFree(off_proc_i,    NALU_HYPRE_MEMORY_DEVICE);
      nalu_hypre_TFree(off_proc_j,    NALU_HYPRE_MEMORY_DEVICE);
      nalu_hypre_TFree(off_proc_data, NALU_HYPRE_MEMORY_DEVICE);
   }

   /* Note: the stack might have been changed in nalu_hypre_IJMatrixAssembleOffProcValsParCSR,
    * so must get the size and the pointers again */
   nelms      = nalu_hypre_AuxParCSRMatrixCurrentStackElmts(aux_matrix);
   stack_i    = nalu_hypre_AuxParCSRMatrixStackI(aux_matrix);
   stack_j    = nalu_hypre_AuxParCSRMatrixStackJ(aux_matrix);
   stack_data = nalu_hypre_AuxParCSRMatrixStackData(aux_matrix);
   stack_sora = nalu_hypre_AuxParCSRMatrixStackSorA(aux_matrix);
   ...
   if (nelms)
   {
      ...
      /* sort and reduce */
      nalu_hypre_IJMatrixAssembleSortAndReduce1(nelms, stack_i, stack_j, stack_sora, stack_data,
                                           &new_nnz, &new_i, &new_j, &new_sora, &new_data);

      /* adjust row indices from global to local */
      NALU_HYPRE_Int *new_i_local = nalu_hypre_TAlloc(NALU_HYPRE_Int, new_nnz, NALU_HYPRE_MEMORY_DEVICE);
#if defined(NALU_HYPRE_USING_SYCL)
      NALU_HYPRE_ONEDPL_CALL( std::transform,
                         new_i,
                         new_i + new_nnz,
      ...
      NALU_HYPRE_Int diag_nnz_existed = nalu_hypre_CSRMatrixNumNonzeros(nalu_hypre_ParCSRMatrixDiag(par_matrix));
      NALU_HYPRE_Int offd_nnz_existed = nalu_hypre_CSRMatrixNumNonzeros(nalu_hypre_ParCSRMatrixOffd(par_matrix));

      nalu_hypre_CSRMatrixSplitDevice_core( 0,
                                       nrows,
                                       new_nnz,
                                       NULL,
                                       new_j,
                                       ...
                                       col_start,
                                       col_end - 1,
                                       nalu_hypre_CSRMatrixNumCols(nalu_hypre_ParCSRMatrixOffd(par_matrix)),
                                       NULL,
                                       NULL,
                                       NULL,
                                       NULL,
      ...
      if (diag_nnz_new)
      {
         diag_i_new = nalu_hypre_TAlloc(NALU_HYPRE_Int,     diag_nnz_existed + diag_nnz_new, NALU_HYPRE_MEMORY_DEVICE);
         diag_j_new = nalu_hypre_TAlloc(NALU_HYPRE_Int,     diag_nnz_existed + diag_nnz_new, NALU_HYPRE_MEMORY_DEVICE);
         diag_a_new = nalu_hypre_TAlloc(NALU_HYPRE_Complex, diag_nnz_existed + diag_nnz_new, NALU_HYPRE_MEMORY_DEVICE);
         if (diag_nnz_existed)
         {
            diag_sora_new = nalu_hypre_TAlloc(char,    diag_nnz_existed + diag_nnz_new, NALU_HYPRE_MEMORY_DEVICE);
         }
      }

      if (offd_nnz_new)
      {
         offd_i_new = nalu_hypre_TAlloc(NALU_HYPRE_Int,     offd_nnz_existed + offd_nnz_new, NALU_HYPRE_MEMORY_DEVICE);
         offd_j_new = nalu_hypre_TAlloc(NALU_HYPRE_Int,     offd_nnz_existed + offd_nnz_new, NALU_HYPRE_MEMORY_DEVICE);
         offd_a_new = nalu_hypre_TAlloc(NALU_HYPRE_Complex, offd_nnz_existed + offd_nnz_new, NALU_HYPRE_MEMORY_DEVICE);
         if (offd_nnz_existed)
         {
            offd_sora_new = nalu_hypre_TAlloc(char,    offd_nnz_existed + offd_nnz_new, NALU_HYPRE_MEMORY_DEVICE);
         }
      }
      ...
      nalu_hypre_CSRMatrixSplitDevice_core( 1,
                                       ...
                                       col_end - 1,
                                       nalu_hypre_CSRMatrixNumCols(nalu_hypre_ParCSRMatrixOffd(par_matrix)),
                                       nalu_hypre_ParCSRMatrixDeviceColMapOffd(par_matrix),
                                       &col_map_offd_map,
                                       &num_cols_offd_new,
                                       &col_map_offd_new,
                                       &diag_nnz_new,
      ...
      nalu_hypre_TFree(new_i_local, NALU_HYPRE_MEMORY_DEVICE);
      nalu_hypre_TFree(new_j,       NALU_HYPRE_MEMORY_DEVICE);
      ...
      if (diag_nnz_new > 0)
      {
         if (diag_nnz_existed)
         {
            ...
            hypreDevice_CsrRowPtrsToIndices_v2(nrows, diag_nnz_existed,
                                               nalu_hypre_CSRMatrixI(nalu_hypre_ParCSRMatrixDiag(par_matrix)), diag_i_new);

            nalu_hypre_TMemcpy(diag_j_new, nalu_hypre_CSRMatrixJ(nalu_hypre_ParCSRMatrixDiag(par_matrix)), NALU_HYPRE_Int,
                          diag_nnz_existed, NALU_HYPRE_MEMORY_DEVICE, NALU_HYPRE_MEMORY_DEVICE);

            nalu_hypre_TMemcpy(diag_a_new, nalu_hypre_CSRMatrixData(nalu_hypre_ParCSRMatrixDiag(par_matrix)), NALU_HYPRE_Complex,
                          diag_nnz_existed, NALU_HYPRE_MEMORY_DEVICE, NALU_HYPRE_MEMORY_DEVICE);

            hypreDevice_CharFilln(diag_sora_new, diag_nnz_existed, 0);

            nalu_hypre_IJMatrixAssembleSortAndReduce2(diag_nnz_existed + diag_nnz_new, diag_i_new, diag_j_new,
                                                 diag_sora_new, diag_a_new,
                                                 &nnz_new, &tmp_i, &tmp_j, &tmp_a, 2);

            nalu_hypre_TFree(diag_i_new,    NALU_HYPRE_MEMORY_DEVICE);
            nalu_hypre_TFree(diag_j_new,    NALU_HYPRE_MEMORY_DEVICE);
            nalu_hypre_TFree(diag_sora_new, NALU_HYPRE_MEMORY_DEVICE);
            nalu_hypre_TFree(diag_a_new,    NALU_HYPRE_MEMORY_DEVICE);

            tmp_j = nalu_hypre_TReAlloc_v2(tmp_j, NALU_HYPRE_Int,     diag_nnz_existed + diag_nnz_new, NALU_HYPRE_Int,
                                      nnz_new, NALU_HYPRE_MEMORY_DEVICE);
            tmp_a = nalu_hypre_TReAlloc_v2(tmp_a, NALU_HYPRE_Complex, diag_nnz_existed + diag_nnz_new, NALU_HYPRE_Complex,
                                      nnz_new, NALU_HYPRE_MEMORY_DEVICE);
         ...
         nalu_hypre_CSRMatrix *diag               = nalu_hypre_CSRMatrixCreate(nrows, ncols, diag_nnz_new);
         nalu_hypre_CSRMatrixI(diag)              = hypreDevice_CsrRowIndicesToPtrs(nrows, diag_nnz_new,
                                                                               diag_i_new);
         nalu_hypre_CSRMatrixJ(diag)              = diag_j_new;
         nalu_hypre_CSRMatrixData(diag)           = diag_a_new;
         nalu_hypre_CSRMatrixMemoryLocation(diag) = NALU_HYPRE_MEMORY_DEVICE;

         nalu_hypre_TFree(diag_i_new, NALU_HYPRE_MEMORY_DEVICE);

         nalu_hypre_CSRMatrixDestroy(nalu_hypre_ParCSRMatrixDiag(par_matrix));
         nalu_hypre_ParCSRMatrixDiag(par_matrix) = diag;
      }
      ...
      if (offd_nnz_new > 0)
      {
         if (offd_nnz_existed)
         {
            ...
            hypreDevice_CsrRowPtrsToIndices_v2(nrows, offd_nnz_existed,
                                               nalu_hypre_CSRMatrixI(nalu_hypre_ParCSRMatrixOffd(par_matrix)), offd_i_new);

            /* adjust with the new col_map_offd_map */
#if defined(NALU_HYPRE_USING_SYCL)
            hypreSycl_gather( nalu_hypre_CSRMatrixJ(nalu_hypre_ParCSRMatrixOffd(par_matrix)),
                              nalu_hypre_CSRMatrixJ(nalu_hypre_ParCSRMatrixOffd(par_matrix)) + offd_nnz_existed,
                              col_map_offd_map,
                              offd_j_new );
#else
            NALU_HYPRE_THRUST_CALL( gather,
                               nalu_hypre_CSRMatrixJ(nalu_hypre_ParCSRMatrixOffd(par_matrix)),
                               nalu_hypre_CSRMatrixJ(nalu_hypre_ParCSRMatrixOffd(par_matrix)) + offd_nnz_existed,
                               col_map_offd_map,
                               offd_j_new );
#endif

            nalu_hypre_TMemcpy(offd_a_new, nalu_hypre_CSRMatrixData(nalu_hypre_ParCSRMatrixOffd(par_matrix)), NALU_HYPRE_Complex,
                          offd_nnz_existed, NALU_HYPRE_MEMORY_DEVICE, NALU_HYPRE_MEMORY_DEVICE);

            hypreDevice_CharFilln(offd_sora_new, offd_nnz_existed, 0);

            nalu_hypre_IJMatrixAssembleSortAndReduce2(offd_nnz_existed + offd_nnz_new, offd_i_new, offd_j_new,
                                                 offd_sora_new, offd_a_new,
                                                 &nnz_new, &tmp_i, &tmp_j, &tmp_a, 0);

            nalu_hypre_TFree(offd_i_new,    NALU_HYPRE_MEMORY_DEVICE);
            nalu_hypre_TFree(offd_j_new,    NALU_HYPRE_MEMORY_DEVICE);
            nalu_hypre_TFree(offd_sora_new, NALU_HYPRE_MEMORY_DEVICE);
            nalu_hypre_TFree(offd_a_new,    NALU_HYPRE_MEMORY_DEVICE);

            tmp_j = nalu_hypre_TReAlloc_v2(tmp_j, NALU_HYPRE_Int,     offd_nnz_existed + offd_nnz_new, NALU_HYPRE_Int,
                                      nnz_new, NALU_HYPRE_MEMORY_DEVICE);
            tmp_a = nalu_hypre_TReAlloc_v2(tmp_a, NALU_HYPRE_Complex, offd_nnz_existed + offd_nnz_new, NALU_HYPRE_Complex,
                                      nnz_new, NALU_HYPRE_MEMORY_DEVICE);
         ...
         nalu_hypre_CSRMatrix *offd               = nalu_hypre_CSRMatrixCreate(nrows, num_cols_offd_new, offd_nnz_new);
         nalu_hypre_CSRMatrixI(offd)              = hypreDevice_CsrRowIndicesToPtrs(nrows, offd_nnz_new,
                                                                               offd_i_new);
         nalu_hypre_CSRMatrixJ(offd)              = offd_j_new;
         nalu_hypre_CSRMatrixData(offd)           = offd_a_new;
         nalu_hypre_CSRMatrixMemoryLocation(offd) = NALU_HYPRE_MEMORY_DEVICE;

         nalu_hypre_TFree(offd_i_new, NALU_HYPRE_MEMORY_DEVICE);

         nalu_hypre_CSRMatrixDestroy(nalu_hypre_ParCSRMatrixOffd(par_matrix));
         nalu_hypre_ParCSRMatrixOffd(par_matrix) = offd;

         nalu_hypre_TFree(nalu_hypre_ParCSRMatrixDeviceColMapOffd(par_matrix), NALU_HYPRE_MEMORY_DEVICE);
         nalu_hypre_ParCSRMatrixDeviceColMapOffd(par_matrix) = col_map_offd_new;

         nalu_hypre_TFree(nalu_hypre_ParCSRMatrixColMapOffd(par_matrix), NALU_HYPRE_MEMORY_HOST);
         nalu_hypre_ParCSRMatrixColMapOffd(par_matrix) = nalu_hypre_TAlloc(NALU_HYPRE_BigInt, num_cols_offd_new,
                                                                 NALU_HYPRE_MEMORY_HOST);
         nalu_hypre_TMemcpy(nalu_hypre_ParCSRMatrixColMapOffd(par_matrix), col_map_offd_new, NALU_HYPRE_BigInt,
                       num_cols_offd_new,
      ...
      nalu_hypre_TFree(col_map_offd_map, NALU_HYPRE_MEMORY_DEVICE);
      nalu_hypre_TFree(col_map_offd_new, NALU_HYPRE_MEMORY_DEVICE);
   } /* if (nelms) */
...
NALU_HYPRE_Int
nalu_hypre_IJMatrixSetConstantValuesParCSRDevice( nalu_hypre_IJMatrix *matrix,
