                          ...
                          nalu_hypre_ParCSRMatrix  *B )
{
   ...
   if (num_cols_offd_C)
   {
      ...
      nalu_hypre_TMemcpy(nalu_hypre_ParCSRMatrixColMapOffd(C), col_map_offd_C, NALU_HYPRE_BigInt, num_cols_offd_C,
                               ...
                               NALU_HYPRE_Int           keep_transpose )
{
   ...
   if (num_procs > 1)
   {
      ...
      nalu_hypre_assert(nalu_hypre_CSRMatrixNumRows(Cbar) == nalu_hypre_ParCSRMatrixNumCols(R) + nalu_hypre_CSRMatrixNumCols(
                      R_offd));
      nalu_hypre_assert(nalu_hypre_CSRMatrixNumCols(Cbar) == nalu_hypre_ParCSRMatrixNumCols(P) + num_cols_offd);

      nalu_hypre_TMemcpy(&local_nnz_Cbar, nalu_hypre_CSRMatrixI(Cbar) + nalu_hypre_ParCSRMatrixNumCols(R), NALU_HYPRE_Int, 1,
                    NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
      ...
      RAP_functor<1, NALU_HYPRE_BigInt> func1(nalu_hypre_ParCSRMatrixNumCols(P), nalu_hypre_ParCSRMatrixFirstColDiag(P),
                                         col_map_offd);
#if defined(NALU_HYPRE_USING_SYCL)
      NALU_HYPRE_ONEDPL_CALL( std::transform,
                         nalu_hypre_CSRMatrixJ(Cbar) + local_nnz_Cbar,
      ...
      nalu_hypre_TMemcpy(nalu_hypre_CSRMatrixI(Cbar) + nalu_hypre_ParCSRMatrixNumCols(R), &local_nnz_Cbar, NALU_HYPRE_Int, 1,
                    NALU_HYPRE_MEMORY_DEVICE, NALU_HYPRE_MEMORY_HOST);
