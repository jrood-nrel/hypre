...
2. Call the AMS solver in NALU_HYPRE to solve that linear system.
*/
      ...
      char *argv[])
{
   ...
   NALU_HYPRE_Int maxit, pcg_maxit, cycle_type, rlx_type, coarse_rlx_type, rlx_sweeps, dim;
   ...
   /* Parse command line */
   {
      ...
      while (arg_index < argc)
      {
         ...
         {
            arg_index++;
            rlx_weight = (NALU_HYPRE_Real)atof(argv[arg_index++]);
         }
         else if ( strcmp(argv[arg_index], "-rlxo") == 0 )
         {
            arg_index++;
            rlx_omega = (NALU_HYPRE_Real)atof(argv[arg_index++]);
         }
         ...
         {
            arg_index++;
            theta = (NALU_HYPRE_Real)atof(argv[arg_index++]);
         }
         ...
         {
            arg_index++;
            rtol = (NALU_HYPRE_Real)atof(argv[arg_index++]);
         }
      ...
      if ((print_usage) && (myid == 0))
      {
         ...
         nalu_hypre_printf("    -maxit <num>         : maximum number of iterations (200)    \n");
         nalu_hypre_printf("    -pcg_maxit <num>     : maximum number of PCG iterations (50) \n");
   ...
   if (solver_id == 5)
   {
      ...
      NALU_HYPRE_AMESetMaxPCGIter(solver, pcg_maxit); /* max iterations */
