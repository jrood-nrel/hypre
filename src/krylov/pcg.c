                ...
                void *x         )
{
   ...
   */
   ...
   if ( logging > 0 || print_level > 0 || cf_tol > 0.0 )
   {
      ...
      if ( logging > 0 || print_level > 0 ) { norms[0] = nalu_hypre_sqrt(i_prod_0); }
   }
   if ( print_level > 1 && my_id == 0 )
   {
      nalu_hypre_printf("\n\n");
   ...
   while ((i + 1) <= max_iter)
   {
      ...
      if (two_norm)
         nalu_hypre_printf("Iter (%d): ||r||_2 = %e, ||r||_2/||b||_2 = %e\n",
                      i, nalu_hypre_sqrt(i_prod), (bi_prod ? nalu_hypre_sqrt(i_prod / bi_prod) : 0));
      else
      ...
      if ( logging > 0 || print_level > 0 )
      {
         ...
         rel_norms[i] = bi_prod ? nalu_hypre_sqrt(i_prod / bi_prod) : 0;
      }
      if ( print_level > 1 && my_id == 0 )
      {
         if (two_norm)
      ...
      if (cf_tol > 0.0)
      {
         ...
         cf_ave_1 = nalu_hypre_pow( i_prod / i_prod_0, 1.0 / (2.0 * i) );
   ...
   {
      char msg[1024];
      nalu_hypre_sprintf(msg, "Reached max iterations %d in PCG before convergence", max_iter);
      nalu_hypre_error_w_msg(NALU_HYPRE_ERROR_CONV, msg);
   }

   (pcg_data -> num_iterations) = i;
   if (bi_prod > 0.0)
   {
      (pcg_data -> rel_residual_norm) = nalu_hypre_sqrt(i_prod / bi_prod);
   }
