                                       ...
                                       NALU_HYPRE_Int *rownnz_exact_ptr)
{
   ...
   *rownnz_exact_ptr = !NALU_HYPRE_ONEDPL_CALL( std::any_of,
                                           d_rf,
                                           d_rf + m,
                                  ...
                                  NALU_HYPRE_Int *d_rc )
{
   ...
   if (can_fail)
   {
      ...
      NALU_HYPRE_Int num_failed_rows =
         NALU_HYPRE_ONEDPL_CALL( std::reduce,
                            oneapi::dpl::make_transform_iterator(d_rf,     type_cast<char, NALU_HYPRE_Int>()),
                            oneapi::dpl::make_transform_iterator(d_rf + m, type_cast<char, NALU_HYPRE_Int>()) );
#else
      NALU_HYPRE_Int num_failed_rows =
      ...
      if (num_failed_rows)
      {
         ...
         NALU_HYPRE_Int *new_end = hypreSycl_copy_if(
                                 count,
                                 count + m,
                                 d_rf,
                                 d_rind,
                                   ...
                                   NALU_HYPRE_Int *d_rc )
{
   ...
   /* naive upper bound */
#if defined(NALU_HYPRE_USING_SYCL)
   NALU_HYPRE_ONEDPL_CALL( std::adjacent_difference, d_ib, d_ib + k + 1, d_rind );
#else
