...
void hypre_smallest_abs_val( NALU_HYPRE_Int   n,
                             NALU_HYPRE_Real *v,
                             ...
                             NALU_HYPRE_Int  &min_j )
{
   ...
   {
      const NALU_HYPRE_Real vj = fabs(v[j]);
      if (vj < min_v)
      {
         min_v = vj;
         min_j = j;
                                         ...
                                         NALU_HYPRE_Int  *P_offd_i_new )
{
   ...
   const NALU_HYPRE_Int nt = hypre_gpu_get_num_threads<1>(item);
   const NALU_HYPRE_Int tid = hypre_gpu_get_thread_id<1>(item);
...
hypre_BoomerAMGInterpTruncationDevice_v1( hypre_ParCSRMatrix *P,
                                          ...
                                          NALU_HYPRE_Int           max_elmts )
{
   NALU_HYPRE_Int        nrows       = hypre_ParCSRMatrixNumRows(P);
   hypre_CSRMatrix *P_diag      = hypre_ParCSRMatrixDiag(P);
   NALU_HYPRE_Int       *P_diag_i    = hypre_CSRMatrixI(P_diag);
   NALU_HYPRE_Int       *P_diag_j    = hypre_CSRMatrixJ(P_diag);
   NALU_HYPRE_Real      *P_diag_a    = hypre_CSRMatrixData(P_diag);
   hypre_CSRMatrix *P_offd      = hypre_ParCSRMatrixOffd(P);
   NALU_HYPRE_Int       *P_offd_i    = hypre_CSRMatrixI(P_offd);
   NALU_HYPRE_Int       *P_offd_j    = hypre_CSRMatrixJ(P_offd);
   NALU_HYPRE_Real      *P_offd_a    = hypre_CSRMatrixData(P_offd);

   NALU_HYPRE_MemoryLocation memory_location = hypre_ParCSRMatrixMemoryLocation(P);

   NALU_HYPRE_Int *P_diag_i_new = hypre_TAlloc(NALU_HYPRE_Int, nrows + 1, memory_location);
   NALU_HYPRE_Int *P_offd_i_new = hypre_TAlloc(NALU_HYPRE_Int, nrows + 1, memory_location);
   ...
   hypre_Memset(&P_diag_i_new[nrows], 0, sizeof(NALU_HYPRE_Int), memory_location);
   hypre_Memset(&P_offd_i_new[nrows], 0, sizeof(NALU_HYPRE_Int), memory_location);
   ...
   hypre_TMemcpy(&nnz_diag, &P_diag_i_new[nrows], NALU_HYPRE_Int, 1,
                 NALU_HYPRE_MEMORY_HOST, memory_location);
   hypre_TMemcpy(&nnz_offd, &P_offd_i_new[nrows], NALU_HYPRE_Int, 1,
                 NALU_HYPRE_MEMORY_HOST, memory_location);

   hypre_CSRMatrixNumNonzeros(P_diag) = nnz_diag;
   hypre_CSRMatrixNumNonzeros(P_offd) = nnz_offd;

   NALU_HYPRE_Int  *P_diag_j_new = hypre_TAlloc(NALU_HYPRE_Int,  nnz_diag, memory_location);
   NALU_HYPRE_Real *P_diag_a_new = hypre_TAlloc(NALU_HYPRE_Real, nnz_diag, memory_location);
   NALU_HYPRE_Int  *P_offd_j_new = hypre_TAlloc(NALU_HYPRE_Int,  nnz_offd, memory_location);
   NALU_HYPRE_Real *P_offd_a_new = hypre_TAlloc(NALU_HYPRE_Real, nnz_offd, memory_location);
                                    ...
                                    NALU_HYPRE_Real *P_a)
{
   ...
   NALU_HYPRE_Int lane = hypre_gpu_get_lane_id<1>(item), p = 0, q;
                                          ...
                                          NALU_HYPRE_Int           max_elmts )
{
   ...
   //NALU_HYPRE_Int        ncols       = hypre_CSRMatrixNumCols(P_diag);
   NALU_HYPRE_Int        nrows       = hypre_CSRMatrixNumRows(P_diag);
   NALU_HYPRE_Int        nnz_diag    = hypre_CSRMatrixNumNonzeros(P_diag);
   NALU_HYPRE_Int        nnz_offd    = hypre_CSRMatrixNumNonzeros(P_offd);
   NALU_HYPRE_Int        nnz_P       = nnz_diag + nnz_offd;
   NALU_HYPRE_Int       *P_i         = hypre_TAlloc(NALU_HYPRE_Int,  nnz_P,     NALU_HYPRE_MEMORY_DEVICE);
   NALU_HYPRE_Int       *P_j         = hypre_TAlloc(NALU_HYPRE_Int,  nnz_P,     NALU_HYPRE_MEMORY_DEVICE);
   NALU_HYPRE_Real      *P_a         = hypre_TAlloc(NALU_HYPRE_Real, nnz_P,     NALU_HYPRE_MEMORY_DEVICE);
   NALU_HYPRE_Int       *P_rowptr    = hypre_TAlloc(NALU_HYPRE_Int,  nrows + 1, NALU_HYPRE_MEMORY_DEVICE);
   NALU_HYPRE_Int       *tmp_rowid   = hypre_TAlloc(NALU_HYPRE_Int,  nnz_P,     NALU_HYPRE_MEMORY_DEVICE);
   ...
   hypre_TMemcpy(P_j, P_diag_j, NALU_HYPRE_Int, nnz_diag, NALU_HYPRE_MEMORY_DEVICE, NALU_HYPRE_MEMORY_DEVICE);
   ...
   hypre_TMemcpy(P_a,            P_diag_a, NALU_HYPRE_Real, nnz_diag, NALU_HYPRE_MEMORY_DEVICE,
                 NALU_HYPRE_MEMORY_DEVICE);
   hypre_TMemcpy(P_a + nnz_diag, P_offd_a, NALU_HYPRE_Real, nnz_offd, NALU_HYPRE_MEMORY_DEVICE,
                 NALU_HYPRE_MEMORY_DEVICE);
   ...
   hypre_CSRMatrixJ   (P_diag) = hypre_TReAlloc_v2(P_diag_j, NALU_HYPRE_Int,  nnz_diag, NALU_HYPRE_Int,
                                                   new_nnz_diag, memory_location);
   hypre_CSRMatrixData(P_diag) = hypre_TReAlloc_v2(P_diag_a, NALU_HYPRE_Real, nnz_diag, NALU_HYPRE_Real,
                                                   new_nnz_diag, memory_location);
   hypre_CSRMatrixJ   (P_offd) = hypre_TReAlloc_v2(P_offd_j, NALU_HYPRE_Int,  nnz_offd, NALU_HYPRE_Int,
                                                   new_nnz_offd, memory_location);
   hypre_CSRMatrixData(P_offd) = hypre_TReAlloc_v2(P_offd_a, NALU_HYPRE_Real, nnz_offd, NALU_HYPRE_Real,
                                                   new_nnz_offd, memory_location);
   hypre_CSRMatrixNumNonzeros(P_diag) = new_nnz_diag;
   hypre_CSRMatrixNumNonzeros(P_offd) = new_nnz_offd;

   hypre_TFree(P_i,       NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(P_j,       NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(P_a,       NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(P_rowptr,  NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(tmp_rowid, NALU_HYPRE_MEMORY_DEVICE);
