...
NALU_HYPRE_SStructVectorDestroy( NALU_HYPRE_SStructVector vector )
{
   ...
   if (vector)
   {
      memory_location = nalu_hypre_SStructVectorMemoryLocation(vector);
...
NALU_HYPRE_SStructVectorInitialize( NALU_HYPRE_SStructVector vector )
{
   ...
   NALU_HYPRE_IJVectorGetObject(ijvector, (void **) &nalu_hypre_SStructVectorParVector(vector));
                                 ...
                                 NALU_HYPRE_Complex       *values )
{
   ...
   NALU_HYPRE_Int           i, d, vindex[NALU_HYPRE_MAXDIM];
                                   ...
                                   NALU_HYPRE_Complex       *values)
{
   ...
   NALU_HYPRE_Int             i, d, vilower[NALU_HYPRE_MAXDIM], viupper[NALU_HYPRE_MAXDIM];
   ...
   for (i = 0; i < fem_nvars; i++)
   {
      ...
      NALU_HYPRE_SStructVectorAddToBoxValues(vector, part, vilower, viupper,
                                        fem_vars[i], tvalues);
   }
