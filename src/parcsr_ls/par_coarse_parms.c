                           ...
                           NALU_HYPRE_BigInt    *coarse_pnts_global)
{
   ...
   NALU_HYPRE_ExecutionPolicy exec = nalu_hypre_GetExecPolicy1(nalu_hypre_IntArrayMemoryLocation(CF_marker));
   if (num_functions > 1)
   {
   ...
   {
      ierr = nalu_hypre_BoomerAMGCoarseParmsDevice(comm, local_num_variables, num_functions, dof_func,
                                              CF_marker, coarse_dof_func_ptr, coarse_pnts_global);
   }
   else
#endif
   {
      ierr = nalu_hypre_BoomerAMGCoarseParmsHost(comm, local_num_variables, num_functions, dof_func,
                                            CF_marker, coarse_dof_func_ptr, coarse_pnts_global);
   }
