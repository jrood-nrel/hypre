                                   ...
                                   T                *d_y )
{
   ...
   const NALU_HYPRE_Int  grid_ngroups  = item.get_group_range(2) * (NALU_HYPRE_SPMV_BLOCKDIM / K);
   NALU_HYPRE_Int        grid_group_id = (item.get_group(2) * NALU_HYPRE_SPMV_BLOCKDIM + item.get_local_id(
                                        2)) / K;
   const NALU_HYPRE_Int  group_lane    = item.get_local_id(2) & (K - 1);
#else
...
{
#if defined(NALU_HYPRE_USING_SYCL)
   NALU_HYPRE_Int grid_ngroups  = item.get_group_range(2) * (NALU_HYPRE_SPMV_BLOCKDIM / K);
   NALU_HYPRE_Int grid_group_id = (item.get_group(2) * NALU_HYPRE_SPMV_BLOCKDIM + item.get_local_id(2)) / K;
   NALU_HYPRE_Int group_lane    = item.get_local_id(2) & (K - 1);
#else
   const NALU_HYPRE_Int  grid_ngroups  = gridDim.x * (NALU_HYPRE_SPMV_BLOCKDIM / K);
   NALU_HYPRE_Int        grid_group_id = (blockIdx.x * NALU_HYPRE_SPMV_BLOCKDIM + threadIdx.x) / K;
   const NALU_HYPRE_Int  group_lane    = threadIdx.x & (K - 1);
                             ...
                             T         *d_y )
{
   ...
   const dim3 bDim = nalu_hypre_dim3(NALU_HYPRE_SPMV_BLOCKDIM);
