                ...
                nalu_hypre_ParVector    *u )
{
   ...
   NALU_HYPRE_BigInt         coarse_pnts_global[2]; // TODO: Change to row_starts_cpts
   NALU_HYPRE_BigInt         row_starts_fpts[2];
   nalu_hypre_Vector       **l1_norms = NULL;
   NALU_HYPRE_Real          *l1_norms_data;
   ...
   NALU_HYPRE_Int (*fgrid_solver_setup)(void*, void*, void*, void*);
   NALU_HYPRE_Int (*fgrid_solver_solve)(void*, void*, void*, void*);
   ...
   NALU_HYPRE_Int (*cgrid_solver_setup)(void*, void*, void*, void*) =
      (NALU_HYPRE_Int (*)(void*, void*, void*, void*)) (mgr_data -> coarse_grid_solver_setup);

   NALU_HYPRE_Int (*cgrid_solver_solve)(void*, void*, void*, void*) =
      (NALU_HYPRE_Int (*)(void*, void*, void*, void*)) (mgr_data -> coarse_grid_solver_solve);
   ...
   nalu_hypre_GpuProfilingPushRange("MGRSetup");
   nalu_hypre_GpuProfilingPushRange("MGRSetup-Init");
   ...
   NALU_HYPRE_Int lvl_to_keep_cpoints = ((mgr_data -> lvl_to_keep_cpoints) >
   ...
   if (block_size < 2 || (mgr_data -> max_num_coarse_levels) < 1)
   {
      ...
      if (use_default_cgrid_solver)
      {
         ...
         cgrid_solver_setup = (NALU_HYPRE_Int (*)(void*, void*, void*, void*)) nalu_hypre_BoomerAMGSetup;
         cgrid_solver_solve = (NALU_HYPRE_Int (*)(void*, void*, void*, void*)) nalu_hypre_BoomerAMGSolve;
   ...
   if (reserved_coarse_size > 0)
   {
      (mgr_data -> reserved_Cpoint_local_indexes) = nalu_hypre_CTAlloc(NALU_HYPRE_Int,
       ...
       P_array || RT_array || CF_marker_array)
   {
      ...
      for (j = 0; j < old_num_coarse_levels; j++)
      {
         if (B_array[j])
         {
            nalu_hypre_ParCSRMatrixDestroy(B_array[j]);
            B_array[j] = NULL;
         }

         if (B_FF_array[j])
         {
            nalu_hypre_ParCSRMatrixDestroy(B_FF_array[j]);
            B_FF_array[j] = NULL;
         }
      ...
      nalu_hypre_TFree(B_array, NALU_HYPRE_MEMORY_HOST);
      B_array = NULL;
      nalu_hypre_TFree(B_FF_array, NALU_HYPRE_MEMORY_HOST);
      B_FF_array = NULL;
   ...
   if (B_array == NULL)
   {
      B_array = nalu_hypre_CTAlloc(nalu_hypre_ParCSRMatrix*, max_num_coarse_levels, NALU_HYPRE_MEMORY_HOST);
   }
   ...
   if (l1_norms == NULL)
   {
      l1_norms = nalu_hypre_CTAlloc(nalu_hypre_Vector*, max_num_coarse_levels, NALU_HYPRE_MEMORY_HOST);
   }
   ...
   */
   ...
   for (i = 0; i < max_num_coarse_levels; i++)
   {
      if (Frelax_type[i] == 0 && interp_type && interp_type[i] != 12)
      {
         ...
         if (print_level)
         {
            nalu_hypre_ParPrintf(comm, "Changing F-relaxation type to 7 at MGR level %d\n", i);
         }
      }
   }
#endif
   ...
   */
   ...
   {
      NALU_HYPRE_Int *level_interp_type   = nalu_hypre_CTAlloc(NALU_HYPRE_Int, max_num_coarse_levels,
                                                     NALU_HYPRE_MEMORY_HOST);
      NALU_HYPRE_Int *level_restrict_type = nalu_hypre_CTAlloc(NALU_HYPRE_Int, max_num_coarse_levels,
                                                     NALU_HYPRE_MEMORY_HOST);
      NALU_HYPRE_Int *level_frelax_type   = nalu_hypre_CTAlloc(NALU_HYPRE_Int, max_num_coarse_levels,
                                                     NALU_HYPRE_MEMORY_HOST);
      for (i = 0; i < max_num_coarse_levels; i++)
      {
   ...
   if (A_ff_array)
   {
      ...
      if (mgr_data -> fsolver_mode != 0)
      {
         if (A_ff_array[0])
         {
            nalu_hypre_ParCSRMatrixDestroy(A_ff_array[0]);
            A_ff_array[0] = NULL;
         }
      }
   ...
   if (aff_solver)
   {
      for (j = 1; j < (old_num_coarse_levels); j++)
      {
         if (aff_solver[j])
         {
            nalu_hypre_BoomerAMGDestroy(aff_solver[j]);
         }
      }
      if (mgr_data -> fsolver_mode == 2)
      {
         if (aff_solver[0])
         {
            nalu_hypre_BoomerAMGDestroy(aff_solver[0]);
         }
      }
   }
   ...
   {
      fgrid_solver_setup = (NALU_HYPRE_Int (*)(void*, void*, void*, void*)) nalu_hypre_BoomerAMGSetup;
   ...
   {
      fgrid_solver_solve = (NALU_HYPRE_Int (*)(void*, void*, void*, void*)) nalu_hypre_BoomerAMGSolve;
   ...
   {
      F_fine_array = nalu_hypre_CTAlloc(nalu_hypre_ParVector*, max_num_coarse_levels + 1,
                                   NALU_HYPRE_MEMORY_HOST);
   }
   if (U_fine_array == NULL)
   {
      U_fine_array = nalu_hypre_CTAlloc(nalu_hypre_ParVector*, max_num_coarse_levels + 1,
                                   NALU_HYPRE_MEMORY_HOST);
   }
   if (aff_solver == NULL)
   {
   ...
   if (B_FF_array == NULL)
   {
      B_FF_array = nalu_hypre_CTAlloc(nalu_hypre_ParCSRMatrix*, max_num_coarse_levels, NALU_HYPRE_MEMORY_HOST);
   }
...
#if MGR_DEBUG_LEVEL == 1
   nalu_hypre_sprintf(fname, "MGR-A_lvl_00.IJ.out");
   nalu_hypre_ParPrintf(comm, "Dumping matrix to file: %s\n", fname);
   ...
   {
      nalu_hypre_sprintf(region_name, "%s-%d", "MGR_Level", lev);
      ...
      /* Initialize A_array */
      A_array[lev] = RAP_ptr;
      nloc = nalu_hypre_ParCSRMatrixNumRows(A_array[lev]);
      ...
      if (level_smooth_iters[lev] > 0)
      {
         /* TODO (VPM): Change option types for block-Jacobi and block-GS to 30 and 31 and
            make them accessible through nalu_hypre_BoomerAMGRelax? */
         if (level_smooth_type[lev] == 0 || level_smooth_type[lev] == 1)
         {
            /* TODO (VPM): move this to nalu_hypre_MGRBlockRelaxSetup and change its declaration */
#if defined (NALU_HYPRE_USING_GPU)
            if (exec == NALU_HYPRE_EXEC_DEVICE)
            {
               if (!B_array[lev])
               {
                  nalu_hypre_ParCSRMatrixBlockDiagMatrixDevice(A_array[lev], level_blk_size,
         ...
         else if (level_smooth_type[lev] == 16)
         {
            ...
            NALU_HYPRE_ILUSetLocalReordering(level_smoother[lev], 1);
         ...
         else
         {
            ...
            nalu_hypre_BoomerAMGRelaxComputeL1Norms(A_array[lev], level_smooth_type[lev],
                                               0, 0, NULL, &l1_norms_data);
            if (l1_norms_data)
            {
               l1_norms[lev] = nalu_hypre_SeqVectorCreate(nloc);
               nalu_hypre_VectorData(l1_norms[lev]) = l1_norms_data;
               nalu_hypre_VectorMemoryLocation(l1_norms[lev]) = memory_location;

               if (print_level)
               {
                  nalu_hypre_ParPrintf(comm, "Setting l1_norms for global relax at MGR level %d\n", i);
               }
            }
         }
      }
      ...
      nalu_hypre_ParPrintf(comm, "Lev = %d, proc = %d - Global smoother setup: %f\n",
                      lev, my_id, wall_time);
#endif

      /* Compute strength matrix for interpolation operator
         use default parameters, to be modified later */
      nalu_hypre_sprintf(region_name, "Coarsen");
      nalu_hypre_GpuProfilingPushRange(region_name);
      NALU_HYPRE_ANNOTATE_REGION_BEGIN("%s", region_name);
      ...
      nalu_hypre_MGRCoarsen(S, A_array[lev], level_coarse_size[lev], level_coarse_indexes[lev],
                       debug_flag, &CF_marker_array[lev], cflag);
      CF_marker = nalu_hypre_IntArrayData(CF_marker_array[lev]);

#if MGR_DEBUG_LEVEL == 1
      nalu_hypre_sprintf(fname, "MGR-CF_marker_lvl_%02d.%05d", lev, my_id);
      nalu_hypre_ParPrintf(comm, "Dumping matrix to file: %s\n", fname);
      fout = fopen(fname, "w");
      for (i = 0; i < nloc; i++)
      {
         nalu_hypre_fprintf(fout, "%d %d\n", i, CF_marker[i]);
      }
      fclose(fout);
#endif

      /* Get global fine/coarse partitionings. TODO: generate dof_func */
      nalu_hypre_MGRCoarseParms(comm, nloc, CF_marker_array[lev],
      ...
      {
         nalu_hypre_ParCSRMatrixGenerateFFFC(A_array[lev], CF_marker, coarse_pnts_global,
                                        NULL, &A_FC, &A_FF);
      }
      ...
      nalu_hypre_sprintf(region_name, "Interp");
      nalu_hypre_GpuProfilingPushRange(region_name);
      NALU_HYPRE_ANNOTATE_REGION_BEGIN("%s", region_name);
      if (interp_type[lev] == 12)
      {
         ...
         {
            nalu_hypre_MGRBuildBlockJacobiWp(A_FF, A_FC, block_jacobi_bsize,
                                        CF_marker, coarse_pnts_global, &Wp);
         }
         nalu_hypre_MGRBuildInterp(A_array[lev], A_FF, A_FC, CF_marker, Wp,
      ...
      {
         nalu_hypre_MGRBuildInterp(A_array[lev], A_FF, A_FC, CF_marker, S,
                              coarse_pnts_global, 1, dof_func_buff_data,
                              debug_flag, trunc_factor, max_elmts,
                              block_jacobi_bsize, &P, interp_type[lev],
                              num_interp_sweeps);
...
#if MGR_DEBUG_LEVEL == 2
      wall_time = time_getWallclockSeconds() - wall_time;
      nalu_hypre_ParPrintf(comm, "Lev = %d, interp type = %d, proc = %d - Build Wp: %f\n",
                      lev, interp_type[lev], my_id, wall_time);
#endif

#if MGR_DEBUG_LEVEL == 1
      nalu_hypre_sprintf(fname, "MGR-P_lvl_%02d.IJ.out", lev);
      nalu_hypre_ParPrintf(comm, "Dumping matrix to file: %s\n", fname);
      ...
      if (interp_type[lev] == 12 && (mgr_data -> num_relax_sweeps)[lev] > 0)
      {
         ...
         if (exec == NALU_HYPRE_EXEC_DEVICE)
         {
            nalu_hypre_ParCSRMatrixBlockDiagMatrixDevice(A_FF, block_jacobi_bsize,
...
#endif
         {
            ...
            NALU_HYPRE_Int   inv_num_rows;
            NALU_HYPRE_Int   inv_size;

            /* TODO: replace this with nalu_hypre_IntArrayCount (VPM) */
            inv_num_rows = 0;
            for (i = 0; i < nloc; i++)
            {
            ...
            diag_inv = nalu_hypre_CTAlloc(NALU_HYPRE_Complex, inv_size, NALU_HYPRE_MEMORY_HOST);

            /* TODO: Extend this to device (VPM) */
            nalu_hypre_ParCSRMatrixExtractBlockDiagHost(A_array[lev], block_jacobi_bsize, inv_num_rows,
                                                   -1, CF_marker, inv_size, 1, diag_inv);
            frelax_diaginv[lev] = diag_inv;
            blk_size[lev] = block_jacobi_bsize;
            nalu_hypre_MGRBuildAff(A_array[lev], CF_marker, debug_flag, &A_FF);
         }
         ...
         F_fine_array[lev + 1] =
            nalu_hypre_ParVectorCreate(nalu_hypre_ParCSRMatrixComm(A_FF),
                                  nalu_hypre_ParCSRMatrixGlobalNumRows(A_FF),
                                  nalu_hypre_ParCSRMatrixRowStarts(A_FF));
      ...
      if (use_air)
      {
         ...
         */
         ...
         nalu_hypre_sprintf(region_name, "RAP");
         nalu_hypre_GpuProfilingPushRange(region_name);
         NALU_HYPRE_ANNOTATE_REGION_BEGIN("%s", region_name);
      ...
      else
      {
         if (mgr_coarse_grid_method[lev] != 0)
         {
            NALU_HYPRE_Int block_num_f_points = level_blk_size - block_num_coarse_indexes[lev];
            ...
            {
               nalu_hypre_MGRBuildRestrict(A_array[lev], A_FF, A_FC, CF_marker, coarse_pnts_global, 1,
            ...
            if (exec == NALU_HYPRE_EXEC_DEVICE)
            {
               nalu_hypre_ParCSRMatrixGenerateCCCFDevice(RAP_ptr, CF_marker,
                                                    coarse_pnts_global,
                                                    S, &A_CF, &A_CC);

               nalu_hypre_MGRComputeNonGalerkinCGDevice(A_FF, A_FC, A_CF, A_CC,
                                                   block_num_f_points,
...
#endif
            {
               nalu_hypre_MGRComputeNonGalerkinCoarseGrid(A_array[lev], Wp, RT,
                                                     block_num_f_points,
                                                     set_c_points_method,
                                                     mgr_coarse_grid_method[lev],
...
#if MGR_DEBUG_LEVEL == 2
            wall_time = time_getWallclockSeconds() - wall_time;
            nalu_hypre_ParPrintf(comm, "Lev = %d, proc = %d - BuildCoarseGrid: %f\n",
                            lev, my_id, wall_time);
#endif
         }
         else
         {
            ...
            nalu_hypre_MGRBuildRestrict(A_array[lev], A_FF, A_FC, CF_marker,
                                   coarse_pnts_global, 1, dof_func_buff_data,
                                   debug_flag, trunc_factor, max_elmts,
...
#if MGR_DEBUG_LEVEL == 2
            wall_time = time_getWallclockSeconds() - wall_time;
            nalu_hypre_ParPrintf(comm, "Lev = %d, restrict type = %d, proc = %d - BuildRestrict: %f\n",
...
#if MGR_DEBUG_LEVEL == 1
      nalu_hypre_sprintf(fname, "MGR-A_lvl_%02d.IJ.out", lev + 1);
      nalu_hypre_ParPrintf(comm, "Dumping matrix to file: %s\n", fname);
      nalu_hypre_ParCSRMatrixPrintIJ(RAP_ptr, 0, 0, fname);
#endif
          ...
          Frelax_type[lev] == 199)
      {
         ...
         if (lev == 0 && (mgr_data -> fsolver_mode) == 0)
         {
            if (Frelax_type[lev] == 2)
            {
               if (((nalu_hypre_ParAMGData*)aff_solver[lev])->A_array != NULL)
               {
                  if (((nalu_hypre_ParAMGData*)aff_solver[lev])->A_array[0] != NULL)
                  {
                     ...
                     A_ff_array[lev] = ((nalu_hypre_ParAMGData*) aff_solver[lev]) -> A_array[0];
                  }
                  else
                  {
            ...
            {
               nalu_hypre_sprintf(msg, "Warning!! User-prescribed F-solver for the first level\n\
                             reduction (set in NALU_HYPRE_MGRSetFSolver()) only supports AMG\n\
                             Ignoring this call and using user prescribed Frelax_type %d",
                             Frelax_type[lev]);
               nalu_hypre_error_w_msg(0, msg);
            }
         }
...
#if MGR_DEBUG_LEVEL == 2
         wall_time = time_getWallclockSeconds() - wall_time;
         nalu_hypre_ParPrintf(comm, "Lev = %d, proc = %d - SetupAFF: %f\n", lev, my_id, wall_time);
#endif

         /* TODO: refactor this block (VPM) */
#if defined (NALU_HYPRE_USING_GPU)
         nalu_hypre_ParCSRMatrix  *P_FF_ptr;
         nalu_hypre_IntArray      *F_marker = nalu_hypre_IntArrayCloneDeep(CF_marker_array[lev]);

         nalu_hypre_IntArrayNegate(F_marker);
         nalu_hypre_MGRBuildPDevice(A_array[lev], nalu_hypre_IntArrayData(F_marker),
         ...
         F_fine_array[lev + 1] =
            nalu_hypre_ParVectorCreate(nalu_hypre_ParCSRMatrixComm(A_ff_array[lev]),
                                  nalu_hypre_ParCSRMatrixGlobalNumRows(A_ff_array[lev]),
                                  nalu_hypre_ParCSRMatrixRowStarts(A_ff_array[lev]));
         nalu_hypre_ParVectorInitialize(F_fine_array[lev + 1]);
      ...
      {
         memory_location = nalu_hypre_IntArrayMemoryLocation(CF_marker_array[lev]);
         if (nalu_hypre_GetActualMemLocation(memory_location) == nalu_hypre_MEMORY_DEVICE)
         {
            nalu_hypre_IntArrayMigrate(CF_marker_array[lev], NALU_HYPRE_MEMORY_HOST);
         }
         CF_marker = nalu_hypre_IntArrayData(CF_marker_array[lev]);
         ...
         if (nalu_hypre_GetActualMemLocation(memory_location) == nalu_hypre_MEMORY_DEVICE)
         {
            nalu_hypre_IntArrayMigrate(CF_marker_array[lev], NALU_HYPRE_MEMORY_DEVICE);
         }
      }
      ...
      nalu_hypre_ParPrintf(comm, "Lev = %d, proc = %d - Setup time: %f\n",
   ...
   if (reserved_coarse_size > 0 && lvl_to_keep_cpoints == 0)
   {
      ...
      NALU_HYPRE_BoomerAMGSetCPoints((mgr_data ->coarse_grid_solver),
   ...
   wall_time = time_getWallclockSeconds();
#endif
   nalu_hypre_sprintf(region_name, "%s-%d", "MGR_Level", num_c_levels);
   nalu_hypre_GpuProfilingPushRange(region_name);
   NALU_HYPRE_ANNOTATE_REGION_BEGIN("%s", region_name);
...
#if MGR_DEBUG_LEVEL == 2
   wall_time = time_getWallclockSeconds() - wall_time;
   nalu_hypre_ParPrintf(comm, "Proc = %d - Coarse grid setup: %f\n", my_id, wall_time);
#endif
   ...
   for (j = 0; j < num_c_levels; j++)
   {
      ...
      {
         /* Compute l1_norms according to relaxation type */
         nalu_hypre_BoomerAMGRelaxComputeL1Norms(A_array[j], Frelax_type[j],
                                            relax_order, 0, CF_marker_array[j],
                                            &l1_norms_data);
         if (l1_norms_data)
         {
            nloc = nalu_hypre_ParCSRMatrixNumRows(A_array[j]);
            l1_norms[j] = nalu_hypre_SeqVectorCreate(nloc);
            nalu_hypre_VectorData(l1_norms[j]) = l1_norms_data;
            nalu_hypre_VectorMemoryLocation(l1_norms[j]) = memory_location;
         }
      }
   }
   ...
   if (use_VcycleSmoother)
   {
      ...
      FrelaxVcycleData = nalu_hypre_TAlloc(nalu_hypre_ParAMGData*, max_num_coarse_levels,
                                      NALU_HYPRE_MEMORY_HOST);
      (mgr_data -> FrelaxVcycleData) = FrelaxVcycleData;

      /* Setup temporary storage - TODO (VPM): Use nalu_hypre_ParVectorInitialize_v2 */
   ...
   else if (use_GSElimSmoother)
   {
      ...
      GSElimData = nalu_hypre_CTAlloc(nalu_hypre_ParAMGData*, max_num_coarse_levels,
                                 NALU_HYPRE_MEMORY_HOST);
      ...
      for (i = 0; i < (mgr_data->num_coarse_levels); i++)
      {
         ...
         {
            GSElimData[i] = (nalu_hypre_ParAMGData*) nalu_hypre_MGRCreateGSElimData();
            ...
            // nalu_hypre_printf("Setting GSElim Solver %d \n", Frelax_type[i]);
            nalu_hypre_GaussElimSetup(GSElimData[i], i, Frelax_type[i]);
         }
      }
   }
...
/*--------------------------------------------------------------------------
 * nalu_hypre_MGRSetupFrelaxVcycleData
 *
                                ...
                                NALU_HYPRE_Int           lev )
{
   ...
   local_size = nalu_hypre_ParCSRMatrixNumRows(A);
   ...
   while (not_finished)
   {
      ...
      if (lev_local == 0)
      {
         ...
         nalu_hypre_BoomerAMGCreateSFromCFMarker(A_array_local[lev_local], strong_threshold,
                                            max_row_sum,
      ...
      {
         nalu_hypre_BoomerAMGCreateS(A_array_local[lev_local], strong_threshold,
      ...
      nalu_hypre_BoomerAMGCoarsenHMIS(S_local, A_array_local[lev_local], measure_type,
