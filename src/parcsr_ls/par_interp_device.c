...
__global__ void hypre_BoomerAMGBuildDirInterp_getnnz( hypre_DeviceItem &item, NALU_HYPRE_Int nr_of_rows,
...
__global__ void hypre_BoomerAMGBuildDirInterp_getcoef( hypre_DeviceItem &item, NALU_HYPRE_Int nr_of_rows,
...
__global__ void
hypre_BoomerAMGBuildInterpOnePnt_getnnz( hypre_DeviceItem &item, NALU_HYPRE_Int nr_of_rows,
                                         NALU_HYPRE_Int *A_diag_i,
                                     ...
                                     hypre_ParCSRMatrix  **P_ptr)
{
   ...
   NALU_HYPRE_Int          *tmp_map_offd_h = NULL;
   ...
   if (num_cols_A_offd)
   {
      CF_marker_offd = hypre_TAlloc(NALU_HYPRE_Int, num_cols_A_offd, NALU_HYPRE_MEMORY_DEVICE);
   }

   num_sends = hypre_ParCSRCommPkgNumSends(comm_pkg);
   int_buf_data = hypre_TAlloc(NALU_HYPRE_Int, hypre_ParCSRCommPkgSendMapStart(comm_pkg, num_sends),
   ...
   if (num_functions > 1)
   {
      ...
      {
         dof_func_offd = hypre_TAlloc(NALU_HYPRE_Int, num_cols_A_offd, NALU_HYPRE_MEMORY_DEVICE);
      }
   ...
   P_diag_i = hypre_TAlloc(NALU_HYPRE_Int, n_fine + 1, memory_location);
   P_offd_i = hypre_TAlloc(NALU_HYPRE_Int, n_fine + 1, memory_location);

   dim3 bDim = hypre_GetDefaultDeviceBlockDimension();
   dim3 gDim = hypre_GetDefaultDeviceGridDimension(n_fine, "warp", bDim);

   NALU_HYPRE_GPU_LAUNCH( hypre_BoomerAMGBuildDirInterp_getnnz, gDim, bDim,
   ...
   hypre_Memset(P_diag_i + n_fine, 0, sizeof(NALU_HYPRE_Int), NALU_HYPRE_MEMORY_DEVICE);
   hypre_Memset(P_offd_i + n_fine, 0, sizeof(NALU_HYPRE_Int), NALU_HYPRE_MEMORY_DEVICE);

   hypreDevice_IntegerExclusiveScan(n_fine + 1, P_diag_i);
   hypreDevice_IntegerExclusiveScan(n_fine + 1, P_offd_i);

   fine_to_coarse_d = hypre_TAlloc(NALU_HYPRE_Int, n_fine, NALU_HYPRE_MEMORY_DEVICE);
   ...
   hypre_TMemcpy(&P_diag_size, &P_diag_i[n_fine], NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST, memory_location);
   hypre_TMemcpy(&P_offd_size, &P_offd_i[n_fine], NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST, memory_location);

   P_diag_j    = hypre_TAlloc(NALU_HYPRE_Int,  P_diag_size, memory_location);
   P_diag_data = hypre_TAlloc(NALU_HYPRE_Real, P_diag_size, memory_location);

   P_offd_j    = hypre_TAlloc(NALU_HYPRE_Int,  P_offd_size, memory_location);
   P_offd_data = hypre_TAlloc(NALU_HYPRE_Real, P_offd_size, memory_location);

   if (interp_type == 3)
   {
      NALU_HYPRE_GPU_LAUNCH( hypre_BoomerAMGBuildDirInterp_getcoef, gDim, bDim,
   ...
   {
      NALU_HYPRE_GPU_LAUNCH( hypre_BoomerAMGBuildDirInterp_getcoef_v2, gDim, bDim,
                        n_fine, A_diag_i, A_diag_j, A_diag_data,
                        A_offd_i, A_offd_j, A_offd_data,
                        Soc_diag_j,
                        Soc_offd_j,
   ...
   /* !!!! Free them here */
   /*
   hypre_TFree(hypre_ParCSRMatrixSocDiagJ(S), NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(hypre_ParCSRMatrixSocOffdJ(S), NALU_HYPRE_MEMORY_DEVICE);
   */
   ...
   if (P_offd_size)
   {
      ...
      NALU_HYPRE_Int *P_marker = hypre_TAlloc(NALU_HYPRE_Int, num_cols_A_offd, NALU_HYPRE_MEMORY_DEVICE);
      NALU_HYPRE_Int *P_colids = hypre_TAlloc(NALU_HYPRE_Int, hypre_max(P_offd_size, num_cols_A_offd),
                                         NALU_HYPRE_MEMORY_DEVICE);

      hypre_TMemcpy(P_colids, P_offd_j, NALU_HYPRE_Int, P_offd_size, NALU_HYPRE_MEMORY_DEVICE, memory_location);
      /* sort and unique */
#if defined(NALU_HYPRE_USING_SYCL)
      NALU_HYPRE_ONEDPL_CALL(std::sort, P_colids, P_colids + P_offd_size);
      NALU_HYPRE_Int *new_end = NALU_HYPRE_ONEDPL_CALL(std::unique, P_colids, P_colids + P_offd_size);
#else
      NALU_HYPRE_THRUST_CALL(sort, P_colids, P_colids + P_offd_size);
      NALU_HYPRE_Int *new_end = NALU_HYPRE_THRUST_CALL(unique, P_colids, P_colids + P_offd_size);
#endif
...
#if defined(NALU_HYPRE_USING_SYCL)
      NALU_HYPRE_ONEDPL_CALL(std::exclusive_scan, P_marker, P_marker + num_cols_A_offd, P_colids, 0);
#else
      NALU_HYPRE_THRUST_CALL(exclusive_scan, P_marker, P_marker + num_cols_A_offd, P_colids);
#endif
...
#else
      NALU_HYPRE_THRUST_CALL(gather, P_offd_j, P_offd_j + P_offd_size, P_colids, P_offd_j);
#endif
      ...
      tmp_map_offd_h = hypre_TAlloc(NALU_HYPRE_Int, num_cols_P_offd, NALU_HYPRE_MEMORY_HOST);
      hypre_TMemcpy(tmp_map_offd_h, P_colids, NALU_HYPRE_Int, num_cols_P_offd, NALU_HYPRE_MEMORY_HOST,
                    NALU_HYPRE_MEMORY_DEVICE);

      hypre_TFree(P_colids, NALU_HYPRE_MEMORY_DEVICE);
      hypre_TFree(P_marker, NALU_HYPRE_MEMORY_DEVICE);
   }
   ...
   fine_to_coarse_h = hypre_TAlloc(NALU_HYPRE_Int, n_fine, NALU_HYPRE_MEMORY_HOST);
   hypre_TMemcpy(fine_to_coarse_h, fine_to_coarse_d, NALU_HYPRE_Int, n_fine, NALU_HYPRE_MEMORY_HOST,
                 NALU_HYPRE_MEMORY_DEVICE);

   hypre_ParCSRMatrixColMapOffd(P) = hypre_CTAlloc(NALU_HYPRE_BigInt, num_cols_P_offd, NALU_HYPRE_MEMORY_HOST);
   ...
   *P_ptr = P;
   ...
   hypre_TFree(CF_marker_offd,   NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(dof_func_offd,    NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(int_buf_data,     NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(fine_to_coarse_d, NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(fine_to_coarse_h, NALU_HYPRE_MEMORY_HOST);
   hypre_TFree(tmp_map_offd_h,   NALU_HYPRE_MEMORY_HOST);
                                      ...
                                      NALU_HYPRE_Int *P_offd_i)
{
   ...
   NALU_HYPRE_Int i = hypre_gpu_get_grid_warp_id<1, 1>(item);
                                        ...
                                        hypre_ParCSRMatrix **P_ptr)
{
   ...
   hypre_CSRMatrix         *A_diag          = hypre_ParCSRMatrixDiag(A);
   NALU_HYPRE_Int               *A_diag_i        = hypre_CSRMatrixI(A_diag);
   NALU_HYPRE_Int               *A_strong_diag_j = hypre_ParCSRMatrixSocDiagJ(S);
   NALU_HYPRE_Complex           *A_diag_a        = hypre_CSRMatrixData(A_diag);

   hypre_CSRMatrix         *A_offd          = hypre_ParCSRMatrixOffd(A);
   NALU_HYPRE_Int               *A_offd_i        = hypre_CSRMatrixI(A_offd);
   NALU_HYPRE_Int               *A_strong_offd_j = hypre_ParCSRMatrixSocOffdJ(S);
   ...
   //NALU_HYPRE_Int col_start = hypre_ParCSRMatrixFirstRowIndex(A);
   ...
   if (num_cols_A_offd)
   {
      CF_marker_offd = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_A_offd, NALU_HYPRE_MEMORY_DEVICE);
   }
   ...
   diag_compress_marker = hypre_CTAlloc(NALU_HYPRE_Int, n_fine, NALU_HYPRE_MEMORY_DEVICE);
   offd_compress_marker = hypre_CTAlloc(NALU_HYPRE_Int, n_fine, NALU_HYPRE_MEMORY_DEVICE);

   /* Overallocate here and compress later */
   P_diag_j_temp = hypre_CTAlloc(NALU_HYPRE_Int, n_fine, NALU_HYPRE_MEMORY_DEVICE);
   P_offd_j_temp = hypre_CTAlloc(NALU_HYPRE_Int, n_fine, NALU_HYPRE_MEMORY_DEVICE);

   dim3 bDim = hypre_GetDefaultDeviceBlockDimension();
   dim3 gDim = hypre_GetDefaultDeviceGridDimension(n_fine, "warp", bDim);

   NALU_HYPRE_GPU_LAUNCH( hypre_BoomerAMGBuildInterpOnePnt_getnnz, gDim, bDim,
    ...
    *-----------------------------------------------------------------------*/
   fine_to_coarse_offd = hypre_CTAlloc(NALU_HYPRE_BigInt, num_cols_A_offd, NALU_HYPRE_MEMORY_DEVICE);
   big_int_buf_data = hypre_CTAlloc(NALU_HYPRE_BigInt, hypre_ParCSRCommPkgSendMapStart(comm_pkg, num_sends),
                                    NALU_HYPRE_MEMORY_DEVICE);
#if defined(NALU_HYPRE_USING_SYCL)
   hypreSycl_gather( hypre_ParCSRCommPkgDeviceSendMapElmts(comm_pkg),
                     hypre_ParCSRCommPkgDeviceSendMapElmts(comm_pkg) +
   ...
   comm_handle = hypre_ParCSRCommHandleCreate_v2(21, comm_pkg, NALU_HYPRE_MEMORY_DEVICE, big_int_buf_data,
                                                 NALU_HYPRE_MEMORY_DEVICE, fine_to_coarse_offd);
   hypre_ParCSRCommHandleDestroy(comm_handle);
   hypre_TFree(big_int_buf_data, NALU_HYPRE_MEMORY_DEVICE);
   ...
   hypre_TMemcpy(&nnz_diag, &P_diag_i[n_fine], NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
   hypre_TMemcpy(&nnz_offd, &P_offd_i[n_fine], NALU_HYPRE_Int, 1, NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
   ...
   P_diag_j_temp_compressed = hypre_TAlloc(NALU_HYPRE_Int, nnz_diag, NALU_HYPRE_MEMORY_DEVICE);
   P_offd_j_temp_compressed = hypre_TAlloc(NALU_HYPRE_Int, nnz_offd, NALU_HYPRE_MEMORY_DEVICE);
   ...
   hypre_TFree(P_diag_j_temp_compressed, NALU_HYPRE_MEMORY_DEVICE);

   /* mark the offd indices for P as a subset of offd indices of A */
   NALU_HYPRE_Int *mark_P_offd_idx = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_A_offd, NALU_HYPRE_MEMORY_DEVICE);
   ...
   NALU_HYPRE_Int *offd_map_P_to_A = hypre_CTAlloc(NALU_HYPRE_Int, num_cols_P_offd, NALU_HYPRE_MEMORY_DEVICE);
#if defined(NALU_HYPRE_USING_SYCL)
   oneapi::dpl::counting_iterator<NALU_HYPRE_Int> count(0);
   ...
   NALU_HYPRE_THRUST_CALL( copy_if,
                      ...
                      equal<NALU_HYPRE_Int>(1) );
#endif
   hypre_TFree(mark_P_offd_idx, NALU_HYPRE_MEMORY_DEVICE);
   ...
   NALU_HYPRE_Int *offd_map_A_to_P = hypre_TAlloc(NALU_HYPRE_Int, num_cols_A_offd, NALU_HYPRE_MEMORY_DEVICE);
   ...
   NALU_HYPRE_THRUST_CALL( gather,
                      ...
                      P_offd_j );
#endif
   hypre_TFree(P_offd_j_temp_compressed, NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(offd_map_A_to_P, NALU_HYPRE_MEMORY_DEVICE);

   /* setup col_map_offd for P */
   col_map_offd_P_device = hypre_CTAlloc(NALU_HYPRE_BigInt, num_cols_P_offd, NALU_HYPRE_MEMORY_DEVICE);
   ...
   NALU_HYPRE_THRUST_CALL( gather,
                      ...
                      col_map_offd_P_device);
#endif
   hypre_TMemcpy(col_map_offd_P, col_map_offd_P_device, NALU_HYPRE_BigInt, num_cols_P_offd,
                 NALU_HYPRE_MEMORY_HOST, NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(offd_map_P_to_A, NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(col_map_offd_P_device, NALU_HYPRE_MEMORY_DEVICE);
   ...
   *P_ptr = P;
   ...
   hypre_TFree(diag_compress_marker, NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(offd_compress_marker, NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(P_diag_j_temp, NALU_HYPRE_MEMORY_DEVICE);
   hypre_TFree(P_offd_j_temp, NALU_HYPRE_MEMORY_DEVICE);
                                         ...
                                         NALU_HYPRE_Int     *P_offd_j)
{
   ...
   for (NALU_HYPRE_Int j = p + lane; j < q; j += NALU_HYPRE_WARP_SIZE)
   {
      ...
      if (col >= 0)
      {
         const NALU_HYPRE_Complex val = fabs( read_only_load(&A_diag_a[j]) );
   ...
   for (NALU_HYPRE_Int j = p + lane; j < q; j += NALU_HYPRE_WARP_SIZE)
   {
      ...
      if (col >= 0)
      {
         const NALU_HYPRE_Complex val = fabs( read_only_load(&A_offd_a[j]) );
