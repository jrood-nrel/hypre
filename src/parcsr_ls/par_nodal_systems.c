                            ...
                            nalu_hypre_ParCSRMatrix   **AN_ptr)
{
   ...
   switch (mode)
   {
      case 1:  /* frobenius norm */
      {
         ...
         {
            AN_diag_data[i] = nalu_hypre_sqrt(AN_diag_data[i]);
         }
            ...
            for (j = 0; j < num_functions; j++)
            {
               for (k = A_diag_i[row]; k < A_diag_i[row + 1]; k++)
               {
                  ...
                  if (counter[k_map] < start_index)
                  {
                     counter[k_map] = index;
                     AN_diag_j[index] = k_map;
                     AN_diag_data[index] = nalu_hypre_abs(A_diag_data[k]);
                     index++;
                  }
                  else
                  {
                     AN_diag_data[counter[k_map]] += nalu_hypre_abs(A_diag_data[k]);
                  }
               }
               row++;
            }
         ...
         for (i = 0; i < num_nodes; i++)
         {
            for (j = 0; j < num_functions; j++)
            {
               for (k = A_diag_i[row]; k < A_diag_i[row + 1]; k++)
               {
                  ...
                  {
                     if (nalu_hypre_abs(A_diag_data[k]) >
                         nalu_hypre_abs(AN_diag_data[counter[k_map]]))
                     {
            ...
            for (j = 0; j < num_functions; j++)
            {
               for (k = A_diag_i[row]; k < A_diag_i[row + 1]; k++)
               {
                  ...
                  if (counter[k_map] < start_index)
                  {
                     counter[k_map] = index;
                     AN_diag_j[index] = k_map;
                     data[index * num_functions + j] = nalu_hypre_abs(A_diag_data[k]);
                     index++;
                  }
                  else
                  {
                     data[(counter[k_map])*num_functions + j] += nalu_hypre_abs(A_diag_data[k]);
                  }
               }
               row++;
            }
   ...
   if (AN_num_nonzeros_offd)
   {
      ...
      switch (mode)
      {
         case 1: /* frobenius norm */
         {
            ...
            {
               AN_offd_data[i] = nalu_hypre_sqrt(AN_offd_data[i]);
            }
         }
         break;
               ...
               for (j = 0; j < num_functions; j++)
               {
                  for (k = A_offd_i[row]; k < A_offd_i[row + 1]; k++)
                  {
                     ...
                     if (counter[k_map] < start_index)
                     {
                        counter[k_map] = index;
                        AN_offd_j[index] = k_map;
                        AN_offd_data[index] = nalu_hypre_abs(A_offd_data[k]);
                        index++;
                     }
                     else
                     {
                        AN_offd_data[counter[k_map]] += nalu_hypre_abs(A_offd_data[k]);
                     }
                  }
                  row++;
               }
            ...
            for (i = 0; i < num_nodes; i++)
            {
               for (j = 0; j < num_functions; j++)
               {
                  for (k = A_offd_i[row]; k < A_offd_i[row + 1]; k++)
                  {
                     ...
                     {
                        if (nalu_hypre_abs(A_offd_data[k]) >
                            nalu_hypre_abs(AN_offd_data[counter[k_map]]))
                        {
               ...
               for (j = 0; j < num_functions; j++)
               {
                  for (k = A_offd_i[row]; k < A_offd_i[row + 1]; k++)
                  {
                     ...
                     if (counter[k_map] < start_index)
                     {
                        counter[k_map] = index;
                        AN_offd_j[index] = k_map;
                        data[index * num_functions + j] = nalu_hypre_abs(A_offd_data[k]);
                        index++;
                     }
                     else
                     {
                        data[(counter[k_map])*num_functions + j] += nalu_hypre_abs(A_offd_data[k]);
                     }
                  }
                  row++;
               }
